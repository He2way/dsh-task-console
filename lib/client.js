...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function applyCanvasSpec(canvasId, spec) {
			...
			const counts = { add: 0, addCard: 0, addPlugin: 0, rebuild: 0, update: 0, move: 0, remove: 0, rename: 0, note: 0 };
			const notes = [];
			...
			const changed = counts.add + counts.addCard + counts.addPlugin + counts.rebuild + counts.update + counts.move + counts.remove + counts.rename;
			const parts = [];
		...
		function CanvasControlView(props) {
			const { card, onMove, onResize, onEdit, onChat, onDelete, onWidgetState, onBlockResize, onBlockReplace, onDuplicate, onBoard, onToggleBare, canvasId, onRebuild } = props;
		...
		function CanvasCardView(props) {
			const { card, scale, onMove, onResize, onEdit, onChat, onDelete, onWidgetState, onBlockResize, onBlockReplace, onFull, onDuplicate, onBoard, onToggleBare, onRebuild } = props;
			const ref = (0, react.useRef)(null);
			const gesture = (0, react.useRef)(null);
			const frame = (0, react.useRef)(0);
			const begin = (mode, event) => {
		 ...
		 */
		function TptChronicleCardBody({ modules }) {
			const hostRef = (0, react.useRef)(null);
			...
			// Theme flips re-render the mounted panel (its own root), never remount it.
			(0, react.useEffect)(() => {
				const host = hostRef.current;
				if (host !== null) host.setAttribute("data-embed-dark", dark ? "1" : "0");
		...
		function TaskBackPanel(props) {
			...
			const cardView = (id) => {
				...
				if (id === "session") {
					...
					fill = true;
					body = (0, react_jsx_runtime.jsx)(TptChronicleCardBody, { modules });
