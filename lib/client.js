...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TASK_CSS_EXTRA = [
			...
			".dsh-tc-canvasChat{flex:none;display:flex;flex-direction:column;border-top:1px solid rgba(148,163,184,.24);background:rgba(255,255,255,.55)}",
			".dsh-tc-panel[data-theme=dark] .dsh-tc-canvasChat{background:rgba(255,255,255,.05)}",
			".dsh-tc-canvasChatBody{display:none;max-height:240px;overflow:auto;padding:10px 14px 4px;flex-direction:column;gap:6px}",
			".dsh-tc-canvasChatOpen .dsh-tc-canvasChatBody{display:flex}",
			".dsh-tc-canvasChatBar{display:flex;align-items:center;gap:8px;padding:8px 14px}",
			".dsh-tc-canvasChatInput{flex:1;min-width:0}",
		...
		function CanvasChatDock(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-tc-canvasChat" + (open ? " dsh-tc-canvasChatOpen" : ""),
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							(0, react_jsx_runtime.jsx)("button", {
								...
								children: open ? "▾ 记录" : "▴ 记录" + (log.length > 0 ? " " + log.length : "")
							}),
							(0, react_jsx_runtime.jsx)("input", {
								className: "dsh-tc-in dsh-tc-canvasChatInput",
								value: text,
								placeholder: "用一句话修改这块画布…（Enter 发送）",
								disabled: busy,
								onChange: (event) => setText(event.target.value),
								onKeyDown: (event) => {
