...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		function canvasExplodeBlocks(canvasId, cardId, blocks, style) {
			const state = canvasSnapshot();
			...
			for (let index = 0; index < blocks.length; index++) {
				...
				if (index === 0 && style !== void 0 && typeof style.icon === "string" && style.icon.length > 0) item.style = { icon: style.icon };
