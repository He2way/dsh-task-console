 ...
 * heading, note, stats, progress, trend, kv, links, chips, checklist, counter,
 * button, table, code, toggle, countdown, bars) plus an allow-listed card
 * style; conversations author and refactor them through [taskcard] blocks and
 * the top-right card chat, which calls a temporary agent session.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TASK_CSS_EXTRA = [
			...
			".dsh-tc-cardBusy{transition:none!important;will-change:transform;backdrop-filter:blur(8px) saturate(160%);-webkit-backdrop-filter:blur(8px) saturate(160%)}",
			".dsh-tc-interacting .dsh-tc-panel::after{animation-play-state:paused}",
			".dsh-tc-interacting .dsh-tc-card:not(.dsh-tc-cardBusy){transition:none}",
			".dsh-tc-panel{contain:paint}",
			"body.dsh-tc-interacting .dsh-tc-card{backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(255,255,255,.86)!important}",
			"body.dsh-tc-interacting[data-ds-dark-theme] .dsh-tc-card{background:rgba(26,30,46,.92)!important}",
			".dsh-tc-lite .dsh-tc-card{backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(255,255,255,.88)!important;animation:none!important}",
			"body[data-ds-dark-theme] .dsh-tc-lite .dsh-tc-card{background:rgba(26,30,46,.94)!important}",
			".dsh-tc-lite .dsh-tc-panel::before,.dsh-tc-lite .dsh-tc-panel::after{display:none}",
			".dsh-tc-lite .dsh-tc-toggle,.dsh-tc-lite .dsh-tc-editor,.dsh-tc-lite .dsh-tc-chat{backdrop-filter:none!important;-webkit-backdrop-filter:none!important}"
		].join("");
		...
		const TASK_LITE_KEY = "dsh.taskconsole.lite";
		/** Low-effects mode: no glass blur / ambient animation on the board (persisted per browser). */
		function readTaskLite() {
			if (typeof localStorage === "undefined") return false;
			try { return localStorage.getItem(TASK_LITE_KEY) === "1"; } catch { return false; }
		}
		function writeTaskLite(value) {
			if (typeof localStorage === "undefined") return;
			try { localStorage.setItem(TASK_LITE_KEY, value ? "1" : "0"); } catch { /* storage may be unavailable */ }
		}
		...
		function TaskCard(props) {
			const { id, title, badge, layout, dragging, zIndex, boardRef, actions, onDragStart, onDragCommit, onDragEnd, onResizeStart, onResizeCommit, onResizeEnd, children } = props;
			...
			const beginGesture = (mode, event) => {
				...
				if (card !== null) card.classList.add("dsh-tc-cardBusy");
			...
			const endGesture = (mode, event) => {
				...
				if (mode === "move") {
					if (typeof state.nextX === "number") onDragCommit(id, state.nextX, state.nextY);
					onDragEnd(id);
					return;
				}
				if (typeof state.nextW === "number") onResizeCommit(id, state.nextW, state.nextH);
				onResizeEnd(id);
			};
		...
		function TaskBackPanel(props) {
			...
			const [lite, setLite] = (0, react.useState)(() => readTaskLite());
			(0, react.useEffect)(() => { writeTaskLite(lite); }, [lite]);
			const stateRef = (0, react.useRef)(state);
			stateRef.current = state;
			(0, react.useEffect)(() => subscribeTaskCards(() => setState(snapshotTaskCards())), []);
			// Mark the frame as interacting while a gesture runs: the panel's ambient
			// background animation pauses and idle polling stays out of the way.
			(0, react.useEffect)(() => {
				if (typeof document === "undefined" || document.body === void 0) return;
				document.body.classList.toggle("dsh-tc-interacting", activeId !== null);
				return () => document.body.classList.remove("dsh-tc-interacting");
			}, [activeId]);
			(0, react.useEffect)(() => {
			...
			// Gestures commit once on release: the card itself moves/sizes on the
			// compositor while the pointer is down (see TaskCard.beginGesture).
			const onDragCommit = (0, react.useCallback)((id, x, y) => {
				...
				commit({ ...previous.cards, [id]: { ...card, x, y } }, previous.order);
			}, [commit]);
			const onDragEnd = (0, react.useCallback)(() => {
				setActiveId(null);
			}, []);
			const onResizeStart = (0, react.useCallback)((id) => {
				setActiveId(id);
				const order = [...stateRef.current.order.filter((key) => key !== id), id];
				setState((previous) => ({ cards: previous.cards, order }));
			}, []);
			const onResizeCommit = (0, react.useCallback)((id, w, h) => {
				...
				commit({ ...previous.cards, [id]: next }, previous.order);
			}, [commit]);
			const onResizeEnd = (0, react.useCallback)(() => {
				setActiveId(null);
			}, []);
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-tc-panel" + (lite ? " dsh-tc-lite" : ""),
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dsh-tc-btn" + (lite ? " dsh-tc-iconOn" : ""),
								title: lite ? "关闭低特效模式（恢复毛玻璃与动画）" : "低特效模式：关闭卡片毛玻璃与看板动画，提升拖动流畅度",
								onClick: () => setLite((value) => !value),
								children: lite ? "⚡ 特效已关" : "⚡ 低特效"
							}),
