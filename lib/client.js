	...
	factory: (require) => {
		...
		function parseCanvasSpec(text) {
			...
			for (const entry of feed.slice(0, 40)) {
				...
				if (op === "addCard") {
					const blocks = sanitizeTaskBlocks(entry.blocks);
			...
			return ops.length > 0 ? { ops, canvas: typeof parsed.canvas === "string" && parsed.canvas.trim().length > 0 ? parsed.canvas.trim().slice(0, 40) : void 0 } : null;
		}
		/**
		...
		function applyCanvasSpec(canvasId, spec) {
			...
			for (const op of spec.ops) {
				...
				if (op.op === "add") {
					...
					if (block === null || typeof block !== "object" || typeof block.kind !== "string") { skipped += 1; continue; }
			...
			return {
				...
				skipped,
				notes,
				message: skipped > 0 ? "已应用：" + summary + "（" + skipped + " 项无法应用，可能引用了不存在的 id 或控件字段不合法）" : "已应用：" + summary,
				summary
			};
		}
