...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Client module system port (`ctx.modules`), handed to the board so the 工作时序
		 * card can materialize another plugin's client bundle. Optional: without it the
		 * card degrades to an explicit message.
		 */
		let taskModulesPort = null;
		//#region styles
		...
		const TASK_CSS_EXTRA = [
			...
			".dsh-tc-canvasChat{position:absolute;left:0;right:0;bottom:16px;margin:0 auto;z-index:40;display:flex;flex-direction:column;width:340px;max-width:calc(100% - 32px);border-radius:14px;border:1px solid rgba(148,163,184,.32);background:rgba(255,255,255,.84);backdrop-filter:blur(14px) saturate(160%);-webkit-backdrop-filter:blur(14px) saturate(160%);box-shadow:0 18px 46px rgba(15,23,42,.26);overflow:hidden;animation:dsh-tc-chat-in .2s cubic-bezier(.22,1,.36,1) both}",
			"@keyframes dsh-tc-chat-in{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}",
			".dsh-tc-panel[data-theme=dark] .dsh-tc-canvasChat{background:rgba(24,28,44,.88);border-color:rgba(148,163,184,.24)}",
			".dsh-tc-canvasChatHead{display:flex;align-items:center;gap:6px;padding:6px 6px 6px 10px;border-bottom:1px solid rgba(148,163,184,.2);font-size:11.5px;font-weight:600;color:var(--dsw-alias-label-secondary)}",
			".dsh-tc-canvasChatBody{display:none;max-height:190px;overflow:auto;padding:8px 10px 2px;flex-direction:column;gap:6px}",
			".dsh-tc-canvasChatOpen .dsh-tc-canvasChatBody{display:flex}",
			".dsh-tc-canvasChatBar{display:flex;align-items:center;gap:6px;padding:6px 8px}",
			".dsh-tc-canvasChatInput{flex:1;min-width:0;font-size:12px;padding:5px 8px}",
			".dsh-tc-chatPill{display:inline-flex;align-items:center;gap:6px;border:0;background:transparent;color:var(--dsw-alias-label-primary);font-size:12px;font-weight:600;padding:9px 14px;cursor:pointer}",
			".dsh-tc-chatPill:hover{background:rgba(99,102,241,.14)}",
			".dsh-tc-canvasChatMini{width:auto;border-radius:999px}",
			// Floating composer: collapse control + the logo chip it collapses into.
			".dsh-tc-miniToggle{position:absolute;top:6px;right:8px;z-index:6;border:0;background:transparent;color:var(--dsw-alias-label-tertiary);cursor:pointer;font-size:13px;line-height:1;padding:3px 7px;border-radius:8px}",
			".dsh-tc-miniToggle:hover{background:rgba(99,102,241,.16);color:var(--dsw-alias-state-business-primary,#3b82f6)}",
			".dsh-tc-seatMini{left:auto!important;top:auto!important;right:18px!important;bottom:74px!important;width:52px!important;height:52px!important;min-width:0!important;min-height:0!important;max-width:none!important;border-radius:999px!important;padding:0!important;overflow:hidden!important;cursor:pointer!important;transition:all .28s cubic-bezier(.25,.8,.25,1)!important;box-shadow:0 12px 32px rgba(15,23,42,.28)!important;border:1px solid rgba(148,163,184,.35)!important}",
			"body[data-ds-dark-theme] .dsh-tc-seatMini{border-color:rgba(148,163,184,.3)!important}",
			".dsh-tc-seatMini > *{display:none!important}",
			".dsh-tc-seatMini > .dsh-tc-miniToggle{display:flex!important;position:static!important;width:100%!important;height:100%!important;align-items:center!important;justify-content:center!important;font-size:22px!important;border-radius:999px!important;padding:0!important}",
			".dsh-tc-seatMini:hover > .dsh-tc-miniToggle{background:rgba(99,102,241,.16)!important}",
			".dsh-tc-quickEdit{max-width:520px}",
			".dsh-tc-pluginHost{position:relative;flex:1;min-height:120px;min-width:0;display:flex;flex-direction:column}",
			".dsh-tc-pluginNode{flex:1;min-height:0;min-width:0;overflow:auto}",
			".dsh-tc-pluginFailed{display:flex;flex-direction:column;gap:6px;padding:10px;border-radius:10px;border:1px dashed rgba(244,63,94,.4);background:rgba(244,63,94,.06);font-size:11.5px;line-height:17px;color:var(--dsw-alias-label-secondary)}",
			".dsh-tc-pluginBadge{flex:none;font-size:10px;line-height:16px;padding:0 6px;border-radius:8px;background:rgba(148,163,184,.2);color:var(--dsw-alias-label-secondary);white-space:nowrap}",
			".dsh-tc-pluginBadgeOn{background:rgba(16,185,129,.18);color:#059669}",
			".dsh-tc-notice{position:absolute;top:52px;right:14px;z-index:2147483600;max-width:min(420px,60vw);padding:8px 12px;border-radius:12px;border:1px solid rgba(99,102,241,.45);background:rgba(238,242,255,.96);color:var(--dsw-alias-label-primary);font-size:11.5px;line-height:17px;box-shadow:0 10px 26px rgba(15,23,42,.18);cursor:pointer}",
			".dsh-tc-notice:hover{border-color:rgba(99,102,241,.8)}",
			".dsh-tc-panel[data-theme=dark] .dsh-tc-notice{background:rgba(30,41,59,.96);border-color:rgba(129,140,248,.5)}",
			// Links: curves + a dashed frame around every connected group (the group shares its
			// workspace context, so the frame marks the unit a conversation inherits).
			".dsh-tc-linkLayer{position:absolute;left:0;top:0;overflow:visible;pointer-events:none}",
			".dsh-tc-linkEdge{fill:none;stroke:rgba(99,102,241,.75);stroke-width:1.6;stroke-dasharray:none}",
			".dsh-tc-linkFrame{position:absolute;z-index:0;border:1.5px dashed rgba(99,102,241,.55);border-radius:16px;background:rgba(99,102,241,.045);pointer-events:none}",
			".dsh-tc-linkFrameLabel{position:absolute;left:10px;top:-11px;pointer-events:auto;border:1px solid rgba(99,102,241,.45);background:rgba(238,242,255,.95);color:#4338ca;border-radius:999px;padding:1px 8px;font-size:10.5px;line-height:15px;cursor:pointer;white-space:nowrap}",
			".dsh-tc-linkChip{position:absolute;transform:translate(-50%,-50%) scale(var(--dsh-tc-linkK,1));transform-origin:center;pointer-events:auto;border:1px solid rgba(99,102,241,.5);background:rgba(238,242,255,.96);color:#4338ca;border-radius:999px;padding:1px 9px;font-size:10.5px;line-height:16px;cursor:pointer;white-space:nowrap}",
			".dsh-tc-linkChip:hover{border-color:rgba(244,63,94,.7);color:#be123c;background:rgba(255,241,242,.98)}",
			".dsh-tc-panel[data-theme=dark] .dsh-tc-linkFrame{border-color:rgba(129,140,248,.5);background:rgba(129,140,248,.08)}",
			".dsh-tc-panel[data-theme=dark] .dsh-tc-linkFrameLabel,.dsh-tc-panel[data-theme=dark] .dsh-tc-linkChip{background:rgba(30,41,59,.95);color:#c7d2fe;border-color:rgba(129,140,248,.5)}",
			".dsh-tc-linkMode{position:absolute;inset:0;z-index:2147483400;cursor:crosshair;background:rgba(99,102,241,.05)}",
			".dsh-tc-linkModeHint{position:absolute;left:50%;top:12px;transform:translateX(-50%);pointer-events:none;padding:5px 12px;border-radius:999px;border:1px solid rgba(99,102,241,.45);background:rgba(238,242,255,.96);color:#4338ca;font-size:11.5px;line-height:17px}",
			".dsh-tc-linkModeBand{position:absolute;left:0;top:0;width:100%;height:100%;overflow:visible;pointer-events:none}",
			".dsh-tc-linkModeBand .dsh-tc-linkEdge{stroke-dasharray:6 5}",
			// The 3D model viewer item: a toolbar over a sandboxed iframe that fills the item.
			".dsh-tc-model{display:flex;flex-direction:column;gap:6px;min-width:0;flex:1;min-height:0}",
			".dsh-tc-modelBar{display:flex;align-items:center;gap:6px;flex-wrap:wrap;flex:none}",
			".dsh-tc-modelName{font-size:11.5px;font-weight:600;max-width:34%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
			".dsh-tc-modelInfo{flex:1;min-width:40px;font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
			".dsh-tc-model[data-state=failed] .dsh-tc-modelInfo{color:#be123c}",
			".dsh-tc-modelBtn{font:inherit;font-size:11px;line-height:16px;padding:1px 7px;border-radius:8px;border:1px solid rgba(148,163,184,.45);background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer;white-space:nowrap}",
			".dsh-tc-modelBtn:hover{border-color:rgba(99,102,241,.7);color:#4338ca}",
			".dsh-tc-modelStage{position:relative;flex:1;min-height:160px;border-radius:10px;overflow:hidden;border:1px solid rgba(148,163,184,.3);background:#0b1120}",
			".dsh-tc-modelFrame{position:absolute;inset:0;width:100%;height:100%;border:0;display:block}",
			".dsh-tc-modelWait{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:10px;text-align:center;font-size:11.5px;color:var(--dsw-alias-label-tertiary)}",
			".dsh-tc-modelFile{display:none}",
			".dsh-tc-bridgePanel{max-width:560px}",
			...
			".dsh-tc-img{display:block;width:100%;height:auto;border-radius:10px;background:rgba(148,163,184,.12)}",
			".dsh-tc-imgCap{font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary)}",
			".dsh-tc-imgLoading{font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary)}",
			// A source that cannot be fetched used to render as an invisible broken image; the
			// item now says what failed and offers the two ways out (retry / local paste).
			".dsh-tc-imgFailed{margin:0;display:flex;flex-direction:column;gap:6px;min-height:132px;justify-content:center;padding:10px;border-radius:10px;border:1px dashed rgba(244,63,94,.45);background:rgba(244,63,94,.06);min-width:0}",
			".dsh-tc-imgFailTitle{font-size:12px;font-weight:600;color:var(--dsw-alias-label-primary)}",
			".dsh-tc-imgFailHint,.dsh-tc-imgFailUrl{font-size:11px;line-height:16px;color:var(--dsw-alias-label-secondary);word-break:break-all}",
			".dsh-tc-imgFailUrl{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;opacity:.85}",
			".dsh-tc-imgFailRow{display:flex;align-items:center;gap:8px;flex-wrap:wrap}",
			".dsh-tc-imgFailRow a.dsh-tc-btn{text-decoration:none}",
			".dsh-tc-imgFailFile{display:none}",
			...
			".dsh-tc-appBadge{flex:none;font-size:10px;line-height:16px;padding:0 6px;border-radius:8px;background:rgba(148,163,184,.2);color:var(--dsw-alias-label-secondary);white-space:nowrap}",
			".dsh-tc-appBadgeOn{background:rgba(16,185,129,.18);color:#059669}",
			".dsh-tc-panel[data-theme=dark] .dsh-tc-appBadgeOn{background:rgba(16,185,129,.22);color:#34d399}",
			...
			".dsh-tc-cardBody[data-fill]{padding:8px;gap:0;max-height:none;flex:1;min-height:0;display:flex;flex-direction:column}",
			".dsh-tc-fullBody[data-fill]:has(> .dsh-tc-tptHost){padding:8px;gap:0}",
			".dsh-tc-tptHost{flex:1;min-height:0;display:flex;flex-direction:column;overflow:hidden;border-radius:10px}",
			".dsh-tc-tptHost>div{flex:1;min-height:0;display:flex;flex-direction:column}",
		...
		const TASK_CARD_DEFAULTS = {
			...
			// 工作时序 ships wider than its siblings: the chronicle panel is a two-column view.
			"chronicle": { x: 740, y: 24, collapsed: false, hidden: false, w: 384, h: 330 }
		};
		...
		const TASK_BLOCK_KINDS = ["text", "heading", "note", "stats", "progress", "trend", "kv", "links", "chips", "checklist", "counter", "button", "table", "code", "toggle", "countdown", "bars", "embed", "model"];
		...
		/** Page-bridge defaults (the local proxy that lets the agent operate an embedded app). */
		const TASK_BRIDGE_STORAGE_KEY = "dsh.taskconsole.bridge.v1";
		const TASK_BRIDGE_DEFAULT_PORT = 8790;
		const TASK_BRIDGE_COMMAND_TIMEOUT = 20000;
		const TASK_BRIDGE_MAX_ACTIONS = 20;
		/** Actions the injected page agent understands. */
		const TASK_BRIDGE_ACTIONS = ["read", "query", "click", "type", "press", "select", "check", "scroll", "wait", "eval", "back", "forward", "reload"];
		const TASK_BRIDGE_ACTION_LABELS = {
			read: "读取内容",
			query: "查询元素",
			click: "点击",
			type: "输入",
		...
		/** Bridge settings live per browser (they point at a local server). */
		function freshTaskBridgeSettings() {
			return { url: "http://127.0.0.1:" + TASK_BRIDGE_DEFAULT_PORT, token: "", enabled: false };
		}
		function sanitizeTaskBridgeSettings(raw) {
			const base = freshTaskBridgeSettings();
			if (raw === null || typeof raw !== "object") return base;
			const url = typeof raw.url === "string" && /^https?:\/\//i.test(raw.url.trim()) ? raw.url.trim().replace(/\/+$/, "").slice(0, 300) : base.url;
			const token = typeof raw.token === "string" ? raw.token.trim().slice(0, 200) : "";
			return { url, token, enabled: raw.enabled === true };
		}
		function readTaskBridgeSettings() {
			if (typeof localStorage === "undefined") return freshTaskBridgeSettings();
			try {
				const raw = localStorage.getItem(TASK_BRIDGE_STORAGE_KEY);
				if (raw === null) return freshTaskBridgeSettings();
				return sanitizeTaskBridgeSettings(JSON.parse(raw));
			} catch {
				return freshTaskBridgeSettings();
			}
		}
		let taskBridgeSettings = readTaskBridgeSettings();
		const taskBridgeListeners = new Set();
		function subscribeTaskBridge(listener) {
			taskBridgeListeners.add(listener);
			return () => { taskBridgeListeners.delete(listener); };
		}
		function notifyTaskBridge() {
			taskBridgeListeners.forEach((listener) => {
				try { listener(); } catch { /* a listener must never break the UI */ }
			});
		}
		function saveTaskBridgeSettings(settings) {
			taskBridgeSettings = sanitizeTaskBridgeSettings(settings);
			if (typeof localStorage !== "undefined") {
				try { localStorage.setItem(TASK_BRIDGE_STORAGE_KEY, JSON.stringify(taskBridgeSettings)); } catch { /* storage blocked */ }
			}
			notifyTaskBridge();
			return taskBridgeSettings;
		}
		/** Stable target key for one app URL — the workbench and the page must agree on it. */
		function taskBridgeTarget(url) {
			...
			return "t" + (hash >>> 0).toString(16).padStart(8, "0") + text.length.toString(16).slice(-4);
		}
		/** URL the iframe loads for a controlled app: the bridge proxy, which injects the agent. */
		function taskBridgePageUrl(url, settings) {
			const config = settings ?? taskBridgeSettings;
			const target = taskBridgeTarget(url);
			const query = "?url=" + encodeURIComponent(url) + "&target=" + encodeURIComponent(target) + (config.token.length > 0 ? "&token=" + encodeURIComponent(config.token) : "");
			return { src: config.url + "/p" + query, target };
		}
		/** Live bridge connections: target → { state, url, title, error, since }. */
		const taskBridgePages = new Map();
		function taskBridgePageState(target) {
			return taskBridgePages.get(target) ?? { state: "unknown", url: "", title: "", error: "", since: 0 };
		}
		function setTaskBridgePageState(target, patch) {
			taskBridgePages.set(target, { ...taskBridgePageState(target), ...patch, since: Date.now() });
			notifyTaskBridge();
		}
		/** Turn one `[page]` action object into a validated command (null when unusable). */
		function sanitizeTaskBridgeAction(raw) {
			if (raw === null || typeof raw !== "object" || Array.isArray(raw)) return null;
			const action = typeof raw.action === "string" ? raw.action.trim() : "";
			if (!TASK_BRIDGE_ACTIONS.includes(action)) return null;
			const out = { action };
			if (typeof raw.selector === "string" && raw.selector.trim().length > 0) out.selector = raw.selector.trim().slice(0, 300);
			if (typeof raw.text === "string" && raw.text.trim().length > 0) out.text = raw.text.trim().slice(0, 200);
			if (typeof raw.value === "string") out.value = raw.value.slice(0, 2000);
			if (typeof raw.label === "string") out.label = raw.label.slice(0, 200);
			if (typeof raw.key === "string") out.key = raw.key.slice(0, 20);
			if (typeof raw.code === "string") out.code = raw.code.slice(0, 4000);
			if (typeof raw.url === "string") out.url = raw.url.trim().slice(0, 1000);
			if (typeof raw.reason === "string") out.reason = raw.reason.trim().slice(0, 200);
			if (typeof raw.y === "number" && Number.isFinite(raw.y)) out.y = Math.round(raw.y);
			if (typeof raw.ms === "number" && Number.isFinite(raw.ms)) out.ms = Math.min(Math.max(0, Math.round(raw.ms)), 15000);
			if (typeof raw.timeout === "number" && Number.isFinite(raw.timeout)) out.timeout = Math.min(Math.max(0, Math.round(raw.timeout)), 15000);
			if (raw.submit === true) out.submit = true;
			if (raw.checked === true || raw.checked === false) out.checked = raw.checked;
			if (raw.bottom === true) out.bottom = true;
			// Interactions need something to act on.
			const needsTarget = action === "click" || action === "type" || action === "press" || action === "select" || action === "check" || action === "query";
			if (needsTarget && out.selector === void 0 && out.text === void 0) return null;
			if (action === "type" && out.selector === void 0) return null;
			if ((action === "select" || action === "check" || action === "query") && out.selector === void 0) return null;
			return out;
		}
		/** Parse the first `[page]…[/page]` block into a validated action list. */
		function parsePageSpec(text) {
			...
			if (start !== -1) {
				...
				const end = text.indexOf("[/page]", after);
			...
			if (raw.startsWith("```")) raw = raw.replace(/^```[a-zA-Z0-9_-]*\s*/, "").replace(/```$/, "").trim();
			if (raw.length === 0 || raw.length > 12000) return null;
			let parsed = null;
			try { parsed = JSON.parse(raw); } catch { return null; }
			if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) return null;
			const feed = Array.isArray(parsed.actions) ? parsed.actions : Array.isArray(parsed) ? parsed : null;
			if (feed === null) return null;
			const actions = [];
			for (const entry of feed.slice(0, TASK_BRIDGE_MAX_ACTIONS)) {
				const action = sanitizeTaskBridgeAction(entry);
				if (action !== null) actions.push(action);
			}
			if (actions.length === 0) return null;
			return {
				actions,
				url: typeof parsed.url === "string" ? parsed.url.trim().slice(0, 1000) : "",
				then: typeof parsed.then === "string" ? parsed.then.trim().slice(0, 200) : "",
				summary: typeof parsed.summary === "string" ? parsed.summary.trim().slice(0, 300) : ""
			};
		}
		/** Human line for one executed action. */
		function describeBridgeAction(action, result) {
			const label = TASK_BRIDGE_ACTION_LABELS[action.action] ?? action.action;
			const where = action.selector !== void 0 ? " " + action.selector : action.text !== void 0 ? "「" + action.text + "」" : "";
			if (result === null || result === void 0 || result.ok !== true) {
				const error = result !== null && result !== void 0 && result.error ? result.error : "失败";
				return label + where + " ✗ " + (error === "no-page" ? "网页未连上桥接（受控模式未开启或页面未加载完）" : error === "timeout" ? "超时" : error);
			}
			if (action.action === "read") {
				const value = result.value ?? {};
				return label + " ✓ " + (value.title ? "「" + value.title + "」" : "") + (typeof value.text === "string" ? value.text.replace(/\s+/g, " ").slice(0, 400) : "");
			}
			if (action.action === "query") {
				const value = result.value ?? {};
				return label + where + " ✓ " + String(value.count ?? 0) + " 个";
			}
			if (action.action === "click") {
				const value = result.value ?? {};
				const clicked = value.clicked;
				return label + where + " ✓ " + (clicked !== null && clicked !== void 0 && typeof clicked.text === "string" ? "「" + clicked.text.slice(0, 40) + "」" : "");
			}
			if (action.action === "eval") return label + " ✓ " + JSON.stringify(result.value ?? null).slice(0, 300);
			return label + where + " ✓";
		}
		/**
		 * Run one action list against a bridged page. Uses the bridge's `host` socket role:
		 * the injected page agent executes each command inside the page and reports back, so
		 * the plugin never needs script access to a cross-origin frame.
		 * @returns `[{ action, result }]` in order (never throws).
		 */
		async function runTaskBridgeActions(target, actions, options) {
			const config = (options ?? {}).settings ?? taskBridgeSettings;
			const timeout = (options ?? {}).timeout ?? TASK_BRIDGE_COMMAND_TIMEOUT;
			if (!Array.isArray(actions) || actions.length === 0) return [];
			if (typeof WebSocket !== "function") return actions.map((action) => ({ action, result: { ok: false, error: "no-websocket", value: null } }));
			const wsBase = config.url.replace(/^http/i, "ws");
			const query = "role=host&target=" + encodeURIComponent(target) + (config.token.length > 0 ? "&token=" + encodeURIComponent(config.token) : "");
			const socket = new WebSocket(wsBase + "/ws?" + query);
			try {
				await new Promise((resolve, reject) => {
					const timer = setTimeout(() => reject(new Error("bridge-connect-timeout")), 6000);
					socket.addEventListener("open", () => { clearTimeout(timer); resolve(); });
					socket.addEventListener("error", () => { clearTimeout(timer); reject(new Error("bridge-unreachable")); });
				});
			} catch (error) {
				const detail = String(error !== null && typeof error === "object" && error.message ? error.message : error);
				setTaskBridgePageState(target, { state: "offline", error: detail });
				try { socket.close(); } catch { /* already closing */ }
				return actions.map((action) => ({ action, result: { ok: false, error: "bridge-unreachable", detail, value: null } }));
			}
			...
			socket.addEventListener("message", (event) => {
				...
				if (message.type === "page") {
					setTaskBridgePageState(target, { state: message.state === "connected" ? "connected" : "disconnected", url: typeof message.url === "string" ? message.url : "", title: typeof message.title === "string" ? message.title : "" });
					return;
				}
				...
				if (settle !== void 0) {
					...
					settle({ ok: message.ok === true, value: message.value ?? null, error: message.error ?? null });
				}
			});
			const results = [];
			try {
				for (let index = 0; index < actions.length; index++) {
					...
					const id = "a" + index + "-" + Date.now().toString(36);
					const result = await new Promise((resolve) => {
						const timer = setTimeout(() => {
							pending.delete(id);
							resolve({ ok: false, error: "timeout", value: null });
						}, timeout);
						pending.set(id, (value) => { clearTimeout(timer); resolve(value); });
						try {
							socket.send(JSON.stringify({ type: "command", id, ...action }));
						} catch {
							clearTimeout(timer);
							pending.delete(id);
							resolve({ ok: false, error: "send-failed", value: null });
						}
					});
					results.push({ action, result });
					if (result.ok !== true && (result.error === "no-page" || result.error === "timeout")) setTaskBridgePageState(target, { state: result.error, error: result.error });
					else if (result.ok === true) setTaskBridgePageState(target, { state: "connected", error: "" });
				}
			} finally {
				try { socket.close(); } catch { /* already closing */ }
			}
		...
		/** One status socket per target, shared by every subscriber (the embed badge, panels). */
		const taskBridgeMonitors = new Map();
		function ensureTaskBridgeMonitor(target) {
			if (typeof WebSocket !== "function" || target.length === 0) return;
			const existing = taskBridgeMonitors.get(target);
			if (existing !== undefined) { existing.refs += 1; return; }
			const monitor = { refs: 1, socket: null, retry: 0, closed: false, timer: null };
			taskBridgeMonitors.set(target, monitor);
			const connect = () => {
				if (monitor.closed) return;
				const config = taskBridgeSettings;
				const wsBase = config.url.replace(/^http/i, "ws");
				const query = "role=host&target=" + encodeURIComponent(target) + (config.token.length > 0 ? "&token=" + encodeURIComponent(config.token) : "");
				let socket = null;
				try { socket = new WebSocket(wsBase + "/ws?" + query); } catch { return; }
				monitor.socket = socket;
				socket.addEventListener("open", () => {
					monitor.retry = 0;
					try { socket.send(JSON.stringify({ type: "status" })); } catch { /* closing */ }
				});
				socket.addEventListener("message", (event) => {
					let message = null;
					try { message = JSON.parse(String(event.data)); } catch { return; }
					if (message === null || typeof message !== "object" || message.type !== "page") return;
					// A notification that carries no url/title (a frame joined or left the target)
					// must not erase what the page already reported.
					const patch = { state: message.state === "connected" ? "connected" : "disconnected" };
					if (typeof message.url === "string") patch.url = message.url;
					if (typeof message.title === "string") patch.title = message.title;
					setTaskBridgePageState(target, patch);
				});
				socket.addEventListener("close", () => {
					if (monitor.closed) return;
					setTaskBridgePageState(target, { state: "offline" });
					monitor.retry = Math.min(monitor.retry + 1, 6);
					monitor.timer = setTimeout(connect, 1200 * monitor.retry);
				});
				socket.addEventListener("error", () => { /* the close handler retries */ });
			};
			connect();
		}
		function releaseTaskBridgeMonitor(target) {
			const monitor = taskBridgeMonitors.get(target);
			if (monitor === undefined) return;
			monitor.refs -= 1;
			if (monitor.refs > 0) return;
			monitor.closed = true;
			if (monitor.timer !== null) clearTimeout(monitor.timer);
			try { monitor.socket?.close(); } catch { /* already closing */ }
			taskBridgeMonitors.delete(target);
		}
		/** Subscribe to one page's live state; keeps a status socket open while subscribed. */
		function subscribeTaskBridgePage(target, listener) {
			if (typeof target === "string" && target.length > 0) ensureTaskBridgeMonitor(target);
			const unsubscribe = subscribeTaskBridge(listener);
			return () => {
				unsubscribe();
				if (typeof target === "string" && target.length > 0) releaseTaskBridgeMonitor(target);
			};
		}
		/** One-line report of an executed action list, for the chat log. */
		function summarizeBridgeResults(results) {
			if (!Array.isArray(results) || results.length === 0) return "";
			return results.map((entry) => describeBridgeAction(entry.action, entry.result)).join("；");
		}
		/** The controlled embed a page action list should drive: explicit URL match, else the first one. */
		function pickBridgeTarget(blocks, wantedUrl) {
			const controls = (Array.isArray(blocks) ? blocks : []).filter((block) => block !== null && typeof block === "object" && block.kind === "embed" && block.control === true && typeof block.url === "string");
			if (controls.length === 0) return null;
			if (typeof wantedUrl === "string" && wantedUrl.length > 0) {
				const exact = controls.find((block) => block.url === wantedUrl);
				const loose = exact ?? controls.find((block) => wantedUrl.startsWith(block.url) || block.url.startsWith(wantedUrl));
				if (loose !== undefined) return { url: loose.url, target: taskBridgeTarget(loose.url) };
				return null;
			}
			return { url: controls[0].url, target: taskBridgeTarget(controls[0].url) };
		}
		/**
		 * Execute the `[page]` block of one agent reply against the controlled embed of the
		 * card / canvas it belongs to. Returns null when the reply had no page actions.
		 */
		async function runPageBlockFromReply(text, blocks) {
			const spec = parsePageSpec(text);
			if (spec === null) return null;
			// The 「允许 agent 操作网页」switch is the safety gate for everything below.
			if (taskBridgeSettings.enabled !== true) {
				return { ok: false, summary: "网页操作已关闭：在「🌉 桥接」面板里打开「允许 agent 操作网页」后重试", results: [], spec };
			}
			const picked = pickBridgeTarget(blocks, spec.url);
			if (picked === null) {
				return { ok: false, summary: "回复里有 [page] 动作，但没有匹配的受控网页（在应用上勾选「受控」并启动桥接即可）", results: [], spec };
			}
			const results = await runTaskBridgeActions(picked.target, spec.actions);
			return {
				ok: results.every((entry) => entry.result.ok === true),
				okCount: results.filter((entry) => entry.result.ok === true).length,
				total: results.length,
				summary: summarizeBridgeResults(results),
				results,
				spec,
				url: picked.url
			};
		}
		/** One entry per controlled embed found in a block list (cards and canvas items). */
		function collectBridgeTargets(blockLists) {
			const out = [];
			for (const list of Array.isArray(blockLists) ? blockLists : []) {
				for (const block of Array.isArray(list) ? list : []) {
					if (block === null || typeof block !== "object" || block.kind !== "embed" || block.control !== true || typeof block.url !== "string") continue;
					if (out.some((entry) => entry.url === block.url)) continue;
					out.push({ url: block.url, target: taskBridgeTarget(block.url), title: typeof block.title === "string" ? block.title : "" });
				}
			}
			return out;
		}
		/** Ping the bridge (healthz) so the settings panel can show a status. */
		async function pingTaskBridge(settings) {
			const config = settings ?? taskBridgeSettings;
			try {
				...
				return { ok: true, message: "桥接在线 · 已连接页面 " + pages + " 个" + (config.token.length > 0 ? " · 已带令牌" : " · 未设令牌") };
			} catch (error) {
				return { ok: false, message: "无法连接桥接：" + String(error !== null && error.message ? error.message : error) };
			}
		}
		/**
		...
		//#region plugin hosting (any DSH plugin shown as a canvas/board item)
		/** Plugin id without its scope and `dsh-plugin-` prefix: `dsh-plugin-tpt-chronicle` → `tpt-chronicle`. */
		function pluginBareName(id) {
			return String(id).replace(/^@[^/]+\//, "").replace(/^dsh-plugin-/, "");
		}
		/** Plugin id → letters/digits/underscore, the convention for its embed global. */
		function pluginGlobalName(id) {
			return "__DSH_" + pluginBareName(id).toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "") + "__";
		}
		/**
		 * Every global a plugin's embed entry may be published under. The convention is
		 * `__DSH_<ID WITHOUT dsh-plugin->__` (what dsh-plugin-tpt-chronicle publishes as
		 * `__DSH_TPT_CHRONICLE__`); the whole-id forms are tried as well so a plugin that
		 * kept its prefix still hosts, and an item may name its entry explicitly.
		 */
		function pluginEntryNames(id, entry) {
			const raw = String(id);
			const bare = pluginBareName(raw);
			const names = [];
			if (typeof entry === "string" && entry.trim().length > 0) names.push(entry.trim());
			names.push(pluginGlobalName(raw));
			names.push("__DSH_" + bare.replace(/[^A-Za-z0-9]+/g, "_") + "__");
			names.push("__DSH_" + raw.toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "") + "__");
			names.push("__DSH_" + raw.replace(/[^A-Za-z0-9]+/g, "_") + "__");
			return [...new Set(names)];
		}
		/** One plugin reference carried by a canvas/board item. */
		function sanitizePluginRef(raw) {
			if (raw === null || typeof raw !== "object" || Array.isArray(raw)) return null;
			const id = typeof raw.id === "string" ? raw.id.trim().slice(0, 120) : "";
			if (id.length === 0 || !/^[@A-Za-z0-9._:/-]{1,120}$/.test(id)) return null;
			const out = { id };
			if (typeof raw.title === "string" && raw.title.trim().length > 0) out.title = raw.title.trim().slice(0, 80);
			if (typeof raw.entry === "string" && raw.entry.trim().length > 0) out.entry = raw.entry.trim().slice(0, 80);
			if (typeof raw.height === "number" && Number.isFinite(raw.height)) out.height = Math.min(Math.max(Math.round(raw.height), 120), 1600);
			if (typeof raw.rev === "number" && Number.isFinite(raw.rev)) out.rev = Math.round(raw.rev);
			if (raw.bare === true) out.bare = true;
			return out;
		}
		/** Runtime status of one hosted plugin: id → { state, message, at }. */
		const taskPluginStatus = new Map();
		const taskPluginListeners = new Set();
		function subscribeTaskPlugin(listener) {
			taskPluginListeners.add(listener);
			return () => { taskPluginListeners.delete(listener); };
		}
		function publishTaskPlugin(id, patch) {
			taskPluginStatus.set(id, { ...(taskPluginStatus.get(id) ?? { state: "idle", message: "", at: 0 }), ...patch, at: Date.now() });
			for (const listener of [...taskPluginListeners]) {
				try { listener(id); } catch { /* a listener must never break the UI */ }
			}
		}
		function taskPluginState(id) {
			return taskPluginStatus.get(id) ?? { state: "idle", message: "", at: 0 };
		}
		/** Every client bundle the boot manifest knows, plus the dynamic plugins the runner reports. */
		function listPluginCandidates() {
			const rows = taskModulesPort?.manifest?.entries ?? globalThis.__DSH_BOOT__?.entries ?? [];
			const seen = new Map();
			for (const row of Array.isArray(rows) ? rows : []) {
				if (row === null || typeof row !== "object" || typeof row.id !== "string" || row.id.length === 0) continue;
				if (typeof row.url !== "string" || row.url.length === 0) continue;
				seen.set(row.id, { id: row.id, url: row.url, source: "bundle", embeddable: taskPluginStatus.get(row.id)?.state === "mounted" });
			}
			return [...seen.values()];
		}
		/** Dynamic (runtime) plugins from the plugin-management port: they have no bundle URL of their own. */
		function listDynamicPlugins() {
			const port = taskPluginsPort;
			if (port === null || port === void 0) return Promise.resolve([]);
			return Promise.resolve(port.read()).then((rows) => (Array.isArray(rows) ? rows.map((row) => ({
				id: typeof row.pluginId === "string" ? row.pluginId : "",
				name: Array.isArray(row.packages) && row.packages.length > 0 && typeof row.packages[row.packages.length - 1]?.name === "string" ? row.packages[row.packages.length - 1].name : "",
				running: row.activeRun !== void 0 && row.activeRun !== null
			})).filter((row) => row.id.length > 0) : []), () => []);
		}
		/** Client bundle URL of one plugin id, from the boot manifest / module system. */
		function pluginBundleUrl(id) {
			const rows = taskModulesPort?.manifest?.entries ?? globalThis.__DSH_BOOT__?.entries ?? [];
			for (const row of Array.isArray(rows) ? rows : []) {
				if (row !== null && typeof row === "object" && row.id === id && typeof row.url === "string") return row.url;
			}
			return "";
		}
		/** Load one plugin's client bundle as a script; `revision` cache-busts it for a rebuild. */
		function loadPluginBundle(id, revision) {
			return new Promise((resolve, reject) => {
				const url = pluginBundleUrl(id);
				if (url.length === 0) {
					reject(new Error("启动清单里没有这个插件的客户端包：" + id));
					return;
				}
				const selector = 'script[data-dsh-task-plugin="' + id.replace(/"/g, "") + '"]';
				if (revision === void 0) {
					const existing = document.querySelector(selector);
					if (existing !== null) { resolve(url); return; }
				}
				const previous = document.querySelector(selector);
				if (previous !== null) previous.remove();
				const script = document.createElement("script");
				script.src = revision === void 0 ? url : url + (url.includes("?") ? "&" : "?") + "rebuild=" + revision;
				script.async = true;
				script.dataset.dshTaskPlugin = id;
				script.onload = () => resolve(script.src);
				script.onerror = () => reject(new Error("客户端包加载失败：" + url));
				document.head.appendChild(script);
			});
		}
		/**
		 * Find the plugin's embed entry point. A hosting-capable plugin publishes a global
		 * either under the `__DSH_<ID>__` convention or under the name the item names, and
		 * exposes `mountEmbedded(host, api)` (the DSH embedding contract) or `mount(host, api)`.
		 */
		function findPluginEntry(id, entry) {
			for (const name of pluginEntryNames(id, entry)) {
				const value = globalThis[name];
				if (value === void 0 || value === null) continue;
				if (typeof value.mountEmbedded === "function") return { name, mount: value.mountEmbedded, api: value };
				if (typeof value.mount === "function") return { name, mount: value.mount, api: value };
				if (typeof value === "function") return { name, mount: value, api: null };
			}
			return null;
		}
		/**
		 * Tear down one plugin root mounted into a host node a component owns.
		 *
		 * The host's children are **never** cleared by hand: `root.unmount()` may defer its
		 * DOM deletion (React defers a `flushSync` issued from inside a commit, which is
		 * exactly where these cleanups run), and clearing the host first makes React remove a
		 * node that is already gone — `removeChild … is not a child of this node`, thrown out
		 * of the next flush and taking the caller with it. The host element is React-owned and
		 * dies with its component (or is emptied by the next `createRoot`), so unmounting is
		 * the whole teardown; `flushSync` is used when available so the container is unmarked
		 * before a rebuild mounts into the same node again.
		 */
		function unmountPluginRoot(root) {
			if (root === null || root === void 0) return;
			try {
				if (typeof react_dom.flushSync === "function") react_dom.flushSync(() => root.unmount());
				else root.unmount();
			} catch { /* the host is already gone: nothing left to unmount */ }
		}
		/**
		 * Host one plugin's UI in a node this component owns: the bundle is materialized, the
		 * entry point mounts into the host with a small API (its own React renderer, theme and
		 * status callback), and `rebuild` reloads the bundle with a fresh revision and remounts.
		 */
		function PluginHostView(props) {
			const { plugin, dark } = props;
			const hostRef = (0, react.useRef)(null);
			const [status, setStatus] = (0, react.useState)(() => taskPluginState(plugin.id));
			(0, react.useEffect)(() => subscribeTaskPlugin((id) => {
				if (id === plugin.id) setStatus({ ...taskPluginState(plugin.id) });
			}), [plugin.id]);
			const revision = plugin.rev;
			(0, react.useEffect)(() => {
				const host = hostRef.current;
				if (host === null || typeof document === "undefined") return void 0;
				let disposed = false;
				let root = null;
				publishTaskPlugin(plugin.id, { state: "loading", message: "正在加载 " + plugin.id });
				loadPluginBundle(plugin.id, typeof revision === "number" ? revision : void 0).then(() => {
					if (disposed) return;
					const found = findPluginEntry(plugin.id, plugin.entry);
					if (found === null) {
						throw new Error("该插件没有发布可嵌入入口（期望 " + pluginGlobalName(plugin.id) + ".mountEmbedded）；它仍然可以在自己的位置使用");
					}
					root = react_dom.createRoot(host);
					// Published before the mount so the plugin's own `onStatus` refines it
					// instead of being overwritten by the generic label.
					publishTaskPlugin(plugin.id, { state: "mounted", message: "已挂载" });
					const mount = () => found.mount.call(found.api, host, {
						render: (element) => root.render(element),
						dark: dark === true,
						status: { id: plugin.id },
						onStatus: (next) => publishTaskPlugin(plugin.id, typeof next === "string" ? { state: "mounted", message: next } : { state: "mounted", message: next !== null && typeof next === "object" && typeof next.message === "string" ? next.message : "" })
					});
					mount();
				}).catch((error) => {
					if (!disposed) publishTaskPlugin(plugin.id, { state: "failed", message: error instanceof Error ? error.message : String(error) });
				});
				return () => {
					disposed = true;
					const target = root;
					root = null;
					// The host node may already be gone (the board/canvas around it was torn
					// down first); unmounting must never take the whole workbench with it.
					unmountPluginRoot(target);
				};
			}, [plugin.id, plugin.entry, revision, dark]);
			if (status.state === "failed") {
				return (0, react_jsx_runtime.jsxs)("div", {
					className: "dsh-tc-pluginHost dsh-tc-pluginFailed",
					children: [
						(0, react_jsx_runtime.jsx)("div", { children: "插件无法显示：" + status.message }),
						(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-fieldHint", children: "装好的插件可以直接用它的原生界面；要放进画布/卡片，插件需要暴露 " + pluginGlobalName(plugin.id) + ".mountEmbedded(host, api)。" }),
						(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-fieldHint", children: "例：globalThis." + pluginGlobalName(plugin.id) + " = { mountEmbedded(host, api) { api.render(React.createElement(App)) } }" })
					]
				});
			}
			return (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-tc-pluginHost",
				"data-state": status.state,
				children: (0, react_jsx_runtime.jsx)("div", { ref: hostRef, className: "dsh-tc-pluginNode" })
			});
		}
		/**
		 * A rebuild revision that always moves forward: `Date.now()` alone can repeat inside
		 * one millisecond, and an unchanged revision would leave the mounted UI in place.
		 */
		function nextPluginRevision(plugin) {
			const previous = plugin !== null && typeof plugin === "object" && typeof plugin.rev === "number" ? plugin.rev : 0;
			return Math.max(Date.now(), previous + 1);
		}
		/** Rebuild one hosted plugin: a fresh bundle revision forces reload + remount. */
		function rebuildHostedPlugin(canvasId, card) {
			if (card === null || typeof card !== "object" || card.plugin === void 0) return false;
			const next = { ...card, plugin: { ...card.plugin, rev: nextPluginRevision(card.plugin) }, updatedAt: Date.now() };
			if (canvasId !== null && canvasId !== void 0) canvasUpsertCard(canvasId, next, true);
			return true;
		}
		/**
		 * Live status chip plus a rebuild button for one plugin item — shared by the canvas
		 * card head and the bare-control toolbar, so a plugin that failed to mount can be
		 * retried without opening a conversation.
		 */
		function CanvasPluginTools(props) {
			const { plugin, onRebuild, buttonClass } = props;
			const [status, setStatus] = (0, react.useState)(() => taskPluginState(plugin.id));
			(0, react.useEffect)(() => subscribeTaskPlugin((id) => {
				if (id === plugin.id) setStatus({ ...taskPluginState(plugin.id) });
			}), [plugin.id]);
			const label = status.state === "mounted" ? "已挂载" : status.state === "loading" ? "加载中…" : status.state === "failed" ? "未挂载" : "待挂载";
			return [
				(0, react_jsx_runtime.jsx)("span", {
					className: "dsh-tc-pluginBadge" + (status.state === "mounted" ? " dsh-tc-pluginBadgeOn" : ""),
					title: status.message !== void 0 && status.message.length > 0 ? status.message : plugin.id,
					children: label
				}, "badge"),
				(0, react_jsx_runtime.jsx)("button", {
					type: "button",
					className: buttonClass,
					title: "重新构建这个插件项（重新加载它的客户端包并重新挂载）",
					onPointerDown: (event) => event.stopPropagation(),
					onClick: () => onRebuild(plugin.id),
		...
		const TASK_CANVAS_ADD_KINDS = ["text", "heading", "note", "stats", "progress", "trend", "kv", "links", "chips", "checklist", "counter", "button", "table", "code", "toggle", "countdown", "bars", "embed", "model"];
		...
		function taskBlockDefault(kind) {
			...
			case "model": return { kind, src: "", name: "", height: 360, wireframe: false, grid: true, background: "dark" };
		...
		/**
		 * Names a conversation may use for a control kind: the canonical id, the Chinese label
		 * and the synonyms models actually write. Several replies in the wild named the kind in
		 * Chinese ("表格", "计数") or wrote the kind **as the key** instead of the `kind` field
		 * (`{ "stats": [...] }`), which used to land on the canvas as an unsupported placeholder.
		 */
		const TASK_BLOCK_ALIASES = {
			text: ["text", "txt", "paragraph", "plain", "正文", "文本", "文字"],
			heading: ["heading", "head", "title", "h1", "h2", "标题", "小标题"],
			note: ["note", "comment", "description", "desc", "remark", "说明", "备注", "注释"],
			stats: ["stats", "stat", "metrics", "kpi", "指标", "统计", "数据卡片"],
			progress: ["progress", "percent", "percentage", "completion", "进度", "完成度", "进度条"],
			trend: ["trend", "line", "linechart", "sparkline", "series", "走势", "趋势", "折线", "折线图"],
			kv: ["kv", "keyvalue", "key-value", "keyvalues", "pairs", "properties", "键值", "属性", "参数"],
			links: ["links", "link", "urls", "bookmarks", "链接", "网址"],
			chips: ["chips", "chip", "tags", "tag", "badges", "标签", "徽标"],
			checklist: ["checklist", "check", "todo", "todos", "tasks", "checkboxes", "清单", "待办", "检查项", "任务列表"],
			counter: ["counter", "count", "number", "stepper", "计数", "计数器", "数值"],
			button: ["button", "btn", "action", "按钮", "操作"],
			table: ["table", "grid", "datatable", "表格", "表"],
			code: ["code", "snippet", "pre", "source", "代码", "代码块"],
			toggle: ["toggle", "switch", "boolean", "checkbox", "开关", "切换"],
			countdown: ["countdown", "timer", "deadline", "deadlineat", "倒计时", "计时", "截止时间"],
			bars: ["bars", "bar", "barchart", "chart", "barschart", "条形图", "柱状图", "占比", "图表"],
			embed: ["embed", "iframe", "web", "webapp", "webpage", "frame", "网页", "网页应用", "内嵌网页"],
			image: ["image", "img", "picture", "photo", "graphic", "图片", "图像", "照片"]
		};
		/** canonical kind per alias (and per canonical name, so the lookup is one map). */
		const TASK_BLOCK_ALIAS_LOOKUP = (() => {
			const map = new Map();
			for (const [kind, names] of Object.entries(TASK_BLOCK_ALIASES)) {
				for (const name of [kind, ...names]) {
					const key = String(name).trim().toLowerCase().replace(/[\s_-]+/g, "");
					if (key.length > 0 && !map.has(key)) map.set(key, kind);
				}
			}
			return map;
		})();
		/** One alias (or canonical name) → canonical kind id, or "" when nothing matches. */
		function taskBlockKindOf(value) {
			if (typeof value !== "string") return "";
			const key = value.trim().toLowerCase().replace(/[\s_-]+/g, "");
			if (key.length === 0) return "";
			const direct = TASK_BLOCK_ALIAS_LOOKUP.get(key);
			if (direct !== void 0) return direct;
			return TASK_BLOCK_KINDS.includes(key) ? key : "";
		}
		/** Which field an array payload of one kind belongs in (`{ "stats": [...] }`). */
		const TASK_BLOCK_ARRAY_FIELDS = { stats: "items", bars: "items", chips: "items", links: "items", checklist: "items", kv: "rows", table: "rows", trend: "values" };
		/** Keys that are metadata or field names, never a kind written as a key. */
		const TASK_BLOCK_RESERVED_KEYS = new Set(["op", "id", "bid", "key", "x", "y", "w", "h", "blocks", "block", "kind", "requested", "raw", "title", "name", "value", "label", "text", "items", "rows", "columns", "values", "url", "src", "height", "fill", "control", "language", "unit", "max", "min", "step", "until", "done", "caption", "alt", "action", "index", "at", "time"]);
		/**
		 * Recover the canonical `{ kind, …fields }` shape from whatever a conversation wrote.
		 * Three shapes are accepted, because all three happen in practice:
		 *   1. `{ "kind": "stats", "items": [...] }` — the documented one;
		 *   2. `{ "stats": [...] }` / `{ "trend": { … } }` / `{ "note": "…" }` — the kind as the key;
		 *   3. a kind-less object the fields give away (`{ until: … }`, `{ src: … }`, `{ text: … }`).
		 * Anything that still cannot be recognised is left untouched, so the caller can decide.
		 */
		function coerceTaskBlockShape(raw) {
			if (raw === null || typeof raw !== "object" || Array.isArray(raw)) return raw;
			const declared = typeof raw.kind === "string" ? raw.kind : "";
			let kind = taskBlockKindOf(declared);
			const out = { ...raw };
			for (const [key, value] of Object.entries(raw)) {
				if (TASK_BLOCK_RESERVED_KEYS.has(key) || typeof key !== "string") continue;
				const aliased = taskBlockKindOf(key);
				if (aliased === "" || (kind !== "" && aliased !== kind)) continue;
				const payload = taskBlockPayloadOf(aliased, value);
				if (payload === null) continue;
				delete out[key];
				for (const [field, fieldValue] of Object.entries(payload)) {
					const existing = out[field];
					// Never overwrite content the block already carries: some replies write both
					// `{ "heading": "标题", "text": "更长的正文" }`, where `text` is the real body.
					if (existing === void 0 || (typeof existing === "string" && existing.trim().length === 0) || (Array.isArray(existing) && existing.length === 0)) out[field] = fieldValue;
				}
				kind = aliased;
				break;
			}
			if (kind === "") {
				const inferred = inferTaskBlockShape(raw);
				if (inferred !== null) return { ...out, ...inferred };
			}
			if (kind !== "" && out.kind !== kind) out.kind = kind;
			return out;
		}
		/** Turn one `kind-as-key` payload into the fields that kind expects, or null. */
		function taskBlockPayloadOf(kind, value) {
			if (value === null || value === void 0) return null;
			if (Array.isArray(value)) {
				const field = TASK_BLOCK_ARRAY_FIELDS[kind];
				return field === void 0 ? null : { [field]: value };
			}
			if (typeof value === "object") {
				const field = TASK_BLOCK_ARRAY_FIELDS[kind];
				// `{ "stats": { "label": "上证", "value": "3,215" } }` is one item, not a field bag.
				if (field !== void 0 && value[field] === void 0 && (value.label !== void 0 || value.value !== void 0 || value.url !== void 0 || value.id !== void 0 || value.key !== void 0)) {
					return { [field]: [value] };
				}
				return { ...value };
			}
			if (typeof value === "string") {
				...
				if (kind === "embed") return { url: value };
				if (kind === "code") return { text: value };
				if (kind === "links") return { items: [{ url: value, label: value }] };
				if (kind === "chips") return { items: [value] };
				if (kind === "counter" || kind === "toggle") return { label: value };
				return { text: value };
			}
			if (typeof value === "number" || typeof value === "boolean") {
				if (kind === "progress") return { value, max: 100 };
				if (kind === "counter" || kind === "toggle") return { value };
				return { text: String(value) };
			}
			return null;
		}
		/**
		 * Best effort kind for a block that names none (or names one that does not exist),
		 * judged by the fields it does carry. Returns the shaped block, or null.
		 */
		function inferTaskBlockShape(fields) {
			const all = (value, test) => Array.isArray(value) && value.length > 0 && value.every(test);
			const hasText = typeof fields.text === "string" && fields.text.trim().length > 0;
			if (fields.until !== void 0) return { kind: "countdown" };
			if (typeof fields.src === "string" && fields.src.trim().length > 0) return { kind: "image" };
			if (typeof fields.url === "string" && /^https?:\/\//i.test(fields.url.trim())) return { kind: "embed" };
			if (all(fields.columns, (entry) => typeof entry === "string") && Array.isArray(fields.rows)) return { kind: "table" };
			if (all(fields.values, (entry) => typeof entry === "number")) return { kind: "trend" };
			if (all(fields.items, (entry) => entry !== null && typeof entry === "object" && entry.id !== void 0 && typeof entry.label === "string")) return { kind: "checklist" };
			if (all(fields.items, (entry) => entry !== null && typeof entry === "object" && typeof entry.url === "string")) return { kind: "links" };
			if (all(fields.items, (entry) => typeof entry === "string")) return { kind: "chips" };
			if (all(fields.items, (entry) => entry !== null && typeof entry === "object" && typeof entry.label === "string" && entry.value !== void 0)) return { kind: "stats" };
			if (all(fields.rows, (entry) => Array.isArray(entry) || (entry !== null && typeof entry === "object"))) return { kind: "kv" };
			if (typeof fields.value === "boolean") return { kind: "toggle" };
			if (typeof fields.value === "number" && typeof fields.max === "number") return { kind: "progress" };
			// `{ label: "上证指数", value: "3,215.42" }` — one stat without the `items` wrapper.
			if (typeof fields.label === "string" && fields.value !== void 0) return { kind: "stats", items: [{ label: fields.label, value: String(fields.value) }] };
			if (hasText && typeof fields.language === "string") return { kind: "code" };
			if (hasText) return { kind: "text" };
			if (typeof fields.language === "string") return { kind: "code" };
			return null;
		}
		/**
		 * Short human name for a control a conversation asked for but this client cannot build:
		 * the kind it named (canonically, `kind` field or kind-as-key), or the fields it used.
		 */
		function describeTaskBlockRequest(raw) {
			if (raw === null || typeof raw !== "object") return "(空)";
			const declared = typeof raw.kind === "string" ? raw.kind.trim() : "";
			if (declared.length > 0 && declared !== "unknown") return declared.slice(0, 40);
			if (typeof raw.requested === "string" && raw.requested.trim().length > 0 && raw.requested.trim() !== "unknown") return raw.requested.trim().slice(0, 40);
			for (const key of Object.keys(raw)) {
				if (TASK_BLOCK_RESERVED_KEYS.has(key)) continue;
				if (taskBlockKindOf(key) !== "") return key.slice(0, 40);
			}
			const fields = Object.keys(raw).filter((key) => !TASK_BLOCK_RESERVED_KEYS.has(key));
			return fields.length > 0 ? "字段 " + fields.slice(0, 4).join("/") : "(空)";
		}
		...
		function taskBlockLabel(block) {
			...
			else if (typeof block.name === "string") excerpt = block.name;
		 ...
		 */
		function sanitizeTaskBlockFields(input, index) {
			if (input === null || typeof input !== "object") return null;
			// Whatever shape the conversation wrote is recovered here: the canonical `kind`
			// field, the kind used as a key, a Chinese kind name, or fields that give it away.
			const raw = coerceTaskBlockShape(input);
			...
			switch (kind) {
				...
				case "model": {
					// The 3D viewer block: a URL to a .glb/.gltf/.obj/.stl, an inline data URL, or
					// nothing at all (an empty viewer the user drops a file onto).
					const src = typeof raw.src === "string" ? raw.src.trim() : "";
					const allowed = src.startsWith("https://") || src.startsWith("http://") || /^data:(application|model)\//i.test(src);
					if (src.length > 0 && (!allowed || src.length > CANVAS_MODEL_LIMIT)) return null;
					const name = typeof raw.name === "string" ? raw.name.trim().slice(0, 120) : "";
					const height = typeof raw.height === "number" && Number.isFinite(raw.height) ? Math.min(Math.max(Math.round(raw.height), 160), 1200) : 360;
					return {
						kind,
						src,
						name,
				...
				case "embed": {
					...
					// `control: true` routes the frame through the local page bridge so the
					// workbench (and the agent) can actually operate the page.
					const control = raw.control === true;
					return { ...base, kind, url, title, fill, control, height: clampTaskEmbedHeight(raw.height) };
				}
				case "trend": {
					const label = typeof raw.label === "string" ? raw.label.trim().slice(0, 40) : "";
					const unit = typeof raw.unit === "string" ? raw.unit.trim().slice(0, 12) : "";
					const items = [];
					// `items` is this kind's own canonical shape; `values` / `points` are the compact
					// forms a conversation writes. Reading only those two lost every trend control
					// that went through a second sanitize (canvas adopt / add / refactor round-trip).
					const feed = Array.isArray(raw.items) ? raw.items : Array.isArray(raw.points) ? raw.points : Array.isArray(raw.values) ? raw.values : [];
				...
				default: {
					...
					// A stored placeholder already carries its original fields under `raw`; that
					// snapshot is kept instead of nesting one wrapper inside the next.
					const carried = raw.raw !== void 0 && raw.raw !== null && typeof raw.raw === "object" && !Array.isArray(raw.raw) ? raw.raw : null;
					const source = carried ?? raw;
					const snapshot = {};
					for (const [key, value] of Object.entries(source)) {
						if (key === "kind" || key === "requested" || key === "raw") continue;
						const type = typeof value;
						if (type === "string") snapshot[key] = value.slice(0, 4000);
						else if (type === "number" || type === "boolean" || value === null) snapshot[key] = value;
						// Lists are kept when they are flat: a reply that wrote `{ "stats": [...] }`
						// stays recoverable later (a future kind, or the coercion above once the
						// client knows that shape) instead of degrading to an empty placeholder.
						else if (Array.isArray(value)) {
							const flat = value.slice(0, 48);
							if (flat.every((entry) => typeof entry === "string" || typeof entry === "number" || typeof entry === "boolean")) snapshot[key] = flat;
							else if (flat.every((entry) => entry !== null && typeof entry === "object" && !Array.isArray(entry) && Object.values(entry).every((cell) => typeof cell === "string" || typeof cell === "number" || typeof cell === "boolean"))) snapshot[key] = flat;
						}
					}
					// An already-unsupported block keeps the name it was authored with, so a second
					// sanitize round-trip never turns "看板" into a bare "unknown".
					const carriedName = typeof raw.requested === "string" && raw.requested.trim().length > 0 && raw.requested.trim() !== "unknown" ? raw.requested.trim() : "";
					const requested = carriedName.length > 0 ? carriedName : kind.length > 0 && kind !== "unknown" ? kind : "(空)";
					return { kind: "unknown", requested: requested.slice(0, 40), raw: snapshot };
				}
			}
		}
		/**
		...
		function TaskCard(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-cardBody", "data-fill": props.fill === true ? "1" : void 0, children }),
		...
		function startTaskCardWatcher() {
			...
			const tick = () => {
				...
				for (const row of rows) {
					...
					// One reply may carry both protocols: a [taskcard] block and a [canvas] block.
					const hasCard = text.indexOf(TASK_SPEC_OPEN) !== -1 && text.indexOf(TASK_SPEC_CLOSE) !== -1;
					const hasCanvas = text.indexOf("[canvas]") !== -1 && text.indexOf("[/canvas]") !== -1;
					if (!hasCard && !hasCanvas) continue;
					...
					seen.delete(row);
					let ok = true;
					// The canvas first: a card refactor in the same reply may refer to what it added.
					if (hasCanvas) ok = applyConversationCanvasSpec(text) && ok;
					if (hasCard) {
						const spec = parseTaskCardSpec(text);
						if (spec !== null) ok = applyTaskCardSpec(spec).ok && ok;
					}
					if (ok) rememberAppliedTaskSpec(hash);
				}
				if (seen.size > 500) {
		...
		//#region notices (a main-conversation [canvas] block lands while the canvas may be shut)
		const taskNoticeListeners = new Set();
		let taskNoticeSeq = 0;
		/** Tell the board that something happened elsewhere (e.g. the main conversation). */
		function publishTaskNotice(text, canvasId) {
			if (typeof text !== "string" || text.trim().length === 0) return;
			taskNoticeSeq += 1;
			const notice = { id: taskNoticeSeq, text: text.trim().slice(0, 240), canvasId: typeof canvasId === "string" && canvasId.length > 0 ? canvasId : null, at: Date.now() };
			for (const listener of [...taskNoticeListeners]) {
				try { listener(notice); } catch { /* a listener must never break the UI */ }
			}
		}
		function subscribeTaskNotice(listener) {
		...
		function TaskEmbedView(props) {
			...
			// Controlled apps go through the local page bridge, whose injected agent lets the
			// workbench (and therefore the agent) click, type and read inside the page.
			const control = block.control === true;
			const bridgeTarget = control ? taskBridgeTarget(block.url) : "";
			const [bridgeState, setBridgeState] = (0, react.useState)(() => taskBridgePageState(bridgeTarget));
			(0, react.useEffect)(() => {
				if (!control) return void 0;
				// A status socket keeps the badge live (and tells the panel a page is registered).
				return subscribeTaskBridgePage(bridgeTarget, () => setBridgeState({ ...taskBridgePageState(bridgeTarget) }));
			}, [control, bridgeTarget]);
			const bridgeUrl = control ? taskBridgePageUrl(block.url).src : block.url;
			const bridgeLabel = control
				? bridgeState.state === "connected" ? "受控 · 已连接" : bridgeState.state === "offline" ? "受控 · 桥接离线" : bridgeState.state === "no-page" ? "受控 · 未连上" : "受控 · 连接中…"
				: "";
			/**
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							control && (0, react_jsx_runtime.jsx)("span", {
								className: "dsh-tc-appBadge" + (bridgeState.state === "connected" ? " dsh-tc-appBadgeOn" : ""),
								title: "受控模式：页面经由本机桥接代理，agent 可以点击 / 输入 / 读取。状态：" + (bridgeState.state === "connected" ? "已连接" : bridgeState.state === "offline" ? "桥接服务未启动" : "等待页面连上"),
								children: bridgeLabel
							}),
		...
		/**
		 * One image control: the picture itself, plus an explicit state when it cannot load.
		 *
		 * A remote `src` is loaded by the *browser*, so it can fail for reasons this plugin
		 * never sees — an unreachable host (blocked network), hot-link protection, a dead link,
		 * or a URL that is a web page rather than an image. Rendering a bare `<img>` leaves an
		 * invisible broken icon in that case, which reads as "the canvas is broken". The states
		 * below say which address failed, why it might have failed, and what to do instead.
		 */
		function TaskImageView(props) {
			const { block, onReplace } = props;
			const [state, setState] = (0, react.useState)("loading");
			const [attempt, setAttempt] = (0, react.useState)(0);
			// A new source (the "use a local file" replacement, an edited src, a refactor) has to
			// be loaded again: without this, React reuses the instance and the failure state would
			// stick to a block that is perfectly displayable now.
			const [loadedSrc, setLoadedSrc] = (0, react.useState)(block.src);
			if (loadedSrc !== block.src) {
				setLoadedSrc(block.src);
				setState("loading");
			}
			const fileRef = (0, react.useRef)(null);
			const host = (() => {
				const match = /^[a-z][a-z0-9+.-]*:\/\/([^/?#]+)/i.exec(block.src);
				return match === null ? "" : match[1];
			})();
			const isData = /^data:/i.test(block.src);
			/** Replace this control's source with a local file (downscaled), no network involved. */
			const pickLocal = () => {
			...
			const onFile = (event) => {
				const file = event.target.files !== null && event.target.files !== void 0 ? event.target.files[0] : null;
				event.target.value = "";
				if (file === void 0 || file === null || typeof onReplace !== "function") return;
				canvasImageDataUrl(file, (dataUrl) => {
					if (typeof dataUrl !== "string" || dataUrl.length === 0) return;
					onReplace({ ...block, src: dataUrl, alt: block.alt !== void 0 && block.alt.length > 0 ? block.alt : file.name.replace(/\.[a-z0-9]+$/i, "") });
				});
			};
			const fileInput = typeof onReplace === "function"
				? (0, react_jsx_runtime.jsx)("input", {
					ref: fileRef,
					type: "file",
					accept: "image/*",
					className: "dsh-tc-imgFailFile",
					onChange: onFile
				})
				: null;
			if (state === "error") {
				return (0, react_jsx_runtime.jsxs)("figure", {
					className: "dsh-tc-imgFailed",
					children: [
						(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-imgFailTitle", children: "图片加载失败" }),
						(0, react_jsx_runtime.jsx)("div", {
							className: "dsh-tc-imgFailHint",
							children: isData
								? "这段内联图片数据无法解码（可能被截断或格式不对）。"
								: host.length > 0
									? "取不到 " + host + "：可能是网络到不了这个站点、图床防盗链，或者图片地址已失效。"
									: "这个地址无法作为图片加载。"
						}),
						(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-imgFailUrl", title: block.src, children: block.src.length > 180 ? block.src.slice(0, 180) + "…" : block.src }),
						(0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-tc-imgFailRow",
							children: [
								typeof onReplace === "function" && (0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-btnPrimary", onClick: pickLocal, children: "用本地图片替换…" }),
								(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-btn", onClick: () => { setState("loading"); setAttempt((value) => value + 1); }, children: "重试" }),
								(0, react_jsx_runtime.jsx)("a", { className: "dsh-tc-btn", href: block.src, target: "_blank", rel: "noreferrer noopener", children: "在新标签打开" }),
								(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-btn", onClick: () => copyTaskText(block.src), children: "复制地址" })
							]
						}),
						(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-imgFailHint", children: typeof onReplace === "function" ? "也可以直接把本地图片粘贴 / 拖到画布上，或点 ✎ 改 src；本地图片不依赖网络。" : "点这项的 ✎ 改 src，或把本地图片粘贴 / 拖到画布上。" }),
						fileInput
					]
				});
			}
			return (0, react_jsx_runtime.jsxs)("figure", {
				...
				children: [
					...
					state === "loading" && (0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-imgLoading", children: "图片加载中…" }),
		...
		/**
		 * Model bytes that were too big to write into the canvas state, kept for this session
		 * and keyed by control id: a freshly created viewer item picks its model up from here,
		 * so dropping a 30 MB .glb still shows it (it just cannot survive a reload or sync).
		 */
		const pendingTaskModels = new Map();
		/**
		 * Read one 3D file into both an ArrayBuffer (to display right away) and a bounded data
		 * URL (to persist into the block when it is small enough).
		 */
		function readTaskModelFile(file, onDone) {
			if (typeof FileReader === "undefined") { onDone(null, null, "这个环境无法读取本地文件"); return; }
			const reader = new FileReader();
			reader.onerror = () => onDone(null, null, "读取文件失败");
			reader.onload = () => {
				const buffer = reader.result;
				if (!(buffer instanceof ArrayBuffer)) { onDone(null, null, "读取文件失败"); return; }
				const name = typeof file.name === "string" ? file.name : "";
				if (buffer.byteLength > CANVAS_MODEL_INLINE) { onDone(buffer, name, ""); return; }
				const binary = new Uint8Array(buffer);
				let text = "";
				for (let index = 0; index < binary.length; index += 0x8000) text += String.fromCharCode.apply(null, binary.subarray(index, index + 0x8000));
				try {
					onDone(buffer, name, "data:application/octet-stream;base64," + btoa(text));
				} catch {
					onDone(buffer, name, "");
				}
			};
		...
		/** True for the model extensions the viewer understands. */
		function isTaskModelFile(file) {
			if (file === null || file === void 0 || typeof file.name !== "string") return false;
			return /\.(glb|gltf|obj|stl)$/i.test(file.name.trim());
		}
		/**
		 * The 3D viewer as a control: a sandboxed iframe running the embedded viewer page.
		 *
		 * The viewer has its own document and only talks to this component over postMessage,
		 * exactly like an embedded web app — but the model bytes come from the host page: a
		 * `data:` source is decoded here and transferred in, a URL is fetched by the viewer,
		 * and a dropped/picked local file is handed over and (when small enough) written back
		 * into the block so the canvas keeps it.
		 */
		function TaskModelView(props) {
			const { block, onReplace } = props;
			const frameRef = (0, react.useRef)(null);
			const readyRef = (0, react.useRef)(false);
			const fileRef = (0, react.useRef)(null);
			const [doc, setDoc] = (0, react.useState)(null);
			const [wireframe, setWireframe] = (0, react.useState)(block.wireframe === true);
			const [grid, setGrid] = (0, react.useState)(block.grid !== false);
			const [background, setBackground] = (0, react.useState)(block.background === "light" ? "light" : "dark");
			const [status, setStatus] = (0, react.useState)({ state: "loading", message: "正在准备 3D 查看器…", triangles: 0, meshes: 0, name: "" });
			const [localBytes, setLocalBytes] = (0, react.useState)(0);
			...
			// The viewer page itself: decompressed once per session and reused by every item.
			(0, react.useEffect)(() => {
				let alive = true;
				taskViewerHtml().then((html) => { if (alive) setDoc(html); }, (error) => {
					if (alive) setStatus({ state: "failed", message: error instanceof Error ? error.message : String(error), triangles: 0, meshes: 0, name: "" });
				});
				return () => { alive = false; };
			}, []);
			const pushSource = () => {
				if (src.length === 0) { send({ type: "clear" }); return; }
				if (src.startsWith("data:")) {
					...
					try {
						...
						send({ type: "load", name: block.name.length > 0 ? block.name : "模型", buffer: bytes.buffer }, [bytes.buffer]);
					} catch {
						setStatus({ state: "failed", message: "内嵌的模型数据无法解码", triangles: 0, meshes: 0, name: "" });
					}
					return;
				}
				send({ type: "loadUrl", url: src, name: block.name });
			};
			const pushOptions = () => send({ type: "options", wireframe, grid, background });
			(0, react.useEffect)(() => {
				const onMessage = (event) => {
					...
					if (message.type === "ready") {
						...
						pushOptions();
						// A model handed over by a drop lives in memory until the viewer boots.
						const bid = typeof block.bid === "string" ? block.bid : "";
						const pending = bid.length > 0 ? pendingTaskModels.get(bid) : void 0;
						if (pending !== void 0) {
							pendingTaskModels.delete(bid);
							setLocalBytes(pending.buffer.byteLength);
							if (!send({ type: "load", name: pending.name, buffer: pending.buffer }, [])) pushSource();
						} else {
							pushSource();
						}
						return;
					...
					if (message.type === "status") {
						setStatus({
							state: message.ok === true ? "ready" : "failed",
							message: typeof message.message === "string" ? message.message : "",
							triangles: typeof message.triangles === "number" ? message.triangles : 0,
							meshes: typeof message.meshes === "number" ? message.meshes : 0,
							name: typeof message.name === "string" ? message.name : ""
						});
						return;
					}
					if (message.type === "picked" && typeof message.name === "string" && message.name.length > 0) {
						// The viewer's own picker only knows the name; the bytes arrive via `dropped`.
						setStatus((previous) => ({ ...previous, name: message.name }));
						return;
					}
					if (message.type === "dropped" && message.buffer instanceof ArrayBuffer) {
						const buffer = message.buffer;
						const name = typeof message.name === "string" ? message.name : "";
						if (buffer.byteLength > CANVAS_MODEL_INLINE) {
							// Too big for the canvas state: show it now, say that it is session-only.
							setLocalBytes(buffer.byteLength);
							setStatus({ state: "ready", message: "模型较大（" + Math.max(1, Math.round(buffer.byteLength / 1048576)) + " MB），只在这次会话里显示，未写入画布", triangles: 0, meshes: 0, name });
							send({ type: "load", name, buffer }, []);
							return;
						}
						const binary = new Uint8Array(buffer);
						let text = "";
						for (let index = 0; index < binary.length; index += 0x8000) text += String.fromCharCode.apply(null, binary.subarray(index, index + 0x8000));
						try {
							if (typeof onReplace === "function") onReplace({ ...block, src: "data:application/octet-stream;base64," + btoa(text), name: name.length > 0 ? name : block.name });
							setLocalBytes(0);
						} catch {
							setStatus({ state: "ready", message: "这个模型无法写入画布（数据过大），只在这次会话里显示", triangles: 0, meshes: 0, name });
						}
						send({ type: "load", name, buffer }, []);
					}
				};
			...
			}, [block, onReplace, src, wireframe, grid, background]);
			// A new source (drop, picker, a conversation refactor) is pushed to the running viewer.
			(0, react.useEffect)(() => {
				if (readyRef.current) pushSource();
			}, [src]);
			(0, react.useEffect)(() => {
				if (readyRef.current) pushOptions();
			...
			const onFile = (event) => {
				...
				setStatus({ state: "loading", message: "正在读取 " + file.name + "…", triangles: 0, meshes: 0, name: file.name });
				readTaskModelFile(file, (buffer, name, dataUrl) => {
					if (buffer === null) { setStatus({ state: "failed", message: name ?? "读取文件失败", triangles: 0, meshes: 0, name: "" }); return; }
					if (typeof onReplace === "function" && typeof dataUrl === "string" && dataUrl.length > 0) onReplace({ ...block, src: dataUrl, name: name.length > 0 ? name : block.name });
					else setLocalBytes(buffer.byteLength);
					send({ type: "load", name, buffer }, []);
				});
			};
			const info = status.state === "failed" && status.message.length > 0
				? status.message
				: status.triangles > 0
					? status.triangles.toLocaleString("en-US") + " 三角形 · " + status.meshes + " 网格" + (localBytes > 0 ? " · 仅本机" : "")
					: status.message;
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-tc-model",
				"data-state": status.state,
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						className: "dsh-tc-modelBar",
						children: [
							(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-modelName", title: status.name.length > 0 ? status.name : block.name, children: status.name.length > 0 ? status.name : block.name.length > 0 ? block.name : "3D 模型" }),
							(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-modelInfo", title: info, children: info }),
							(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-modelBtn", onClick: pick, title: "打开本地的 .glb/.gltf/.obj/.stl（也可以直接拖进这个框）", children: "打开模型…" }),
							(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-modelBtn" + (wireframe ? " dsh-tc-iconOn" : ""), onClick: () => setWireframe((value) => !value), title: "显示三角网格边", children: "线框" }),
							(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-modelBtn" + (grid ? " dsh-tc-iconOn" : ""), onClick: () => setGrid((value) => !value), title: "显示地面网格", children: "网格" }),
							(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-modelBtn", onClick: () => setBackground((value) => (value === "dark" ? "light" : "dark")), title: "深色/浅色背景", children: "背景" }),
							(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-modelBtn", onClick: () => send({ type: "fit" }), title: "把模型重新放到视野中央", children: "复位视角" })
						]
					}),
					(0, react_jsx_runtime.jsx)("div", {
						className: "dsh-tc-modelStage",
						children: doc === null
							? (0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-modelWait", children: status.message })
							: (0, react_jsx_runtime.jsx)("iframe", {
								ref: frameRef,
								className: "dsh-tc-modelFrame",
								title: "3D 模型查看器",
					...
					(0, react_jsx_runtime.jsx)("input", { ref: fileRef, type: "file", accept: ".glb,.gltf,.obj,.stl", className: "dsh-tc-modelFile", onChange: onFile })
				]
			});
		}
		/** Render one declarative control block authored by the conversation. */
		function TaskBlockView(props) {
			const { block, scope, value, onWidget, onRun, feedbackText, blockIndex, onResizeHeight, onReplaceImage } = props;
			...
			// The image body offers "replace with a local file"; the callback is bound to this
			// block's index here, because the view itself only knows the block it renders.
			if (block.kind === "image") return (0, react_jsx_runtime.jsx)(TaskImageView, {
				block,
				onReplace: typeof onReplaceImage === "function" ? (next) => onReplaceImage(blockIndex, next) : void 0
			});
			if (block.kind === "model") return (0, react_jsx_runtime.jsx)(TaskModelView, {
				block,
				onReplace: typeof onReplaceImage === "function" ? (next) => onReplaceImage(blockIndex, next) : void 0
			});
		...
		function TaskUserBody(props) {
			const { card, onWidget, onWidgetState, onBlockResize, onBlockReplace, cardId, thumbnail } = props;
			...
			const replaceSink = onBlockReplace !== void 0 ? (index, block) => onBlockReplace(cardId, index, block) : void 0;
		...
		function TaskCardEditor(props) {
			...
			const save = () => {
				...
				for (const row of embeds) {
					...
					cleanEmbeds.push({ url: normalized, title: row.title.trim().slice(0, 80), height: clampTaskEmbedHeight(Number(row.height)), fill: row.fill === true, control: row.control === true });
				}
								...
								(0, react_jsx_runtime.jsxs)("div", { className: "dsh-tc-field", children: [
									...
									embeds.map((row, index) => (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											...
											(0, react_jsx_runtime.jsxs)("label", { className: "dsh-tc-editCheck", title: "受控：应用经由本机网页桥接代理，agent 可以在里面点击 / 输入 / 读取（需先启动 bridge/server.mjs）", children: [
												(0, react_jsx_runtime.jsx)("input", { type: "checkbox", checked: row.control, onChange: (event) => setEmbedRow(index, "control", event.target.checked) }),
												"受控"
											] }),
											(0, react_jsx_runtime.jsx)("button", {
												type: "button",
		...
		async function taskChatPromptViaSessions(ctx, text) {
			try {
				...
				const current = taskListCurrentId(taskSessionsListState(sessions));
				if (current === null) {
					return { ok: false, message: "当前没有进行中的会话：请先在侧栏选择或新建一个会话" };
				}
				// The current session is normally retained by the main view, so borrowing its
				// binding works. On a client where it is not (or a panel opened before the view
				// retained it) the session is retained here for the length of the prompt.
				let borrowed = typeof sessions.binding === "function" ? sessions.binding(current) : void 0;
				let release = null;
				if ((borrowed === null || borrowed === void 0) && typeof sessions.retain === "function") {
					const reference = sessions.retain(current, { source: "dsh-task-console" });
					await Promise.race([reference.ready, taskDelay(2500)]);
					borrowed = reference.binding;
					release = () => { try { reference.release(); } catch { /* already released */ } };
				}
				try {
					const session = borrowed !== null && borrowed !== void 0 && borrowed.session !== void 0 ? borrowed.session : void 0;
		...
		/** Prompt section listing the card's controlled pages and the [page] action vocabulary. */
		function composePagePromptSection(blocks) {
			const targets = collectBridgeTargets([blocks]);
			if (targets.length === 0) return "";
			return [
				"可操作的网页应用（受控，已在卡片里）：" + targets.map((entry) => entry.url + (entry.title.length > 0 ? "（" + entry.title + "）" : "")).join("、"),
				"需要真的操作这个网页时，在同一个回复里再输出一个 [page] 块，客户端会把它执行掉并把结果回报给你：",
				"[page] { \"actions\": [ { \"action\": \"click\", \"selector\": \"#buy\" }, { \"action\": \"type\", \"selector\": \"#q\", \"value\": \"关键词\", \"submit\": true }, { \"action\": \"read\" } ], \"url\": \"可选：要操作哪个网页\", \"then\": \"可选：读完成结果后希望我继续做的事（会带着结果再问你一次）\" } [/page]",
				"可用动作：" + TASK_BRIDGE_ACTIONS.join(" / ") + "。字段：selector（CSS 选择器）或 text（按钮上的文字，二选一）；type 需要 selector 与 value（可选 submit: true 回车提交）；select 需要 selector 与 value 或 label；check 需要 selector 与 checked；scroll 可用 selector 或 y 或 bottom: true；wait 可用 selector 或 ms；eval 用 code。先 read 看清页面（会返回标题、正文与输入框），再操作，必要时再 read 确认；不要输出 explain，只给块。" 
			].join("\n");
		}
		...
		function composeTaskCardChatPrompt(card, text) {
			...
			return [
				...
				"可用控件 kind 与字段：" + TASK_BLOCK_KINDS.join(" / ") + "（text/heading/note：{ text }；stats：{ items: [{ label, value }] }；progress：{ label?, value, max?, unit? }；kv：{ rows: [[键, 值], …] }；links：{ items: [{ label?, url }] }；chips：{ items: [\"标签\", …] }；trend：{ label?, unit?, values: [数字…] } 或 points: [{ label?, value }]；table：{ columns: [表头…], rows: [[单元格…]] }；code：{ language?, text }；toggle：{ key, label, value? }；countdown：{ label?, until: ISO 时间或毫秒时间戳, done? }；bars：{ label?, items: [{ label, value }] }；checklist：{ key, items: [{ id, label }] }；counter：{ key, label, step?, min?, max? }；button：{ action: copy|link|fill, label, value }；embed：{ url, title?, height?, fill? }；image：{ src, caption?, alt? }）。字段缺失或写错的控件会被丢弃，所以请按上面的字段名书写；checklist / counter / toggle 等交互控件请带稳定 key。",
				"控件名要写在 kind 字段里，其余字段与它平级——写成 { \"kind\": \"stats\", \"items\": [ … ] }，**不要**把控件名当键写成 { \"stats\": [ … ] }（后者会被跳过并在结果里点名）。",
				"图片控件（image）：src 由**用户的浏览器**去取。只在你能确认真实存在、且用户网络能打开时写 https 图片地址；想画示意图/曲线而没有可靠图床时，用自包含的 { \"kind\": \"image\", \"src\": \"data:image/svg+xml;base64,…\" }（离线也能显示）；不要写搜索页、网页地址或猜测的图片链接——那只会显示「图片加载失败」；真实照片建议让用户直接粘贴到卡片上。",
				...
				// Cards joined by a link share their workspace context: a session started here gets
				// the neighbours' content, which is what the dashed group on the canvas means.
				composeLinkPromptSection(id, typeof card.canvasId === "string" ? card.canvasId : void 0),
				composePagePromptSection(card.blocks)
		...
		/** Read the sessions list state through either generation of the service. */
		function taskSessionsListState(sessions) {
			try {
				const source = sessions === null || sessions === void 0 ? null : sessions.list;
				if (typeof source === "function") return source();
				if (source !== null && source !== void 0 && typeof source.getSnapshot === "function") return source.getSnapshot();
			} catch { /* an unreadable list is treated as empty */ }
			return null;
		}
		/**
		 * The session the main view currently holds.
		 *
		 * Older clients carried it as `list.current`. Newer ones dropped that field entirely
		 * (`SessionListState` is `{ ids, byId, phase, projectionsBySession }`), and the current
		 * session is the one the **main view retains** (`retainedBy.mainView`) — exactly how the
		 * harness's own panels resolve it. Without this fallback the plugin saw no current
		 * session at all on a newer client: a canvas refactor created its helper in the
		 * deployment's default workspace instead of the conversation's, and the card chat could
		 * not find a session to send to.
		 * @param state - a sessions list snapshot.
		 * @returns the session id, or null when there is none.
		 */
		function taskListCurrentId(state) {
			if (state === null || typeof state !== "object") return null;
			if (typeof state.current === "string" && state.current.length > 0) return state.current;
			...
			for (const row of Object.values(byId)) {
				...
				if ((retained.mainView ?? 0) > 0 && typeof row.id === "string" && row.id.length > 0) return row.id;
			}
			return null;
		}
		/** Status map stand-in: hooks cannot be called conditionally, so this keeps the slot stable. */
		const TASK_EMPTY_STATUSES = new Map();
		const TASK_EMPTY_STATUS_HOOK = () => TASK_EMPTY_STATUSES;
		/**
		 * Subagent rows of one session.
		 *
		 * A legacy list carried them as `subagentsByParent[sessionId] = { entries }`. The newer
		 * client drops that field and projects `subagentCatalog` per session instead, with the
		 * running flag moved to the shared status map (`useSessionStatus`).
		 * @param state - a sessions list snapshot.
		 * @param sessionId - the parent session.
		 * @param statuses - the status snapshot, when the harness provides one.
		 * @returns `{ entries }`, or undefined when this client reports no subagents at all.
		 */
		function taskSessionSubagents(state, sessionId, statuses) {
			if (state === null || typeof state !== "object" || typeof sessionId !== "string") return void 0;
			const legacy = state.subagentsByParent;
			if (legacy !== null && legacy !== void 0 && typeof legacy === "object") return legacy[sessionId];
			const projections = state.projectionsBySession;
			const projection = projections !== null && projections !== void 0 && typeof projections === "object" ? projections[sessionId] : void 0;
			const catalog = projection !== null && projection !== void 0 && projection.values !== void 0 ? projection.values.subagentCatalog : void 0;
			if (!Array.isArray(catalog)) return void 0;
			return {
				entries: catalog.map((entry) => ({
					id: entry !== null && typeof entry === "object" && typeof entry.id === "string" ? entry.id : "",
					label: entry !== null && typeof entry === "object" && typeof entry.label === "string" && entry.label.length > 0 ? entry.label : (entry !== null && typeof entry === "object" && typeof entry.id === "string" ? entry.id : "子代理"),
					activity: statuses !== null && statuses !== void 0 && typeof statuses.get === "function" && statuses.get(entry.id)?.running === true ? "running" : "idle"
				}))
			};
		}
		/** Current main-session id from the sessions service. */
		function currentTaskSessionId(sessions) {
			return taskListCurrentId(taskSessionsListState(sessions));
		}
		...
		function taskTempCreateOptions(ctx, cwd) {
			...
			try {
				...
				for (const item of items) {
					if (item === null || typeof item !== "object" || typeof item.path !== "string" || item.path.toLowerCase() !== target) continue;
					// Newer clients name the field `workspaceId`; older ones used `id`.
					const id = typeof item.workspaceId === "string" && item.workspaceId.length > 0 ? item.workspaceId : item.id;
					if (typeof id === "string" && id.length > 0) return { workspaceId: id };
				}
		 ...
		 *
		 * Two generations of the client sessions service are in the wild, and they differ in a
		 * way that silently breaks a helper session this plugin created itself:
		 *
		 *   - older: `sessions.binding(id)` returns the live binding of any listed session;
		 *   - newer: `binding(id)` only *borrows* the binding of a session somebody still
		 *     **retains** ("Borrow an already-retained binding without extending its lifetime"),
		 *     and the way to get one is `sessions.retain(id, { source })` (or `sessions.using`).
		 *
		 * A freshly created helper is retained by nobody, so on the newer client `binding()`
		 * reports nothing for it: the helper stayed empty and every refactor died with
		 * "临时 agent 会话不可用" while the session list slowly filled with blank sessions.
		 * Retaining it is therefore tried first; the returned `release` handle is handed to the
		 * caller, which releases it once the task is done.
		 * @param sessions - the client sessions service.
		 * @param childId - the session to bind.
		 * @returns `{ session, eventSource, release }`, or null when the child is unusable.
		 */
		async function openTaskChildSession(sessions, childId) {
			const adopt = (binding, release) => {
				...
				return { session, eventSource, release: typeof release === "function" ? release : () => {} };
			};
			...
			if (typeof sessions.retain === "function") {
				...
				while (Date.now() < deadline) {
					let reference = null;
					try {
						reference = sessions.retain(childId, { source: "dsh-task-console" });
						// `ready` resolves to the binding once the session is really open.
						await Promise.race([reference.ready, taskDelay(2500)]);
						const adopted = adopt(reference.binding, () => reference.release());
						if (adopted !== null) {
							await openWindow(adopted.session);
							return adopted;
						}
					} catch { /* a host that refuses the target: retry until the deadline */ }
					if (reference !== null) {
						try { reference.release(); } catch { /* already released */ }
					}
					await taskDelay(250);
				}
			}
			// Older clients (or a host without `retain`): a live session has a binding already.
			let binding = null;
			...
			const adopted = adopt(binding, null);
		...
		function createTaskCardAgentBridge(ctx) {
			...
			/**
			 * Run one natural-language task through a temporary helper session and hand its
			 * reply to the task's parser. `task` = { build(instruction) → prompt text,
			 * marker(prompt) → the line that identifies our own user message, parse(text),
			 * apply(spec), missing → the message when the reply carried no block }.
			 */
			return async function runTempAgentTask(task, payload, onEvent) {
				...
				let childId = null;
				// The binding of a helper session may be on loan (newer clients): release it when
				// the task is done, on every exit path, or the session leaks a retention.
				...
				try {
					...
					const instruction = task.build(payload);
					// The instruction embeds a marker line (card id / canvas id); it identifies
					// our own user message in the child's window even when several prompts pass
					// through the same session.
					const ownPromptMarker = task.marker(payload, instruction);
					let accepted = null;
					try {
					...
					while (Date.now() < deadline) {
						...
						if (latest.length > 0) {
							const parsed = task.parse(latest);
							...
							archiveChild(childId);
							return { ok: false, fallback: true, message: "临时 agent 会话的回合已结束但没有产出 " + task.block + "，已改用主会话" };
					...
					if (spec === null) spec = task.parse(latest);
					if (spec === null) {
						archiveChild(childId);
						return { ok: false, message: "agent 的回复里没有可用的 " + task.block + " 定义", text: latest };
					}
					const applied = task.apply(spec, payload, latest);
					archiveChild(childId);
					if (applied === null || applied === void 0 || applied.ok !== true) {
						return { ok: false, message: applied !== null && applied !== void 0 && applied.message ? applied.message : "更新失败：" + (applied !== null && applied !== void 0 && applied.reason ? applied.reason : "未知原因"), text: latest };
					}
					...
					if (typeof releaseChild === "function") {
						try { releaseChild(); } catch { /* already released */ }
					}
				}
			};
		}
		/** The card task: refactor one card through its [taskcard] block. */
		const TASK_CARD_AGENT_TASK = {
			block: "[taskcard]",
			build: (payload) => composeTaskCardChatPrompt(payload, payload.instruction),
			marker: (payload, instruction) => (typeof payload.id === "string" && payload.id.length > 0 ? "卡片 id：" + payload.id : instruction.slice(0, 40)),
			parse: (text) => parseTaskCardSpec(text),
			apply: (spec, payload) => {
				const target = { ...spec, id: payload.id };
				return typeof payload.applySpec === "function" ? payload.applySpec(target) : applyTaskCardSpec(target);
			}
		};
		/** Result of the last canvas agent run, so the dock can report what changed. */
		let canvasLastAgentResult = null;
		/** The canvas task: change a whole canvas through its [canvas] operation list. */
		const TASK_CANVAS_AGENT_TASK = {
			block: "[canvas]",
			build: (payload) => composeCanvasChatPrompt(payload, payload.instruction),
			marker: (payload, instruction) => (typeof payload.canvasId === "string" && payload.canvasId.length > 0 ? "画布 id：" + payload.canvasId : instruction.slice(0, 40)),
			parse: (text) => parseCanvasSpec(text),
			apply: (spec, payload) => {
				const outcome = applyCanvasSpec(payload.canvasId, spec);
				canvasLastAgentResult = outcome !== null && typeof outcome === "object" && outcome.ok === true ? outcome : null;
				return outcome;
			}
		};
		/**
		 * Describe one canvas for the canvas chat: every item, its controls and its place —
		 * so the agent edits what is actually there instead of guessing.
		 */
		function canvasInventory(canvas) {
			if (canvas === null || typeof canvas !== "object") return "（空画布）";
			const items = Object.values(canvas.cards).sort((left, right) => (left.y - right.y) || (left.x - right.x)).slice(0, 40);
			if (items.length === 0) return "（空画布：还没有任何卡片或控件）";
			return items.map((item) => {
				const blocks = effectiveTaskBlocks(item);
				const kinds = blocks.map((block) => block.kind + (typeof block.bid === "string" ? "#" + block.bid.slice(-4) : "")).join("+");
				const label = typeof item.title === "string" && item.title.length > 0 ? item.title : taskBlockLabel(blocks[0] ?? {});
				return "- id=" + item.id + " | " + (item.bare === true ? "裸控件" : "卡片") + " | " + label + " | 控件: " + (kinds.length > 0 ? kinds : "无") + " | x=" + Math.round(item.x) + " y=" + Math.round(item.y) + (typeof item.w === "number" ? " w=" + item.w : "") + (typeof item.h === "number" ? " h=" + item.h : "");
			}).join("\n");
		}
		/** Full prompt for the canvas chat: what is on the plane + the operation vocabulary. */
		function composeCanvasChatPrompt(payload, text) {
			const canvas = payload.canvas ?? null;
			const name = canvas !== null && typeof canvas.name === "string" ? canvas.name : "未命名画布";
			// A reference canvas mirrors one board card, so what is created here lands on it.
			const owner = instanceCardOf(typeof payload.canvasId === "string" ? payload.canvasId : "");
			return [
				"【任务台画布编辑】请按下面的要求修改这块画布，并只输出一个 [canvas] 块，不要任何解释、不要调用任何工具、不要反问。",
				"画布 id：" + (typeof payload.canvasId === "string" ? payload.canvasId : ""),
				"画布名称：" + name,
				owner.length > 0 ? "这块画布是任务台上某张卡片的副本画布：上面每个已绑定控件对应卡片的一个控件；用 add / addCard 新建的项会在应用后自动挂回那张卡片（成为它的新控件），所以缺什么就直接加，不必先问用户。" : "",
				"画布现有内容（每行一项，id 用来引用它）：",
				canvasInventory(canvas),
				"修改要求：" + (typeof text === "string" ? text.trim() : ""),
				"输出格式：[canvas] { \"ops\": [ … ] } [/canvas]，ops 是一串操作，按顺序执行：",
				"・{ \"op\": \"add\", \"kind\": \"控件类型\", \"x\": 数字?, \"y\": 数字?, …控件字段 } —— 在画布上新建一个控件（裸控件，不套卡片）。x/y 省略时放在默认位置。",
				"・{ \"op\": \"addCard\", \"title\": \"卡片标题\", \"blocks\": [ …控件… ], \"x\": 数字?, \"y\": 数字? } —— 新建一张卡片，里面放这些控件。",
				"・{ \"op\": \"addPlugin\", \"id\": \"插件 id 或包名\", \"title\": \"显示名\", \"height\": 数字?, \"bare\": true?, \"x\": 数字?, \"y\": 数字? } —— 把**另一个 DSH 插件**的界面放进画布显示（不局限于卡片与控件）。id 用下面「可挂载的插件」里的 id；若插件没有发布可嵌入入口，它会显示一条说明而不是空白。",
				"・{ \"op\": \"rebuild\", \"id\": \"已有项 id\" } —— 重新构建某个插件项（重新加载它的客户端包并重新挂载），插件更新或挂载失败后用这个。",
				"・{ \"op\": \"link\", \"from\": \"项 id\", \"to\": \"项 id\", \"kind\": \"context\"? } —— 给两项连线（默认 kind=context，含义：从其中任一项发起会话时，它们的工作区上下文共享，画布上会用一个虚线框把这一组框起来）。取消用 { \"op\": \"unlink\", \"from\": \"项 id\", \"to\": \"项 id\" }。",
				"・{ \"op\": \"update\", \"id\": \"已有项 id\", \"block\": { …新控件… } } —— 把该项的控件换成这个（只适合单项控件，会连同卡片上对应控件一起更新）。",
				"・{ \"op\": \"move\", \"id\": \"已有项 id\", \"x\": 数字, \"y\": 数字 } —— 移动一项。",
				"・{ \"op\": \"remove\", \"id\": \"已有项 id\" } —— 删除一项（若它绑定了卡片上的控件，会同时从卡片上移除）。",
				"・{ \"op\": \"rename\", \"name\": \"新画布名\" } —— 改画布名字。",
				"・{ \"op\": \"note\", \"text\": \"说明\" } —— 只回一句话给用户，不改画布。",
				"控件字段与卡片协议完全一致：" + TASK_BLOCK_KINDS.join(" / ") + "（text/heading/note：{ text }；stats：{ items: [{ label, value }] }；progress：{ label?, value, max?, unit? }；kv：{ rows: [[键, 值], …] }；links：{ items: [{ label?, url }] }；chips：{ items: [\"标签\", …] }；trend：{ label?, unit?, values: [数字…] }；table：{ columns: [表头…], rows: [[单元格…]] }；code：{ language?, text }；toggle：{ key, label, value? }；countdown：{ label?, until: ISO 时间或毫秒时间戳, done? }；bars：{ label?, items: [{ label, value }] }；checklist：{ key, items: [{ id, label }] }；counter：{ key, label, step?, min?, max? }；button：{ action: copy|link|fill, label, value }；embed：{ url, title?, height?, fill? }；image：{ src, caption?, alt? }；model：{ src?, name?, height?, wireframe?, grid?, background? }）。字段缺失或写错的控件会被丢弃，所以请严格按字段名书写。",
				"每个控件都必须把控件名写在 kind 字段里，其余字段与它平级——写成 { \"kind\": \"stats\", \"items\": [ … ] }，**不要**把控件名当键写成 { \"stats\": [ … ] }（后者会被当成无法识别的控件而跳过，摘要里会显示「跳过 …」）。一个完整例子：[canvas] { \"ops\": [ { \"op\": \"addCard\", \"title\": \"股市\", \"blocks\": [ { \"kind\": \"heading\", \"text\": \"主要指数\" }, { \"kind\": \"stats\", \"items\": [ { \"label\": \"上证\", \"value\": \"3,215\" } ] }, { \"kind\": \"trend\", \"values\": [1, 2, 3] } ] } ] } [/canvas]。",
				"图片控件（image）额外注意：src 由**用户的浏览器**去取，这里帮不了它。所以只在你能确认真实存在、且用户网络能打开时写 https 图片地址；想画示意图/曲线而没有可靠图床时，用自包含的 { \"kind\": \"image\", \"src\": \"data:image/svg+xml;base64,…\" }（离线也能显示）；不要写搜索页、网页地址或你猜测的图片链接——那只会显示「图片加载失败」。真正的照片建议让用户直接粘贴到画布上。",
				"3D 模型控件（model）：画布/卡片里的内嵌 3D 查看器，支持 .glb/.gltf/.obj/.stl。写 \"src\" 时用能被用户浏览器下载的 https 地址（需要对方允许跨域）或 data: 内嵌；**模型通常很大，不要自己往对话里塞 base64**——让用户把模型文件直接拖到画布或这个控件上（本机会自动读取，小模型会写进画布状态、大模型只在本次会话显示）。空 src 的 model 控件是一个「把模型拖进来」的空查看器。",
				"规则：只输出 [canvas] 块；不要在块外写任何文字；一次可以给出多个 ops；引用已有项时必须使用上面列出的 id 原样；用户要求的东西画布上还没有时，直接用 add / addCard 建出来，不要只写 note 说做不到。",
				composePluginPromptSection(),
				composeCanvasLinkSection(canvas),
				composePagePromptSection(canvas === null ? [] : Object.values(canvas.cards).flatMap((item) => effectiveTaskBlocks(item)))
			].filter((line) => typeof line === "string" && line.length > 0).join("\n");
		}
		/** Prompt section listing the plugins a canvas may host, plus how to add/rebuild them. */
		function composePluginPromptSection() {
			const rows = listPluginCandidates();
			if (rows.length === 0) return "";
			const lines = rows.slice(0, 40).map((row) => "- " + row.id + (row.embeddable ? "（已验证可挂载 ✓）" : ""));
			return [
				"可挂载的插件（客户端包都在启动清单里；用 addPlugin 放到画布上显示）：",
				...lines,
				"提示：标 ✓ 的插件已经成功挂载过，最稳妥；其它插件也可以尝试，若它没有发布可嵌入入口（globalThis." + pluginGlobalName("插件id") + ".mountEmbedded），画布上会显示一条说明。要新增一个还不存在的插件，先用插件工具把它创建/运行起来，再用 addPlugin 放到画布上。"
			].join("\n");
		}
		/** Parse the first [canvas]…[/canvas] block of a reply into an operation list. */
		function parseCanvasSpec(text) {
			if (typeof text !== "string") return null;
			const start = text.indexOf("[canvas]");
			let raw = null;
			if (start !== -1) {
				const after = start + "[canvas]".length;
				const end = text.indexOf("[/canvas]", after);
				if (end !== -1) raw = text.slice(after, end).trim();
			}
			if (raw === null) {
				const fence = /```canvas[\t ]*\n?([\s\S]*?)```/g.exec(text);
			...
			if (raw === null) return null;
			if (raw.startsWith("```")) raw = raw.replace(/^```[a-zA-Z0-9_-]*\s*/, "").replace(/```$/, "").trim();
			if (raw.length === 0 || raw.length > 20000) return null;
			let parsed = null;
			try { parsed = JSON.parse(raw); } catch { return null; }
			if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) return null;
			const feed = Array.isArray(parsed.ops) ? parsed.ops : Array.isArray(parsed) ? parsed : null;
			if (feed === null) return null;
			const ops = [];
			// Ops this client cannot perform. They are reported back in the summary instead of
			// vanishing: a conversation that wrote an unsupported control should be told which
			// one it was, rather than seeing "没有改动".
			const dropped = [];
			for (const entry of feed.slice(0, 40)) {
				if (entry === null || typeof entry !== "object" || Array.isArray(entry)) continue;
				const op = typeof entry.op === "string" ? entry.op : "";
				if (op === "add") {
					const block = sanitizeTaskBlock(entry, 0);
					if (block === null) {
						dropped.push("add:" + describeTaskBlockRequest(entry));
						continue;
					}
					if (block.kind === "unknown") {
						dropped.push("add:" + describeTaskBlockRequest(entry));
					...
					ops.push({ op, block, x: typeof entry.x === "number" && Number.isFinite(entry.x) ? Math.round(entry.x) : void 0, y: typeof entry.y === "number" && Number.isFinite(entry.y) ? Math.round(entry.y) : void 0 });
					continue;
				}
				if (op === "addCard") {
					const feedBlocks = Array.isArray(entry.blocks) ? entry.blocks : [];
					const blocks = sanitizeTaskBlocks(feedBlocks).filter((block) => block.kind !== "unknown");
					if (blocks.length === 0) {
						dropped.push("addCard:" + describeTaskBlockRequest(feedBlocks.find((block) => block !== null && typeof block === "object") ?? entry));
						continue;
					}
					if (blocks.length < feedBlocks.length) dropped.push(...feedBlocks.slice(blocks.length).map((block) => "addCard:" + describeTaskBlockRequest(block)));
					const title = typeof entry.title === "string" && entry.title.trim().length > 0 ? entry.title.trim().slice(0, TASK_TITLE_LIMIT) : "新卡片";
					ops.push({ op, title, blocks, x: typeof entry.x === "number" && Number.isFinite(entry.x) ? Math.round(entry.x) : void 0, y: typeof entry.y === "number" && Number.isFinite(entry.y) ? Math.round(entry.y) : void 0 });
					continue;
				}
				if (op === "addPlugin") {
					// A plugin item hosts another plugin's UI; the plugin reference is validated here.
					const ref = sanitizePluginRef({ id: entry.id ?? entry.pluginId ?? entry.plugin, title: entry.title, entry: entry.entry, height: entry.height, bare: entry.bare });
					if (ref === null) continue;
					ops.push({
						op,
						plugin: ref,
						bare: entry.bare === true,
						x: typeof entry.x === "number" && Number.isFinite(entry.x) ? Math.round(entry.x) : void 0,
						y: typeof entry.y === "number" && Number.isFinite(entry.y) ? Math.round(entry.y) : void 0,
						w: typeof entry.w === "number" && Number.isFinite(entry.w) ? Math.round(entry.w) : void 0,
						h: typeof entry.h === "number" && Number.isFinite(entry.h) ? Math.round(entry.h) : void 0
					});
					continue;
				}
				if (op === "rebuild") {
				...
				if (op === "link") {
					// An edge between two items: "these share their workspace context".
					const from = typeof entry.from === "string" ? entry.from.trim() : typeof entry.a === "string" ? entry.a.trim() : "";
					const to = typeof entry.to === "string" ? entry.to.trim() : typeof entry.b === "string" ? entry.b.trim() : "";
					if (from.length === 0 || to.length === 0 || from === to) {
						dropped.push("link:" + (from.length === 0 ? "(缺 from)" : to.length === 0 ? "(缺 to)" : "(两端相同)"));
						continue;
					}
					const kind = typeof entry.kind === "string" && TASK_LINK_KINDS.includes(entry.kind) ? entry.kind : "context";
					ops.push({ op, from, to, kind });
					continue;
				}
				if (op === "unlink") {
					const from = typeof entry.from === "string" ? entry.from.trim() : typeof entry.a === "string" ? entry.a.trim() : "";
					...
					if (id.length === 0 && (from.length === 0 || to.length === 0)) {
						dropped.push("unlink:(缺 id 或两端)");
						continue;
					}
					ops.push({ op, from, to, id });
					continue;
				}
				if (op === "update") {
					const id = typeof entry.id === "string" ? entry.id.trim() : "";
					const block = sanitizeTaskBlock(entry.block, 0);
					if (id.length === 0 || block === null) continue;
					ops.push({ op, id, block });
					continue;
				}
				if (op === "move") {
					const id = typeof entry.id === "string" ? entry.id.trim() : "";
					if (id.length === 0 || typeof entry.x !== "number" || typeof entry.y !== "number") continue;
					ops.push({ op, id, x: Math.round(entry.x), y: Math.round(entry.y) });
					continue;
				}
				if (op === "remove") {
					const id = typeof entry.id === "string" ? entry.id.trim() : "";
				...
				if (op === "note") {
					const note = typeof entry.text === "string" ? entry.text.trim().slice(0, 500) : "";
					if (note.length === 0) continue;
					ops.push({ op, note });
					continue;
				}
				dropped.push("op:" + (op.length > 0 ? op.slice(0, 24) : "(空)"));
			}
			if (ops.length === 0 && dropped.length === 0) return null;
			return {
				ops,
				dropped,
				canvas: typeof parsed.canvas === "string" && parsed.canvas.trim().length > 0 ? parsed.canvas.trim().slice(0, 40) : void 0
			};
		}
		/**
		 * Apply a `[canvas]` block written in the **main** conversation — the same op list the
		 * canvas' own chat box produces, so both routes can add, rebuild and rearrange items
		 * (hosted plugins included). The block may name its canvas (`"canvas": "名字或 id"`);
		 * without it the active canvas is used, and a name that does not exist yet is created,
		 * so "建个画布，把工作时序插件放上去" works in a single reply.
		 * @returns true when the block was applied (or there was nothing to apply).
		 */
		function applyConversationCanvasSpec(text) {
			const spec = parseCanvasSpec(text);
			if (spec === null) return true;
			const state = canvasSnapshot();
			const wanted = typeof spec.canvas === "string" ? spec.canvas : "";
			let canvas = wanted.length === 0
				? activeCanvas(state)
				: Object.values(state.canvases).find((item) => !isCanvasInstance(item.id) && (item.id === wanted || item.name === wanted)) ?? null;
			if (canvas === null) canvas = canvasCreate(wanted.length > 0 ? wanted : "对话画布");
			const applied = applyCanvasSpec(canvas.id, spec);
			if (applied.ok !== true) {
				publishTaskNotice("画布操作没有应用：" + String(applied.reason ?? "未知原因"));
				return false;
			}
			canvasLastAgentResult = applied;
			publishTaskNotice("画布「" + canvas.name + "」" + applied.message, canvas.id);
			return true;
		}
		/** Apply a parsed canvas operation list; every op goes through the normal canvas helpers. */
		function applyCanvasSpec(canvasId, spec) {
			if (spec === null || typeof spec !== "object" || !Array.isArray(spec.ops)) return { ok: false, reason: "invalid" };
			const state = canvasSnapshot();
			if (state.canvases[canvasId] === void 0) return { ok: false, reason: "no-canvas" };
			const counts = { add: 0, addCard: 0, addPlugin: 0, rebuild: 0, update: 0, move: 0, remove: 0, rename: 0, note: 0, link: 0, unlink: 0 };
			const notes = [];
			// Requests the parser (or this pass) could not carry out, reported by name below.
			const dropped = Array.isArray(spec.dropped) ? spec.dropped.slice(0, 20).map((entry) => String(entry).slice(0, 60)) : [];
			let skipped = 0;
			for (const op of spec.ops) {
				const canvas = canvasSnapshot().canvases[canvasId];
				if (canvas === void 0) break;
				if (op.op === "add") {
					// The spec normally comes from parseCanvasSpec; a hand-written one may be
					// malformed, and a malformed op must be skipped, never thrown.
					const block = op.block;
					if (block === null || typeof block !== "object" || typeof block.kind !== "string" || block.kind === "unknown") {
						dropped.push("add:" + describeTaskBlockRequest(block));
						skipped += 1;
						continue;
					}
					if (canvasAddControl(canvasId, block.kind, op.x, op.y, block) !== null) counts.add += 1;
					else { dropped.push("add:" + describeTaskBlockRequest(block)); skipped += 1; }
					continue;
				}
				if (op.op === "addCard") {
					if (!Array.isArray(op.blocks)) { skipped += 1; continue; }
					const blocks = op.blocks.filter((block) => block !== null && typeof block === "object" && block.kind !== "unknown");
					if (blocks.length === 0) {
						op.blocks.forEach((block) => dropped.push("addCard:" + describeTaskBlockRequest(block)));
						skipped += 1;
						continue;
					}
					if (blocks.length < op.blocks.length) op.blocks.filter((block) => block === null || typeof block !== "object" || block.kind === "unknown").forEach((block) => dropped.push("addCard:" + describeTaskBlockRequest(block)));
					const stack = Object.keys(canvas.cards).length % 6;
					const item = sanitizeCanvasCard({
						id: nextCanvasCardId(),
						title: op.title,
						x: (op.x ?? 80) + stack * 26,
						y: (op.y ?? 80) + stack * 26,
					...
					canvasUpsertCard(canvasId, item, true);
					counts.addCard += 1;
					continue;
				}
				// Canvas-level ops do not reference an item, so they come before the lookup.
				if (op.op === "addPlugin") {
					// Both shapes work: the parsed one (`plugin` object, what a [canvas] block
					// produces) and a raw one (`id`/`pluginId`), so programmatic callers need
					// not run the parser first.
					const source = op.plugin !== void 0 && typeof op.plugin === "object"
						? op.plugin
						: {
							id: typeof op.plugin === "string" ? op.plugin : (op.id ?? op.pluginId),
							title: op.title,
							entry: op.entry,
							height: op.height,
							bare: op.bare
						};
					const ref = sanitizePluginRef(source);
					const addedId = ref === null ? null : canvasAddPlugin(canvasId, ref, { x: op.x, y: op.y, w: op.w, h: op.h, bare: op.bare === true || ref.bare === true });
					if (addedId === null) skipped += 1;
					else counts.addPlugin += 1;
					continue;
				}
				if (op.op === "rename") {
					canvasRename(canvasId, op.name);
					counts.rename += 1;
					continue;
				}
				if (op.op === "note") {
				...
				// Links join two items, so they are resolved before the single-item lookup below.
				if (op.op === "link") {
					const from = canvas.cards[op.from] !== void 0 ? op.from : canvas.cards[op.to] !== void 0 ? op.to : "";
					const to = from === op.from ? op.to : op.from;
					if (from.length === 0 || to.length === 0 || canvasLinkAdd(canvasId, from, to, op.kind) === null) {
						skipped += 1;
						dropped.push("link:" + op.from + "→" + op.to);
						continue;
					}
					counts.link += 1;
					continue;
				...
				if (op.op === "unlink") {
					const linkId = op.id.length > 0 && canvas.links[op.id] !== void 0 ? op.id : "";
					if (canvasLinkRemove(canvasId, linkId, op.from, op.to)) counts.unlink += 1;
					else {
						skipped += 1;
						dropped.push("unlink:" + (linkId.length > 0 ? linkId : op.from + "→" + op.to));
					}
					continue;
				}
				const target = typeof op.id === "string" ? canvas.cards[op.id] : void 0;
				if (target === void 0) { skipped += 1; continue; }
				if (op.op === "update") {
					if (canvasUpdateControl(canvasId, target, op.block).ok === true) counts.update += 1;
					else skipped += 1;
					continue;
				}
				if (op.op === "move") {
					canvasUpsertCard(canvasId, { ...target, x: op.x, y: op.y }, true);
					canvasSyncBlockOrder(canvasId, canvasCardBinding(target)?.card ?? "");
					counts.move += 1;
					continue;
				}
				if (op.op === "rebuild") {
					if (target.plugin === void 0) { skipped += 1; continue; }
					canvasUpsertCard(canvasId, { ...target, plugin: { ...target.plugin, rev: nextPluginRevision(target.plugin) }, updatedAt: Date.now() }, true);
					counts.rebuild += 1;
					continue;
				}
				if (op.op === "remove") {
					canvasDeleteCard(canvasId, op.id);
					// A bound item also removes its control from the card it maps.
					canvasRemoveBoundBlock(target);
					counts.remove += 1;
				}
			}
			const changed = counts.add + counts.addCard + counts.addPlugin + counts.rebuild + counts.update + counts.move + counts.remove + counts.rename + counts.link + counts.unlink;
			const parts = [];
			if (counts.add > 0) parts.push("新增控件 " + counts.add);
			if (counts.addCard > 0) parts.push("新增卡片 " + counts.addCard);
			if (counts.addPlugin > 0) parts.push("新增插件 " + counts.addPlugin);
			if (counts.rebuild > 0) parts.push("重建插件 " + counts.rebuild);
			if (counts.link > 0) parts.push("连线 " + counts.link);
			if (counts.unlink > 0) parts.push("删除连线 " + counts.unlink);
			if (counts.update > 0) parts.push("修改控件 " + counts.update);
			if (counts.move > 0) parts.push("移动 " + counts.move);
			if (counts.remove > 0) parts.push("删除 " + counts.remove);
			if (counts.rename > 0) parts.push("重命名画布");
			const names = [...new Set(dropped)];
			if (names.length > 0) parts.push("跳过 " + names.length + "（" + names.slice(0, 4).join("、") + "）");
			const summary = parts.length > 0 ? parts.join(" · ") : "没有改动";
			// The dropped names matter: a conversation reads this message, and "跳过 1（看板）" is
			// what tells it to retry with a kind this client can actually build.
			const tail = names.length === 0 && skipped > 0 ? "（" + skipped + " 项无法应用，可能引用了不存在的 id）" : "";
			return {
				ok: true,
				changed,
				skipped,
			...
			const doSend = () => {
				...
				if (agentMode && taskCardAgentRefactor !== null) {
					...
					Promise.resolve(outcome).then((result) => {
						...
						if (result === null || result === void 0 || result.ok !== true) {
							setError("重构失败：" + (result !== null && result !== void 0 && result.message ? result.message : "未知原因"));
							setBusy(false);
							return;
						}
						setHint("卡片已更新，可继续描述进一步修改。");
						// The reply may also carry [page] actions: run them inside the embedded
						// web app through the local bridge, then report what actually happened.
						void runPageBlockFromReply(result.text, card.blocks).then((page) => {
							if (page === null) { setBusy(false); return; }
							appendLog({ key: "p" + Date.now(), label: "系统", text: "网页操作：" + page.summary });
							if (page.ok !== true) setError("网页操作未全部成功：" + page.summary);
							if (typeof page.spec.then === "string" && page.spec.then.length > 0) {
								// One bounded follow-up turn: hand the observation back to the agent.
								appendLog({ key: "t" + Date.now(), label: "系统", text: "继续：" + page.spec.then });
								setBusy(false);
								setText(page.spec.then + "\n（上一步的网页观察结果：" + page.summary + "）");
								setHint("已把网页结果填回输入框，回车继续这一轮操作。");
								return;
							}
							setBusy(false);
						}, () => setBusy(false));
		...
		/** Biggest model that is written into the canvas state as a data URL (~450 KB of bytes). */
		const CANVAS_MODEL_INLINE = 450000;
		/** A model block may reference a URL or carry one of these data URLs. */
		const CANVAS_MODEL_LIMIT = 1200000;
		...
		function sanitizeCanvasCard(raw, fallbackId) {
			...
			// A plugin item hosts another plugin's UI instead of carrying control blocks.
			const plugin = sanitizePluginRef(raw.plugin);
			if (plugin !== null) card.plugin = plugin;
			return card;
		}
		/**
		 * Link kinds a canvas edge may carry. `context` (the default, and the only one today)
		 * means: the cards it joins share their workspace context — starting a conversation from
		 * either of them hands the agent the other side's content as shared context.
		 */
		const TASK_LINK_KINDS = ["context"];
		const TASK_LINK_LABELS = { context: "工作区上下文共享" };
		/** Validate one link (an edge between two items of the same canvas). */
		function sanitizeCanvasLink(raw, fallbackId) {
			if (raw === null || typeof raw !== "object") return null;
			const from = typeof raw.from === "string" ? raw.from.trim() : "";
			const to = typeof raw.to === "string" ? raw.to.trim() : "";
			if (from.length === 0 || to.length === 0 || from === to) return null;
			if (!/^[A-Za-z0-9._:-]{1,64}$/.test(from) || !/^[A-Za-z0-9._:-]{1,64}$/.test(to)) return null;
			const rawId = typeof raw.id === "string" && /^[A-Za-z0-9._:-]{1,64}$/.test(raw.id) ? raw.id : fallbackId;
			const kind = typeof raw.kind === "string" && TASK_LINK_KINDS.includes(raw.kind) ? raw.kind : "context";
			const [left, right] = from < to ? [from, to] : [to, from];
			const link = {
				id: typeof rawId === "string" && rawId.length > 0 ? rawId : "lk-" + left + "-" + right,
				// Normalised pair: one edge between two items, whichever way it was drawn.
				from: left,
				to: right,
				kind,
				updatedAt: typeof raw.updatedAt === "number" && Number.isFinite(raw.updatedAt) ? raw.updatedAt : Date.now(),
			...
			if (typeof raw.label === "string" && raw.label.trim().length > 0) link.label = raw.label.trim().slice(0, 40);
			return link;
		}
		/** Validate a canvas' link table. */
		function sanitizeCanvasLinks(raw) {
			const links = {};
			if (raw === null || typeof raw !== "object") return links;
			for (const [key, value] of Object.entries(raw)) {
				const link = sanitizeCanvasLink(value, key);
				if (link !== null) links[link.id] = link;
			}
			return links;
		}
		...
		function sanitizeCanvas(raw) {
			...
			// A link only exists while both of its items do.
			for (const [key, value] of Object.entries(sanitizeCanvasLinks(raw.links))) {
				if (canvas.cards[value.from] === void 0 || canvas.cards[value.to] === void 0) continue;
				canvas.links[key] = value;
			}
			return canvas;
		}
		...
		function mergeCanvasState(local, remote) {
			...
			for (const [id, remoteCanvas] of Object.entries(remote.canvases)) {
				...
				const remoteLinks = remoteCanvas.links !== null && typeof remoteCanvas.links === "object" ? remoteCanvas.links : {};
				for (const [linkId, remoteLink] of Object.entries(remoteLinks)) {
					const mineLink = mine.links[linkId];
					if (remoteLink.deleted === true) {
						if (mineLink !== void 0 && mineLink.updatedAt <= remoteLink.updatedAt) delete mine.links[linkId];
						continue;
					}
					// Same last-writer-wins rule as cards: newest updatedAt, rev as the tie-break.
					if (mineLink === void 0 || mergeCanvasCard(mineLink, remoteLink) === remoteLink) mine.links[linkId] = remoteLink;
				}
				// A link whose items are gone (deleted on either side) is dropped, not kept dangling.
				for (const [linkId, link] of Object.entries(mine.links)) {
					if (mine.cards[link.from] === void 0 || mine.cards[link.to] === void 0) delete mine.links[linkId];
				}
			}
			return next;
		}
		...
		function taskEditorBlocks(content) {
			...
			for (const embed of Array.isArray(content.embeds) ? content.embeds.slice(0, TASK_EMBED_LIMIT) : []) {
				const embedBlock = sanitizeTaskBlock({ kind: "embed", url: embed.url, title: embed.title, height: embed.height, fill: embed.fill === true, control: embed.control === true }, blocks.length);
		...
		function applyCanvasRemoteOp(op) {
			...
			if (op.kind === "card.delete" && typeof op.canvasId === "string" && typeof op.cardId === "string") {
				...
				publishCanvasState({ ...state, canvases: { ...state.canvases, [canvas.id]: { ...canvas, cards, links: pruneCanvasLinks(canvas.links, cards), updatedAt: Date.now() } } }, true);
				return;
			}
			if (op.kind === "link.upsert" && typeof op.canvasId === "string" && op.link !== void 0) {
				const canvas = state.canvases[op.canvasId];
				const link = sanitizeCanvasLink(op.link, op.link !== null && typeof op.link === "object" ? op.link.id : void 0);
				if (canvas === void 0 || link === null || canvas.cards[link.from] === void 0 || canvas.cards[link.to] === void 0) return;
				const mine = canvas.links[link.id];
				if (mine !== void 0 && mergeCanvasCard(mine, link) === mine) return;
				publishCanvasState({ ...state, canvases: { ...state.canvases, [canvas.id]: { ...canvas, links: { ...canvas.links, [link.id]: link }, updatedAt: Date.now() } } }, true);
				return;
			}
			if (op.kind === "link.delete" && typeof op.canvasId === "string" && typeof op.linkId === "string") {
				const canvas = state.canvases[op.canvasId];
				if (canvas === void 0 || canvas.links[op.linkId] === void 0) return;
				const links = { ...canvas.links };
				delete links[op.linkId];
				publishCanvasState({ ...state, canvases: { ...state.canvases, [canvas.id]: { ...canvas, links, updatedAt: Date.now() } } }, true);
				return;
			}
		...
		/** Drop every link whose two items are not both present any more. */
		function pruneCanvasLinks(links, cards) {
			const out = {};
			for (const [id, link] of Object.entries(links !== null && typeof links === "object" ? links : {})) {
				if (cards[link.from] === void 0 || cards[link.to] === void 0) continue;
				out[id] = link;
			}
			return out;
		}
		...
		function canvasCreate(name) {
			const state = canvasSnapshot();
			const canvas = { id: nextCanvasId(), name: typeof name === "string" && name.trim().length > 0 ? name.trim().slice(0, 40) : "新画布", cards: {}, links: {}, updatedAt: Date.now() };
		...
		/** Canvas id bound to one board card ("instance" canvas created by maximizing the card). */		function canvasInstanceId(cardId) {
		...
		/** The card an instance canvas belongs to ("" for a normal canvas). */
		function instanceCardOf(canvasId) {
			if (typeof canvasId !== "string" || !canvasId.startsWith(CANVAS_INSTANCE_PREFIX)) return "";
			const cardId = canvasId.slice(CANVAS_INSTANCE_PREFIX.length);
			return cardId.length > 0 ? cardId : "";
		}
		...
		function estimateTaskBlockHeight(block) {
			...
			if (block.kind === "model") return (typeof block.height === "number" ? block.height : 360) + 76;
		...
		/**
		 * Take control of the first free spot below the items already on an instance canvas.
		 * Harvesting is order-independent: it lays out one column per tab in first-seen order.
		 */
		function canvasHarvestCollector() {
			const columns = { left: 0, right: 0 };
			const columnX = { left: 60, right: 60 + TASK_CANVAS_COLUMN + 28 };
			return (item) => {
				const column = columns.left <= columns.right ? "left" : "right";
				const y = columns[column];
				columns[column] = y + estimateTaskBlockHeight(effectiveTaskBlocks(item)[0] ?? {}) + 24;
				return { x: columnX[column], y };
			};
		}
		/**
		 * Adopt the free items an instance canvas gained — the controls the canvas chat (or the
		 * manual "+ 控件" button) created on it — onto the card the instance belongs to: each
		 * item becomes a control with a stable `bid` and is bound to it, so it survives the trip
		 * back to the board instead of vanishing with the plane.
		 *
		 * Normal canvases have no card to adopt into, and every item already bound to this card
		 * is left alone, so this is a no-op unless something genuinely new appeared.
		 *
		 * @param canvasId - the canvas whose new free items are adopted.
		 * @returns the adopted item ids in canvas order (empty when nothing was adopted).
		 */
		function canvasAdoptItems(canvasId) {
			const cardId = instanceCardOf(canvasId);
			...
			if (stored === undefined || !isUserTaskCard(stored)) return [];
			const free = [];
			for (const item of Object.values(canvas.cards)) {
				const binding = canvasCardBinding(item);
				if (binding !== null && binding.card === cardId) continue;
				// One canvas item maps exactly one control: anything else stays a plane-only card.
				if (binding !== null || effectiveTaskBlocks(item).length !== 1) continue;
				free.push(item);
			}
			if (free.length === 0) return [];
			free.sort((left, right) => (left.y - right.y) || (left.x - right.x));
			const spot = canvasHarvestCollector();
			const cardBlocks = effectiveTaskBlocks(stored);
			const adoptedBlocks = [];
			const cards = { ...canvas.cards };
			const adoptedIds = [];
			const boundIds = cardBlocks.filter((block) => typeof block.bid === "string").map((block) => block.bid);
			for (const item of free) {
				const block = { ...effectiveTaskBlocks(item)[0] };
				block.bid = typeof block.bid === "string" && block.bid.length > 0 && !boundIds.includes(block.bid) ? block.bid : nextTaskBlockId();
				boundIds.push(block.bid);
				const placed = spot(item);
				const next = sanitizeCanvasCard({ ...item, blocks: [block], title: taskBlockLabel(block), from: { card: cardId, bid: block.bid }, x: placed.x, y: placed.y, updatedAt: Date.now() }, item.id);
				if (next === null) continue;
				cards[next.id] = next;
				adoptedBlocks.push(block);
				adoptedIds.push(next.id);
			}
			if (adoptedIds.length === 0) return [];
			const order = []; // canvas arrangement first, then the controls that have no item
			const seen = new Set();
			for (const item of Object.values(cards)
				.filter((entry) => canvasCardBinding(entry)?.card === cardId)
				.sort((left, right) => (left.y - right.y) || (left.x - right.x))) {
				const block = cardBlocks.concat(adoptedBlocks).find((entry) => entry.bid === canvasCardBinding(item).bid);
				if (block !== undefined && !seen.has(block.bid)) order.push(block);
				if (block !== undefined) seen.add(block.bid);
			}
			for (const block of cardBlocks.concat(adoptedBlocks)) if (!seen.has(block.bid)) { order.push(block); seen.add(block.bid); }
			publishCanvasState(
				{ ...state, canvases: { ...state.canvases, [canvasId]: { ...canvas, cards, updatedAt: Date.now() } } },
				false,
			);
			for (const id of adoptedIds) canvasSync.sendOp({ kind: "card.upsert", canvasId, card: cards[id] });
			publishTaskCards(
				{ ...taskCardsStore.cards, [cardId]: { ...stored, blocks: sanitizeTaskBlocks(order), updatedAt: Date.now() } },
				taskCardsStore.order,
			);
			return adoptedIds;
		}
		/**
		 ...
		 */
		function canvasAddControl(canvasId, kind, x, y, blockOverride) {
			const state = canvasSnapshot();
			const canvas = state.canvases[canvasId];
			if (canvas === void 0) return null;
			const source = blockOverride !== null && typeof blockOverride === "object" ? blockOverride : taskBlockDefault(kind);
			const bid = typeof source.bid === "string" && /^[A-Za-z0-9._:-]{1,64}$/.test(source.bid) ? source.bid : nextTaskBlockId();
			const block = sanitizeTaskBlock({ ...source, bid }, 0);
		...
		/**
		 * Add one plugin's UI to a canvas as an item — the plugin counterpart of `+ 控件`.
		 * @returns the new item id, or null when the reference is unusable.
		 */
		function canvasAddPlugin(canvasId, plugin, options) {
			const state = canvasSnapshot();
			const canvas = state.canvases[canvasId];
			const ref = sanitizePluginRef(plugin);
			if (canvas === void 0 || ref === null) return null;
			const stack = Object.keys(canvas.cards).length % 6;
			const item = {
				id: nextCanvasCardId(),
				title: ref.title ?? ref.id,
				x: Math.round((typeof options?.x === "number" ? options.x : 80) + stack * 26),
				y: Math.round((typeof options?.y === "number" ? options.y : 80) + stack * 26),
				createdAt: Date.now(),
				updatedAt: Date.now(),
				// No revision on a fresh item: the bundle is loaded once per session and every
				// item of that plugin mounts into its own host. A rebuild sets a revision,
				// which is what forces the bundle to be materialized again.
				plugin: { ...ref },
				w: Math.min(Math.max(typeof options?.w === "number" ? options.w : 420, TASK_CARD_MIN_W), TASK_CARD_MAX_W),
				h: Math.min(Math.max(typeof options?.h === "number" ? options.h : (ref.height ?? 360), TASK_CARD_MIN_H), TASK_CARD_MAX_H)
			};
			if (options?.bare === true) item.bare = true;
			const clean = sanitizeCanvasCard(item, item.id);
			if (clean === null) return null;
			canvasUpsertCard(canvasId, clean, true);
			return clean.id;
		...
		/**
		 * Swap one control of a canvas item in place — what the image control calls when its
		 * source cannot be loaded and the user picks a local file instead. One-control items
		 * take the normal update path (title, binding and the card it mirrors all follow);
		 * a multi-control item is spliced, keeping every other block and its `bid`.
		 */
		function canvasReplaceBlock(canvasId, card, blockIndex, block) {
			if (card === null || typeof card !== "object") return null;
			const clean = sanitizeTaskBlock(block, 0);
			if (clean === null) return null;
			const blocks = effectiveTaskBlocks(card);
			...
			if (blocks.length === 1) {
				const outcome = canvasUpdateControl(canvasId, card, clean);
				return outcome.ok === true ? canvasSnapshot().canvases[canvasId]?.cards[card.id] ?? null : null;
			}
			const nextBlocks = blocks.slice();
			nextBlocks[blockIndex] = clean;
			const next = { ...card, blocks: nextBlocks, updatedAt: Date.now() };
			canvasUpsertCard(canvasId, next, true);
			canvasPushBinding(canvasId, next);
			return next;
		}
		...
		function canvasDeleteCard(canvasId, cardId) {
			...
			const cards = { ...canvas.cards };
			delete cards[cardId];
			// Links are edges between items: deleting an item removes the edges it carried.
			const links = pruneCanvasLinks(canvas.links, cards);
			publishCanvasState({ ...state, canvases: { ...state.canvases, [canvasId]: { ...canvas, cards, links, updatedAt: Date.now() } } }, false);
			for (const linkId of Object.keys(canvas.links ?? {})) {
				if (links[linkId] === void 0) canvasSync.sendOp({ kind: "link.delete", canvasId, linkId });
			}
			canvasSync.sendOp({ kind: "card.delete", canvasId, cardId });
		}
		/**
		 * Draw one link between two canvas items. The pair is normalised, so the same edge is
		 * never stored twice whichever end was picked first; `kind` defaults to `context`
		 * (工作区上下文共享) — what a conversation started from either card then shares.
		 */
		function canvasLinkAdd(canvasId, from, to, kind) {
			const state = canvasSnapshot();
			const canvas = state.canvases[canvasId];
			if (canvas === void 0) return null;
			const link = sanitizeCanvasLink({ from, to, kind: typeof kind === "string" ? kind : "context" }, void 0);
			if (link === null || canvas.cards[link.from] === void 0 || canvas.cards[link.to] === void 0) return null;
			const existing = canvas.links[link.id];
			link.updatedAt = Date.now();
			link.rev = canvasClientId + ":" + link.updatedAt.toString(36);
			if (existing !== void 0 && existing.kind === link.kind && existing.label === link.label) return existing;
			publishCanvasState({ ...state, canvases: { ...state.canvases, [canvasId]: { ...canvas, links: { ...canvas.links, [link.id]: link }, updatedAt: Date.now() } } }, false);
			canvasSync.sendOp({ kind: "link.upsert", canvasId, link });
			return link;
		}
		/** Remove one link by id (or by its two ends). */
		function canvasLinkRemove(canvasId, linkId, from, to) {
			const state = canvasSnapshot();
			const canvas = state.canvases[canvasId];
			if (canvas === void 0) return false;
			let id = typeof linkId === "string" && canvas.links[linkId] !== void 0 ? linkId : "";
			if (id.length === 0) {
				const probe = sanitizeCanvasLink({ from, to, kind: "context" }, void 0);
				if (probe !== null && canvas.links[probe.id] !== void 0) id = probe.id;
			}
			if (id.length === 0) return false;
			const links = { ...canvas.links };
			delete links[id];
			publishCanvasState({ ...state, canvases: { ...state.canvases, [canvasId]: { ...canvas, links, updatedAt: Date.now() } } }, false);
			canvasSync.sendOp({ kind: "link.delete", canvasId, linkId: id });
			return true;
		}
		/** Every link touching one item. */
		function canvasLinksOf(canvas, cardId) {
			if (canvas === null || canvas === void 0) return [];
			return Object.values(canvas.links ?? {}).filter((link) => link.from === cardId || link.to === cardId);
		}
		/**
		 * Plain-text content of a control list: what a linked card actually contributes when a
		 * conversation started on its neighbour inherits the shared workspace context.
		 */
		function taskBlocksAsContext(blocks, limit) {
			const parts = [];
			for (const block of Array.isArray(blocks) ? blocks : []) {
				const label = TASK_BLOCK_LABELS[block.kind] ?? block.kind;
				let text = "";
				if (block.kind === "stats" || block.kind === "bars") text = block.items.map((item) => item.label + "=" + item.value).join("；");
				else if (block.kind === "kv") text = block.rows.map((row) => row[0] + "=" + row[1]).join("；");
				else if (block.kind === "trend") text = block.items.map((item) => item.value).join(", ");
				else if (block.kind === "chips") text = block.items.join("、");
				else if (block.kind === "links") text = block.items.map((item) => item.label + " " + item.url).join("；");
				else if (block.kind === "checklist") text = block.items.map((item) => item.label).join(" / ");
				else if (block.kind === "table") text = [block.columns.join(" | "), ...block.rows.map((row) => row.join(" | "))].join(" / ");
				else if (block.kind === "progress") text = block.label + " " + block.value + "/" + block.max + (block.unit.length > 0 ? block.unit : "");
				else if (block.kind === "countdown") text = block.label + " 截止 " + new Date(block.until).toISOString().slice(0, 16).replace("T", " ");
				else if (block.kind === "toggle") text = block.label + "=" + (block.value === true ? "开" : "关");
				else if (block.kind === "counter") text = block.label;
				else if (block.kind === "button") text = block.label;
				else if (block.kind === "image") text = block.caption.length > 0 ? block.caption : "图片";
				else if (block.kind === "embed") text = block.url;
				else if (typeof block.text === "string") text = block.text;
				if (typeof text !== "string" || text.trim().length === 0) continue;
				parts.push(label + "：" + text.replace(/\s+/g, " ").trim().slice(0, 400));
			}
			const bound = typeof limit === "number" && limit > 0 ? limit : 1400;
			const joined = parts.join("；");
			return joined.length > bound ? joined.slice(0, bound) + "…" : joined;
		}
		/**
		 * The cards connected to one card/item, with the content they contribute. Looks the id
		 * up both as a canvas item and as the board card those items mirror, so the same lookup
		 * answers for a canvas card chat and for a board card chat.
		 */
		function linkedCardsOf(cardId, canvasId) {
			if (typeof cardId !== "string" || cardId.length === 0) return [];
			const state = canvasSnapshot();
			const found = [];
			const seen = new Set();
			for (const canvas of Object.values(state.canvases)) {
				const explicit = typeof canvasId === "string" && canvasId.length > 0 && canvas.id === canvasId;
				const item = canvas.cards[cardId] !== void 0 && (canvasId === void 0 || canvasId === null || explicit) ? canvas.cards[cardId] : void 0;
				const mirrors = item !== void 0 ? [] : Object.values(canvas.cards).filter((entry) => entry.from !== void 0 && entry.from.card === cardId);
				// Either the item itself is the node, or every item mirroring this board card is.
				const nodes = item !== void 0 ? [item] : mirrors;
				for (const node of nodes) {
					const links = Object.values(canvas.links ?? {}).filter((link) => link.from === node.id || link.to === node.id);
					for (const link of links) {
						const peerId = link.from === node.id ? link.to : link.from;
						const peer = canvas.cards[peerId];
						if (peer === void 0) continue;
						// A mirror of the same board card is not "another" card.
						if (peer.from !== void 0 && peer.from.card === cardId) continue;
						if (seen.has(canvas.id + "/" + peer.id)) continue;
						seen.add(canvas.id + "/" + peer.id);
						found.push({
							id: peer.id,
							canvasId: canvas.id,
							canvasName: isCanvasInstance(canvas.id) ? "" : canvas.name,
							kind: link.kind,
							title: peer.title.length > 0 ? peer.title : "未命名卡片",
							blocks: effectiveTaskBlocks(peer),
							plugin: peer.plugin !== void 0 ? peer.plugin.id : ""
						});
					}
				}
			}
		...
		/**
		 * Prompt section for the cards connected to one card: this is what "发起会话时工作区
		 * 上下文共享" means in practice — the agent gets the neighbours' content as context.
		 */
		function composeLinkPromptSection(cardId, canvasId) {
			const peers = linkedCardsOf(cardId, canvasId);
			if (peers.length === 0) return "";
			const lines = peers.slice(0, 8).map((peer) => {
				const content = peer.plugin.length > 0 ? "（插件项：" + peer.plugin + "）" : taskBlocksAsContext(peer.blocks);
				return "- 「" + peer.title + "」" + (peer.canvasName.length > 0 ? "（画布：" + peer.canvasName + "）" : "") + "：" + (content.length > 0 ? content : "（空卡片）");
			});
			return [
				"共享的工作区上下文（这些卡片与当前卡片有连线，连线的初始含义就是「发起会话时工作区上下文共享」）：",
				...lines,
				"请把它们当成同一工作区里的上下文一起考虑：需要时可以直接引用、对比或统一它们的内容；不要把连线本身当成要修改的控件。"
			].join("\n");
		}
		/** Prompt section describing every connected group of a canvas (canvas chat). */
		function composeCanvasLinkSection(canvas) {
			if (canvas === null || canvas === void 0) return "";
			const clusters = canvasLinkClusters(canvas);
			if (clusters.length === 0) return "";
			const lines = [];
			for (const cluster of clusters) {
				const titles = cluster.ids.map((id) => {
					const item = canvas.cards[id];
					return "「" + (item !== void 0 && item.title.length > 0 ? item.title : id) + "」";
				});
				lines.push("- " + titles.join(" ↔ ") + "（" + (TASK_LINK_LABELS[cluster.links[0]?.kind ?? "context"] ?? "连线") + "）：这几项共享工作区上下文——从其中任一项发起会话时，它们的内容会一起带给 agent。");
			}
			return [
				"画布上已有的连线（虚线框就是连线组）：",
				...lines,
				"用户要求把两项关联/共享上下文时，用 { \"op\": \"link\", \"from\": \"项 id\", \"to\": \"项 id\" }（kind 默认 context）；取消用 { \"op\": \"unlink\", \"from\": \"项 id\", \"to\": \"项 id\" }。"
			].join("\n");
		}
		/** Connected groups of a canvas: one entry per cluster of two or more linked items. */
		function canvasLinkClusters(canvas) {
			const ids = Object.keys(canvas !== null && canvas !== void 0 ? canvas.cards ?? {} : {});
			const parent = new Map(ids.map((id) => [id, id]));
			const find = (id) => {
				let root = id;
				while (parent.get(root) !== root) root = parent.get(root);
				let walk = id;
				while (parent.get(walk) !== root) { const next = parent.get(walk); parent.set(walk, root); walk = next; }
				return root;
			};
			const links = Object.values(canvas !== null && canvas !== void 0 ? canvas.links ?? {} : {});
			for (const link of links) {
				if (!parent.has(link.from) || !parent.has(link.to)) continue;
				const left = find(link.from);
				const right = find(link.to);
			...
			return [...groups.values()]
				...
				.map((members) => ({ ids: members, links: links.filter((link) => members.includes(link.from) && members.includes(link.to)) }));
		}
		...
		/**
		 * Bridge settings popover: point the workbench at a local page bridge so the agent
		 * can operate apps that are embedded with 「受控」 enabled.
		 */
		function TaskBridgePanel(props) {
			const { onClose } = props;
			const [settings, setSettings] = (0, react.useState)(() => readTaskBridgeSettings());
			const [status, setStatus] = (0, react.useState)("");
			const [busy, setBusy] = (0, react.useState)(false);
			const [targets, setTargets] = (0, react.useState)(() => taskBridgePages.size);
			(0, react.useEffect)(() => subscribeTaskBridge(() => setTargets(taskBridgePages.size)), []);
			const update = (patch) => setSettings(saveTaskBridgeSettings({ ...settings, ...patch }));
			const test = () => {
				setBusy(true);
				void pingTaskBridge(settings).then((outcome) => {
			...
			return (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-tc-chatOverlay",
				children: (0, react_jsx_runtime.jsxs)("div", {
					className: "dsh-tc-chat dsh-tc-bridgePanel",
					children: [
						(0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-tc-chatHead",
							children: [
								(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-chatTitle", children: "网页桥接 · 让 agent 操作内嵌应用" }),
								(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-chatSub", children: "桥接是一个本机小服务：它代理目标网页并注入受控脚本，agent 就能在里面点击 / 输入 / 读取。卡片上的内嵌应用勾选「受控」后才会走桥接。" })
							]
						}),
						(0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-tc-chatBody",
							children: [
								(0, react_jsx_runtime.jsxs)("div", { className: "dsh-tc-field", children: [
									(0, react_jsx_runtime.jsx)("label", { className: "dsh-tc-fieldLabel", children: "桥接地址（默认 http://127.0.0.1:8790）" }),
									(0, react_jsx_runtime.jsx)("input", { className: "dsh-tc-in", value: settings.url, placeholder: "http://127.0.0.1:8790", onChange: (event) => update({ url: event.target.value }) })
								] }),
								(0, react_jsx_runtime.jsxs)("div", { className: "dsh-tc-field", children: [
									(0, react_jsx_runtime.jsx)("label", { className: "dsh-tc-fieldLabel", children: "令牌（启动桥接时若加了 --token 就填这里）" }),
									(0, react_jsx_runtime.jsx)("input", { className: "dsh-tc-in", value: settings.token, placeholder: "可留空", onChange: (event) => update({ token: event.target.value }) })
								] }),
								(0, react_jsx_runtime.jsxs)("div", { className: "dsh-tc-field", children: [
									(0, react_jsx_runtime.jsxs)("label", { className: "dsh-tc-fieldLabel", children: [
										(0, react_jsx_runtime.jsx)("input", { type: "checkbox", checked: settings.enabled, onChange: (event) => update({ enabled: event.target.checked }) }),
										" 允许 agent 操作网页（关闭后 [page] 动作不会执行）"
									] }),
									(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-fieldHint", children: "启动桥接：node bridge/server.mjs --port 8790（可加 --token 与 --allow 限制目标站）" })
								] }),
								status.length > 0 && (0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-fieldHint", children: status }),
								(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-fieldHint", children: "当前已登记页面：" + targets })
							]
						}),
						(0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-tc-chatFoot",
							children: [
								(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-btn", disabled: busy, onClick: test, children: busy ? "测试中…" : "测试连接" }),
								(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-btnPrimary", onClick: onClose, children: "完成" })
							]
						})
					]
				})
		...
		/**
		 * The canvas' own chat dock: describe a change in language and the canvas is edited —
		 * items added, moved, rewritten or removed — through the temporary agent session and
		 * the [canvas] operation list.
		 */
		function CanvasChatDock(props) {
			const { canvas, canvasId, onApply, adoptOnApply = false } = props;
			const [text, setText] = (0, react.useState)("");
			const [busy, setBusy] = (0, react.useState)(false);
			const [open, setOpen] = (0, react.useState)(false);
			const [mini, setMini] = (0, react.useState)(false);
			const [log, setLog] = (0, react.useState)([]);
			const [error, setError] = (0, react.useState)(null);
			const bodyRef = (0, react.useRef)(null);
			const append = (label, message) => setLog((previous) => [...previous.slice(-24), { key: label + Date.now() + Math.random(), label, text: message }]);
			(0, react.useEffect)(() => {
				const body = bodyRef.current;
				if (body !== null && typeof body.scrollTop === "number") body.scrollTop = body.scrollHeight;
			}, [log, open]);
			const send = () => {
				const instruction = text.trim();
				if (instruction.length === 0 || busy) return;
				setError(null);
				setOpen(true);
				setBusy(true);
				append("我", instruction);
				setText("");
				const prompt = { canvasId, canvas, instruction };
				if (taskCardAgentRefactor === null) {
					append("系统", "agent 服务不可用：无法修改画布");
					setBusy(false);
					return;
				}
				const outcome = taskCardAgentRefactor(TASK_CANVAS_AGENT_TASK, prompt, (event) => {
					if (event.phase === "started") append("系统", "已创建临时 agent 会话，正在按你的描述修改画布…");
					else if (event.phase === "assistant") append("AI", event.text.length > 1500 ? event.text.slice(0, 1500) + "…" : event.text);
				});
				Promise.resolve(outcome).then((result) => {
					if (result === null || result === void 0 || result.ok !== true) {
						setError(result !== null && result !== void 0 && result.message ? result.message : "修改失败");
						setBusy(false);
						return;
					}
					const applied = typeof onApply === "function" ? onApply() : null;
					// A reference canvas is bound to a card: controls the chat creates here are
					// adopted onto that card (and bound to their new items), so they survive the
					// trip back to the board instead of vanishing with the plane.
					const adopted = adoptOnApply ? canvasAdoptItems(canvasId) : [];
					if (adopted.length > 0) append("系统", "已把新控件挂回卡片：" + adopted.length + " 个（返回任务台即可看到）");
					if (applied !== null && applied !== void 0 && typeof applied.message === "string") append("系统", applied.message);
					if (applied !== null && applied !== void 0 && Array.isArray(applied.notes)) for (const note of applied.notes) append("AI", note);
					// Page actions in the same reply run against every controlled embed on the canvas.
					const liveCanvas = canvasSnapshot().canvases[canvasId] ?? null;
					const lists = liveCanvas === null ? [] : Object.values(liveCanvas.cards).map((item) => effectiveTaskBlocks(item));
					void runPageBlockFromReply(result.text, lists.flat()).then((page) => {
						setBusy(false);
						if (page === null) return;
						append("系统", "网页操作：" + page.summary);
						if (page.ok !== true) setError("网页操作未全部成功：" + page.summary);
						if (typeof page.spec.then === "string" && page.spec.then.length > 0) {
							append("系统", "继续：" + page.spec.then);
							setText(page.spec.then + "\n（上一步的网页观察结果：" + page.summary + "）");
						}
					}, () => setBusy(false));
				}, () => {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-tc-canvasChat" + (open ? " dsh-tc-canvasChatOpen" : "") + (mini ? " dsh-tc-canvasChatMini" : ""),
				children: mini
					? [(0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: "dsh-tc-chatPill",
						title: "展开画布对话（用语言修改这块画布）",
						onClick: () => setMini(false),
						children: "💬 改画布" + (log.length > 0 ? " · " + log.length : "")
					}, "pill")]
					: [
						(0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-tc-canvasChatHead",
							children: [
								(0, react_jsx_runtime.jsx)("span", { children: "画布对话" }),
								(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-boardSpacer" }),
								(0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: "dsh-tc-cardIconBtn",
									title: open ? "收起对话记录" : "展开对话记录",
									disabled: log.length === 0,
									onClick: () => setOpen((value) => !value),
									children: (0, react_jsx_runtime.jsx)("span", { children: open ? "▾" : "▴" })
								}),
								(0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: "dsh-tc-cardIconBtn",
						...
						open && (0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-tc-canvasChatBody",
							ref: bodyRef,
							children: [
								log.length === 0 && (0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-chatHint", children: adoptOnApply ? "说说你想怎么改这张卡片的画布，例如「把本地的清单和进度放到左上角，再加一个倒计时到明天 18 点」——新加的东西会挂回卡片。" : "说说你想怎么改这块画布，例如「把本地的清单和进度放到左上角，再加一个倒计时到明天 18 点」。" }),
								log.map((entry) => (0, react_jsx_runtime.jsxs)("div", {
									className: "dsh-tc-chatRow",
									children: [
										(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-chatRole", children: entry.label }),
										(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-chatText", children: entry.text })
									]
								}, entry.key)),
								busy && (0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-chatHint", children: "修改中…" }),
								error !== null && (0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-editErr", children: error })
							]
						}),
						(0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-tc-canvasChatBar",
							children: [
								(0, react_jsx_runtime.jsx)("input", {
									className: "dsh-tc-in dsh-tc-canvasChatInput",
									value: text,
									placeholder: "一句话改画布…",
									disabled: busy,
									onChange: (event) => setText(event.target.value),
		...
		function CanvasControlView(props) {
			const { card, onMove, onResize, onEdit, onChat, onDelete, onWidgetState, onBlockResize, onBlockReplace, onDuplicate, onBoard, onToggleBare, canvasId, onRebuild, onLink, linkCount } = props;
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							card.plugin !== void 0 && onRebuild !== void 0 && (0, react_jsx_runtime.jsx)(CanvasPluginTools, { plugin: card.plugin, onRebuild, buttonClass: "dsh-tc-bareBtn" }),
							(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-bareSpacer" }),
							onLink !== void 0 && (0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-bareBtn" + (linkCount > 0 ? " dsh-tc-iconOn" : ""), title: linkCount > 0 ? "已连线 " + linkCount + " 条（点这里再连一条）" : "与另一项连线：初始含义是工作区上下文共享", onPointerDown: (event) => event.stopPropagation(), onClick: () => onLink(card.id), children: (0, react_jsx_runtime.jsx)("span", { children: "🔗" }) }),
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						onPointerUp: end,
						onPointerCancel: end,
						children: card.plugin !== void 0
							? (0, react_jsx_runtime.jsx)(PluginHostView, { plugin: card.plugin, dark: typeof document !== "undefined" && document.body !== void 0 && document.body.hasAttribute("data-ds-dark-theme") })
							: (0, react_jsx_runtime.jsx)(TaskUserBodyView, {
								card,
								cardId: card.id,
								onWidgetState,
								onBlockResize: onBlockResize !== void 0 ? (id, index, height) => onBlockResize(id, index, height) : void 0,
								onBlockReplace: onBlockReplace !== void 0 ? (id, index, block) => onBlockReplace(id, index, block) : void 0
							})
					}),
					(0, react_jsx_runtime.jsx)("div", {
		...
		function CanvasCardView(props) {
			const { card, scale, onMove, onResize, onEdit, onChat, onDelete, onWidgetState, onBlockResize, onBlockReplace, onFull, onDuplicate, onBoard, onToggleBare, onRebuild, onLink, linkCount } = props;
			const ref = (0, react.useRef)(null);
			const gesture = (0, react.useRef)(null);
			const frame = (0, react.useRef)(0);
			const begin = (mode, event) => {
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							card.plugin !== void 0 && onRebuild !== void 0 && (0, react_jsx_runtime.jsx)(CanvasPluginTools, { plugin: card.plugin, onRebuild, buttonClass: "dsh-tc-cardIconBtn" }),
							onLink !== void 0 && (0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-cardIconBtn" + (linkCount > 0 ? " dsh-tc-iconOn" : ""), title: linkCount > 0 ? "已连线 " + linkCount + " 条（点这里再连一条）" : "与另一张卡片连线：初始含义是工作区上下文共享", onPointerDown: (event) => event.stopPropagation(), onClick: () => onLink(card.id), children: (0, react_jsx_runtime.jsx)("span", { children: "🔗" }) }),
					...
					(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-cardBody", children: card.plugin !== void 0
						? (0, react_jsx_runtime.jsx)(PluginHostView, { plugin: card.plugin, dark: document !== void 0 && document.body !== void 0 && document.body.hasAttribute("data-ds-dark-theme") })
						: (0, react_jsx_runtime.jsx)(TaskUserBodyView, { card, cardId: card.id, onWidgetState, onBlockResize, onBlockReplace }) }),
					(0, react_jsx_runtime.jsx)("div", {
						className: "dsh-tc-cardResize",
						title: "拖动调整卡片大小",
						onPointerDown: (event) => begin("resize", event),
		...
		/**
		 * The link layer of a canvas: one curve per link, one dashed frame per connected group,
		 * and a clickable chip per link so the edge can be removed where it is seen.
		 *
		 * A link's initial meaning is **workspace-context sharing**: a conversation started from
		 * any card of a group is handed the other cards' content as shared context, which is why
		 * the frame is drawn around the whole group rather than around a single card.
		 */
		function CanvasLinkLayer(props) {
			const { links, clusters, boxes, scale, onRemove, onExplain } = props;
			const hub = (id) => (boxes[id] !== void 0 && boxes[id].w > 0 ? { cx: boxes[id].x + boxes[id].w / 2, cy: boxes[id].y + boxes[id].h / 2, box: boxes[id] } : null);
			const curveOf = (link) => {
				const from = hub(link.from);
				const to = hub(link.to);
				if (from === null || to === null) return null;
				// Leave/enter on the sides that face the other item, so curves read like a flow.
				const forward = to.cx >= from.cx;
				const x1 = forward ? from.box.x + from.box.w : from.box.x;
				const x2 = forward ? to.box.x : to.box.x + to.box.w;
				const y1 = from.cy;
				const y2 = to.cy;
				const bend = Math.max(36, Math.abs(x2 - x1) / 2.4);
				const path = "M " + x1 + " " + y1 + " C " + (forward ? x1 + bend : x1 - bend) + " " + y1 + ", " + (forward ? x2 - bend : x2 + bend) + " " + y2 + ", " + x2 + " " + y2;
				return { path, midX: (x1 + x2) / 2, midY: (y1 + y2) / 2, from, to };
			};
			const drawn = links.map((link) => ({ link, curve: curveOf(link) })).filter((entry) => entry.curve !== null);
			const frames = clusters.map((cluster) => {
				...
				for (const id of cluster.ids) {
					const box = boxes[id];
					if (box === void 0 || box.w === 0) continue;
				...
				const pad = 18;
				return { key: cluster.ids.slice().sort().join("|"), x: left - pad, y: top - pad, w: right - left + pad * 2, h: bottom - top + pad * 2, count: cluster.ids.length, kind: cluster.links[0]?.kind ?? "context" };
			}).filter((frame) => frame !== null);
			if (drawn.length === 0 && frames.length === 0) return null;
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				// A 1×1 SVG with visible overflow: coordinates stay the plane's own coordinates,
				// so the curves pan and zoom with the items without any extra transform maths.
				(0, react_jsx_runtime.jsx)("svg", {
					className: "dsh-tc-linkLayer",
					width: "1",
					height: "1",
					viewBox: "0 0 1 1",
					children: drawn.map((entry) => (0, react_jsx_runtime.jsx)("path", {
						className: "dsh-tc-linkEdge",
						"data-link": entry.link.id,
						d: entry.curve.path
					}, "edge:" + entry.link.id))
				}),
				frames.map((frame) => (0, react_jsx_runtime.jsx)("div", {
					className: "dsh-tc-linkFrame",
					"data-count": frame.count,
					style: { left: frame.x, top: frame.y, width: frame.w, height: frame.h },
					children: (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: "dsh-tc-linkFrameLabel",
						title: "说明这几张卡片共享什么；点连线上的标签可删除连线",
						onClick: onExplain,
						children: "⚯ " + (TASK_LINK_LABELS[frame.kind] ?? "连线") + " · " + frame.count + " 项"
					})
				}, "frame:" + frame.key)),
				drawn.map((entry) => (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					className: "dsh-tc-linkChip",
					"data-link": entry.link.id,
					style: { left: entry.curve.midX, top: entry.curve.midY, "--dsh-tc-linkK": scale >= 0.6 ? 1 : 0.75 },
					title: "点击删除这条连线（" + (TASK_LINK_LABELS[entry.link.kind] ?? "连线") + "）",
					onClick: () => onRemove(entry.link.id),
					children: ["⚯ ", TASK_LINK_LABELS[entry.link.kind] ?? "连线", " ×"]
				}, "chip:" + entry.link.id))
			] });
		}
		/**
		...
		function TaskCanvasView(props) {
			...
			const [pluginRows, setPluginRows] = (0, react.useState)(() => listPluginCandidates());
			const [dynamicRows, setDynamicRows] = (0, react.useState)([]);
			(0, react.useEffect)(() => {
				if (!pluginOpen) return void 0;
				setPluginRows(listPluginCandidates());
				let alive = true;
				void listDynamicPlugins().then((rows) => { if (alive) setDynamicRows(rows); }, () => {});
				const off = subscribeTaskPlugin(() => { if (alive) setPluginRows(listPluginCandidates()); });
				return () => { alive = false; off(); };
			}, [pluginOpen]);
			...
			// Linking: `linkFrom` is the item the user picked, `linkPoint` the rubber-band end.
			const [linkFrom, setLinkFrom] = (0, react.useState)(null);
			const [linkPoint, setLinkPoint] = (0, react.useState)(null);
			// Measured item geometry, what the link layer (curves + dashed frames) is drawn from.
			const [boxes, setBoxes] = (0, react.useState)({});
			...
			/**
			 * Keep the link layer's geometry in step with the plane. Items are absolutely
			 * positioned inside the plane, so their `offsetLeft/Top` *are* plane coordinates
			 * (a CSS transform during a drag does not move them). The state is only replaced
			 * when something actually moved, so this cannot loop.
			 */
			(0, react.useEffect)(() => {
				const plane = planeRef.current;
				if (plane === null) return;
				const next = {};
				for (const node of plane.querySelectorAll("[data-id]")) {
				...
				setBoxes((previous) => {
					...
					const same = keys.length === Object.keys(previous).length && keys.every((key) => previous[key] !== void 0 &&
						previous[key].x === next[key].x && previous[key].y === next[key].y &&
						previous[key].w === next[key].w && previous[key].h === next[key].h);
					return same ? previous : next;
				});
			});
			/** Compute plane coordinates from a pointer event inside the scroll area. */
			const planePointOf = (event) => {
				const scroll = scrollRef.current;
				if (scroll === null) return null;
				const rect = scroll.getBoundingClientRect();
				const base = currentView();
				const k = base.k > 0 ? base.k : 1;
				return { x: (event.clientX - rect.left - base.x) / k, y: (event.clientY - rect.top - base.y) / k };
			};
			/** The item under one plane point (topmost first: later items paint above). */
			const itemAtPoint = (point) => {
				if (point === null || canvas === null) return null;
				const list = Object.values(canvas.cards);
				for (let index = list.length - 1; index >= 0; index--) {
					const box = boxes[list[index].id];
					if (box === void 0 || box.w === 0) continue;
					if (point.x >= box.x && point.x <= box.x + box.w && point.y >= box.y && point.y <= box.y + box.h) return list[index];
				}
				return null;
			};
			...
			// While a link is being drawn, Escape cancels the link instead of closing the canvas.
			const linkPhaseRef = (0, react.useRef)(null);
			linkPhaseRef.current = linkFrom === null ? null : () => { setLinkFrom(null); setLinkPoint(null); setHint("已取消连线"); };
			...
			(0, react.useEffect)(() => {
				...
				const onKey = (event) => {
					if (event.key !== "Escape") return;
					if (linkPhaseRef.current !== null) {
						event.stopPropagation();
						event.preventDefault();
						linkPhaseRef.current();
						return;
					}
			...
			/**
			 * Drop a 3D file on the plane: put a viewer item where it landed, or hand the model to
			 * the viewer item it was dropped on. A model small enough is written into the canvas
			 * state; a bigger one is displayed from memory for this session only (the canvas is
			 * synced as JSON, and a multi-megabyte model has no business in it).
			 */
			const addModelFile = (file, target, place) => {
				if (canvasId === null) return;
				readTaskModelFile(file, (buffer, name, dataUrl) => {
					if (buffer === null) { setHint(name ?? "模型无法读取"); return; }
					const inline = typeof dataUrl === "string" && dataUrl.length > 0 && dataUrl.length <= CANVAS_MODEL_LIMIT;
					const targetBlock = target !== null && target !== void 0 && (target.blocks ?? [])[0]?.kind === "model" ? target.blocks[0] : null;
					if (targetBlock !== null && target !== null && target !== void 0) {
						const bid = typeof targetBlock.bid === "string" ? targetBlock.bid : nextTaskBlockId();
						if (!inline) pendingTaskModels.set(bid, { buffer, name });
						canvasUpsertCard(canvasId, { ...target, title: name.length > 0 ? name.slice(0, TASK_TITLE_LIMIT) : target.title, blocks: [{ ...targetBlock, bid, src: inline ? dataUrl : "", name }] }, inline && dataUrl.length <= CANVAS_SYNC_LIMIT);
						setHint(inline ? "已载入模型：" + name : "已载入模型（较大，仅在本地这次会话显示）：" + name);
						return;
					}
					const bid = nextTaskBlockId();
					if (!inline) pendingTaskModels.set(bid, { buffer, name });
					const item = {
						id: nextCanvasCardId(),
						title: name.length > 0 ? name.slice(0, TASK_TITLE_LIMIT) : "3D 模型",
						bare: true,
						x: place.x,
						y: place.y,
						updatedAt: Date.now(),
						blocks: [{ kind: "model", src: inline ? dataUrl : "", name, height: 360, wireframe: false, grid: true, background: "dark", bid }]
					};
					canvasUpsertCard(canvasId, item, inline && dataUrl.length <= CANVAS_SYNC_LIMIT);
					setHint(inline ? "已添加 3D 模型控件" : "已添加 3D 模型控件（较大，仅在本地这次会话显示）");
				});
			};
			const onPaste = (event) => {
			...
			const onDrop = (event) => {
				...
				const image = files.find((item) => typeof item.type === "string" && item.type.startsWith("image/"));
				if (image !== void 0) {
					event.preventDefault();
					addImageFile(image, event.clientX, event.clientY);
					return;
				}
				// A 3D file dropped on the plane becomes (or replaces) a viewer item.
				const model = files.find((item) => isTaskModelFile(item));
				if (model === undefined || canvas === null || canvasId === null) return;
				event.preventDefault();
				const point = planePointOf(event);
				const target = point === null ? null : itemAtPoint(point);
				const place = point === null ? { x: 80, y: 80 } : { x: Math.round(point.x), y: Math.round(point.y) };
				addModelFile(model, canvasId, target, place);
			};
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							canvas !== null && (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dsh-tc-btn" + (pluginOpen ? " dsh-tc-iconOn" : ""),
								title: "把任意插件的界面放到画布上显示（可重建）",
								onClick: () => { setPluginOpen((value) => !value); setAddOpen(false); },
								children: "🧩 插件"
							}),
							canvas !== null && (0, react_jsx_runtime.jsx)("button", {
								type: "button",
					...
					pluginOpen && canvas !== null && (0, react_jsx_runtime.jsxs)("div", {
						className: "dsh-tc-addPanel",
						children: [
							(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-addHint", children: "点一个插件，把它的界面放到画布上（可拖动 / 缩放 / 重建）：" }),
							pluginRows.length === 0 && (0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-fieldHint", children: "启动清单里没有可用的客户端插件包。" }),
							pluginRows.slice(0, 24).map((row) => (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dsh-tc-addChip",
								title: row.id + (row.embeddable ? "（已验证可挂载）" : "（未验证，可能没有嵌入入口）"),
								onClick: () => {
									if (canvasId === null) return;
									const scroll = scrollRef.current;
									const base = currentView();
									const rect = scroll !== null ? scroll.getBoundingClientRect() : null;
									const centerX = rect !== null ? (rect.width / 2 - base.x) / base.k - 200 : 80;
									const centerY = rect !== null ? (rect.height / 2 - base.y) / base.k - 120 : 80;
									const added = canvasAddPlugin(canvasId, { id: row.id }, { x: centerX, y: centerY });
									setHint(added === null ? "无法添加这个插件" : "已添加插件项：" + row.id + "（失败可点 ⟳ 重建，或让它暴露嵌入入口）");
								},
								children: (row.embeddable ? "✓ " : "") + row.id.replace(/^dsh-plugin-/, "").replace(/^@[^/]+\//, "")
							}, "bundle:" + row.id)),
							dynamicRows.map((row) => (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dsh-tc-addChip",
								title: "动态插件 " + row.id + (row.running ? "（运行中）" : "（已停止）"),
								onClick: () => {
									if (canvasId === null) return;
									const added = canvasAddPlugin(canvasId, { id: row.id, title: row.name.length > 0 ? row.name : row.id }, { x: 120, y: 120 });
									setHint(added === null ? "无法添加这个插件" : "已添加动态插件项：" + row.id);
								},
								children: "◇ " + (row.name.length > 0 ? row.name : row.id)
							}, "dynamic:" + row.id)),
							(0, react_jsx_runtime.jsx)("span", { className: "dsh-tc-fieldHint", children: "插件需要发布嵌入入口才能显示内容：globalThis.__DSH_<ID>__.mountEmbedded(host, api)；没有入口时画布上会显示说明与示例。" })
						]
					}),
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: (0, react_jsx_runtime.jsx)("div", {
							...
							// Links live under the items: the curves, one dashed frame per connected
							// group, and a clickable chip per link (click = remove that link).
							(0, react_jsx_runtime.jsx)(CanvasLinkLayer, {
								boxes,
								links: canvas === null ? [] : Object.values(canvas.links ?? {}),
								clusters: canvas === null ? [] : canvasLinkClusters(canvas),
								scale: view.k,
								onRemove: (linkId) => {
									if (canvasId === null) return;
									if (canvasLinkRemove(canvasId, linkId)) setHint("已删除这条连线");
								},
								onExplain: () => setHint("连线的初始含义：从其中任一张卡片发起会话时，这些卡片的工作区上下文共享（互相的内容会一起带给 agent）。点连线上的标签可删除。")
							}),
							canvas === null
								...
								: Object.values(canvas.cards).map((card) => card.bare === true
									? (0, react_jsx_runtime.jsx)(CanvasControlView, {
										...
										onRebuild: (id) => {
											const current = canvas.cards[id];
											if (current === void 0 || canvasId === null) return;
											if (rebuildHostedPlugin(canvasId, current)) setHint("已重建插件项：" + current.plugin.id);
										},
										onDelete: (id) => {
											const current = canvas.cards[id];
										...
										onBlockReplace: (id, blockIndex, block) => {
											const current = canvas.cards[id];
											if (current === void 0 || canvasId === null) return;
											if (canvasReplaceBlock(canvasId, current, blockIndex, block) !== null) setHint("已换成本地图片（不再依赖网络）");
										},
										onLink: (id) => {
											setLinkPoint(null);
											setLinkFrom(id);
											setHint("连线中：点另一项完成连线（初始含义：工作区上下文共享）");
										},
										linkCount: canvasLinksOf(canvas, card.id).length
									}, card.id)
									...
									onLink: (id) => {
										setLinkPoint(null);
										setLinkFrom(id);
										setHint("连线中：点另一张卡片完成连线（初始含义：工作区上下文共享）");
									},
									linkCount: canvasLinksOf(canvas, card.id).length
								}, card.id))
							] })
					...
					// Drawing a link: the overlay swallows the pointer, follows it with a rubber
					// band and completes the edge on the item that is clicked next.
					linkFrom !== null && (0, react_jsx_runtime.jsxs)("div", {
						className: "dsh-tc-linkMode",
						onPointerMove: (event) => setLinkPoint(planePointOf(event)),
						onPointerLeave: () => setLinkPoint(null),
						onPointerDown: (event) => {
							const target = itemAtPoint(planePointOf(event));
							if (target === null) {
								setHint("没点到卡片：点另一张卡片完成连线，Esc 取消");
								return;
							}
							if (target.id === linkFrom) {
								setLinkFrom(null);
								setLinkPoint(null);
								setHint("已取消连线");
								return;
							...
							if (canvasId === null) return;
							const link = canvasLinkAdd(canvasId, linkFrom, target.id, "context");
							setLinkFrom(null);
							setLinkPoint(null);
							setHint(link === null ? "无法连线：两项必须都在这个画布上" : "已连线：" + (TASK_LINK_LABELS[link.kind] ?? "连线") + "（虚线框内的工作区上下文会共享）");
						},
						children: [
							(0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-linkModeHint", children: "连线中：点另一张卡片完成连线（Esc 或右键取消）" }),
							(0, react_jsx_runtime.jsx)("svg", {
								className: "dsh-tc-linkModeBand",
								children: (() => {
									const source = boxes[linkFrom] === void 0 ? null : { x: boxes[linkFrom].x + boxes[linkFrom].w / 2, y: boxes[linkFrom].y + boxes[linkFrom].h / 2 };
									if (source === null || linkPoint === null) return null;
									const base = currentView();
									const k = base.k > 0 ? base.k : 1;
									return (0, react_jsx_runtime.jsx)("line", {
										className: "dsh-tc-linkEdge",
										x1: source.x * k + base.x,
										y1: source.y * k + base.y,
										x2: linkPoint.x * k + base.x,
										y2: linkPoint.y * k + base.y
									});
								})()
							})
						]
					}),
					canvas !== null && (0, react_jsx_runtime.jsx)(CanvasChatDock, {
						canvas,
						canvasId,
						adoptOnApply: boundId !== null,
						onApply: () => {
							// Belt and braces: the chat adopts right after it applies an [canvas]
							// spec; this covers every other route into the same canvas (quick
							// editor, manual "+ 控件", a refactor landing late), so a control the
							// user sees on the plane cannot be lost on the way back to the board.
							if (boundId !== null) canvasAdoptItems(canvasId);
							return canvasLastAgentResult;
						}
					}),
					...
					fullCard !== null && (0, react_jsx_runtime.jsx)(TaskCardFullscreen, {
						...
						children: (0, react_jsx_runtime.jsx)(TaskUserBodyView, {
							...
							onBlockReplace: (id, blockIndex, block) => {
								const current = canvasSnapshot().canvases[canvasId]?.cards[id];
								if (current === void 0) return;
								canvasReplaceBlock(canvasId, current, blockIndex, block);
							}
						})
					})
				]
		...
		/** sha256 of the decoded viewer/viewer.html, asserted by the smoke test. */
		const TASK_VIEWER_SHA256 = "82b84b93feea19e8d4403af1984a83464225133dea88efcc78184897d751c406";
		const TASK_VIEWER_GZIP_B64 = "";
		let taskViewerHtmlCache = null;
		let taskViewerHtmlPromise = null;
		/** Decompress the embedded 3D viewer page into HTML text (shared by every model item). */
		function taskViewerHtml() {
			if (taskViewerHtmlCache !== null) return Promise.resolve(taskViewerHtmlCache);
			if (taskViewerHtmlPromise !== null) return taskViewerHtmlPromise;
			const pending = (async () => {
				if (TASK_VIEWER_GZIP_B64.length === 0) throw new Error("这个构建里没有内嵌 3D 查看器（运行 node tools/embed-viewer.mjs 后重新打包）");
				if (typeof DecompressionStream !== "function" || typeof atob !== "function") throw new Error("当前浏览器不支持 DecompressionStream，无法内嵌渲染 3D 查看器");
				const binary = atob(TASK_VIEWER_GZIP_B64);
				const bytes = new Uint8Array(binary.length);
				for (let index = 0; index < binary.length; index++) bytes[index] = binary.charCodeAt(index);
				const stream = new Blob([bytes]).stream().pipeThrough(new DecompressionStream("gzip"));
				const html = await new Response(stream).text();
				if (typeof html !== "string" || html.length === 0) throw new Error("3D 查看器内容为空");
				taskViewerHtmlCache = html;
				return html;
			})();
			taskViewerHtmlPromise = pending;
			pending.catch(() => {
				if (taskViewerHtmlPromise === pending) taskViewerHtmlPromise = null;
			});
			return pending;
		}
		...
		//#endregion
		//#region 工作时序 card (host integration with dsh-plugin-tpt-chronicle)
		/**
		 * Preset board card "工作时序" (work chronicle). Unlike the other built-in cards
		 * this one renders another plugin's panel: the TPT client bundle is materialized
		 * through the client module system (`ctx.modules`) and asked to mount itself into
		 * a node this card owns, so the panel keeps its own DOM, styles and API access
		 * while living inside the card's React tree. When the plugin is absent the card
		 * degrades to an explicit install hint.
		 */
		const TPT_PLUGIN_ID = "dsh-plugin-tpt-chronicle";
		/** One shared load per page: the bundle registers itself once. */
		let tptBundlePromise = null;

		/**
		 * Load the TPT client bundle as a script and return its imperative host API.
		 * The client module table exposes no public `import()` for another package, so the
		 * bundle publishes `globalThis.__DSH_TPT_CHRONICLE__` and this card drives it.
		 * @param modules - the client module system (`ctx.modules`), used only for the
		 * bundle's revisioned URL; the plugin also works from the raw boot global.
		 * @returns the plugin's host API.
		 */
		function loadTptBundle(modules) {
			const existing = globalThis.__DSH_TPT_CHRONICLE__;
			if (existing !== void 0 && typeof existing.mountEmbedded === "function") {
				return Promise.resolve(existing);
			}
			if (tptBundlePromise !== null) return tptBundlePromise;
			tptBundlePromise = (async () => {
				// Row lookup: the module system's own parsed manifest first, the raw boot
				// global second (they carry the same rows; either may be ahead after HMR).
				const rows = modules?.manifest?.entries ?? globalThis.__DSH_BOOT__?.entries ?? [];
				const row = rows.find((entry) => entry.id === TPT_PLUGIN_ID);
				if (row === void 0) {
					throw new Error("DSH 启动清单里没有 dsh-plugin-tpt-chronicle：请先安装并重启 DSH");
				}
				const alreadyInjected = document.querySelector("script[data-tpt-chronicle]") !== null;
				if (!alreadyInjected) {
					await new Promise((resolve, reject) => {
						const script = document.createElement("script");
						script.src = row.url;
						script.async = true;
						script.dataset.tptChronicle = "1";
						script.onload = () => resolve();
						script.onerror = () => reject(new Error("工作时序客户端包加载失败：" + row.url));
						document.head.appendChild(script);
					});
				}
				const api = globalThis.__DSH_TPT_CHRONICLE__;
				if (api === void 0 || typeof api.mountEmbedded !== "function") {
					throw new Error("工作时序客户端包没有发布挂载入口：请更新 dsh-plugin-tpt-chronicle");
				}
				return api;
			})();
			tptBundlePromise.catch(() => { tptBundlePromise = null; });
			return tptBundlePromise;
		}

		/** Last load report of the 工作时序 card, shown as its badge. */
		const chronicleStatus = { value: null };
		function publishChronicleStatus(next) {
			chronicleStatus.value = next;
		}

		/**
		 * The card body: a host node the TPT panel mounts into, plus the loading and
		 * failure states the card shows before the panel is live.
		 * @param props - `modules` is the client module system handed down by the board.
		 */
		function TptChronicleCardBody({ modules, mode = "compact" }) {
			const hostRef = (0, react.useRef)(null);
			...
			// The panel reports every load; keep the callback identity stable so the panel's
			// own effect (keyed on it) never loops.
			const statusRef = (0, react.useRef)(null);
			statusRef.current = (next) => publishChronicleStatus(next);
			// Mounted panel handle: `{ root, api, host }`, kept outside state so a theme
			// change re-renders the panel instead of remounting it.
			const panelRef = (0, react.useRef)(null);
			const darkRef = (0, react.useRef)(dark);
			darkRef.current = dark;
			// The pane's shape is the host's: a compact card gets the icon rail, an expanded
			// pane (fullscreen view) gets the panel's own wide layout.
			...
			(0, react.useEffect)(() => {
				...
				let disposed = false;
				let root = null;
				loadTptBundle(modules).then((plugin) => {
					if (disposed) return;
					if (plugin === void 0 || typeof plugin.mountEmbedded !== "function") {
						throw new Error("工作时序客户端包没有发布挂载入口：请更新 dsh-plugin-tpt-chronicle");
					}
					root = react_dom.createRoot(host);
					const mount = () => plugin.mountEmbedded(host, {
						render: (element) => root.render(element),
				...
				return () => {
					...
					// The card's root owns the host's children: unmounting is the teardown.
					unmountPluginRoot(root);
				};
			}, [modules]);
			// Theme or pane-shape flips re-render the mounted panel (its own root) with the
			// new mode; the host pane attributes are rewritten on the live node too.
			(0, react.useEffect)(() => {
				const host = hostRef.current;
				if (host !== null) {
					host.setAttribute("data-embed-dark", dark ? "1" : "0");
					host.setAttribute("data-embed-mode", mode === "full" ? "full" : "compact");
				}
				const panel = panelRef.current;
				if (panel !== null) panel.mount();
			}, [dark, mode, ready]);
			if (failure !== "") {
				return (0, react_jsx_runtime.jsx)("div", {
					className: "dsh-tc-empty",
					children: "工作时序不可用：" + failure
				});
			}
			return (0, react_jsx_runtime.jsx)("div", {
				ref: hostRef,
				className: "dsh-tc-tptHost",
				"data-embed-dark": dark ? "1" : "0",
				"data-ready": ready ? "1" : void 0
			});
		}
		//#endregion
		...
		function TaskBackPanel(props) {
			const { useSessions, useSessionStatus, onClose, plugins, modules } = props;
			// One hook call either way: the harness's status hook, or a constant stand-in when the
			// slot does not provide one (hooks must not be called conditionally).
			const statusSource = typeof useSessionStatus === "function" ? useSessionStatus : TASK_EMPTY_STATUS_HOOK;
			const statuses = statusSource((value) => value);
			const current = useSessions((state) => taskListCurrentId(state));
			const byId = useSessions((state) => state.byId);
			// `jobsBySession` only exists on older clients: guard the read on both fields.
			const jobs = useSessions((state) => (current === null || state.jobsBySession === void 0 || state.jobsBySession === null ? void 0 : state.jobsBySession[current]));
			const catalog = useSessions((state) => taskSessionSubagents(state, current, statuses));
			...
			// A [canvas] block written in the main conversation is applied by the watcher, so the
			// board shows what happened (and offers a click straight into that canvas).
			(0, react.useEffect)(() => {
				let timer = null;
				const off = subscribeTaskNotice((next) => {
					setNotice(next);
			...
			/**
			 * Swap one control of a board card in place — what the image control calls when its
			 * source cannot be loaded and the user picks a local file instead. The card's canvas
			 * copy follows, so the picture appears in both places.
			 */
			const replaceCardBlock = (0, react.useCallback)((id, blockIndex, block) => {
				const previous = stateRef.current;
				const card = previous.cards[id];
				if (card === void 0 || !isUserTaskCard(card)) return;
				const clean = sanitizeTaskBlock(block, 0);
				if (clean === null) return;
				const blocks = effectiveTaskBlocks(card).slice();
				if (blockIndex < 0 || blockIndex >= blocks.length) return;
				blocks[blockIndex] = clean;
				commit({ ...previous.cards, [id]: { ...card, blocks, updatedAt: Date.now() } }, previous.order);
				syncTaskCardInstance(id);
			}, [commit]);
			...
			const cardView = (id, mode = "compact") => {
				...
				if (id === "session") {
					...
					title = "工作时序";
					const artifacts = chronicleStatus.value === null ? void 0 : chronicleStatus.value.artifacts;
					badge = artifacts === void 0 ? void 0 : artifacts + " 份材料";
					fill = true;
					body = (0, react_jsx_runtime.jsx)(TptChronicleCardBody, { modules, mode: mode === "full" ? "full" : "compact" });
				} else if (isUserTaskCard(layout)) {
					title = layout.title ?? id;
					badge = layout.pinned === true ? "置顶" : void 0;
					body = layout.plugin !== void 0
						? (0, react_jsx_runtime.jsx)(PluginHostView, { plugin: layout.plugin, dark })
						: (0, react_jsx_runtime.jsx)(TaskUserBodyView, { card: layout, cardId: id, onWidgetState: setWidgetState, onBlockResize: resizeEmbedBlock, onBlockReplace: replaceCardBlock, thumbnail: true });
				} else {
					title = "工作区";
				...
				return { layout, title, badge, body, actions, isUser, confirmDeleteId, hasFillApp: hasFillApp || fill };
			};
			const fullView = fullCardId === null ? null : cardView(fullCardId, "full");
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("button", { type: "button", className: "dsh-tc-btn" + (bridgeOpen ? " dsh-tc-iconOn" : ""), title: "网页桥接：让 agent 操作卡片里内嵌的网页应用", onClick: () => setBridgeOpen((value) => !value), children: "🌉 桥接" }),
					...
					notice !== null && (0, react_jsx_runtime.jsx)("div", {
						className: "dsh-tc-notice",
						title: "点击打开这块画布",
						onClick: () => {
							if (notice.canvasId !== null) canvasSetActive(notice.canvasId);
							setInstance(null);
							setCanvasOpen(true);
							setNotice(null);
						},
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							bridgeOpen && (0, react_jsx_runtime.jsx)(TaskBridgePanel, { onClose: () => setBridgeOpen(false) }),
		...
		function TaskConsoleRoot(props) {
			const { useSessions, useSessionStatus } = props;
			const [open, setOpen] = (0, react.useState)(false);
			...
			/** Holds the floating composer's collapse-into-logo control. */
			const miniRef = (0, react.useRef)(null);
			...
			const moveInputFloating = (0, react.useCallback)(() => {
				...
				/**
				 * Collapse control: the whole floating input can shrink into a small logo
				 * chip in the corner and come back, so the board can be used with nothing
				 * but the logo on top of it. The composer itself is only hidden by CSS —
				 * React keeps it mounted, so nothing is lost while it is a logo.
				 */
				const miniToggle = document.createElement("button");
				miniToggle.type = "button";
				miniToggle.className = "dsh-tc-miniToggle";
				miniToggle.textContent = "—";
				miniToggle.title = "把输入框收成 Logo";
				const syncMini = (collapsed) => {
					seat.classList.toggle("dsh-tc-seatMini", collapsed);
					miniToggle.textContent = collapsed ? "🐋" : "—";
					miniToggle.title = collapsed ? "展开对话输入框" : "把输入框收成 Logo";
				};
				miniToggle.addEventListener("pointerdown", (event) => event.stopPropagation());
				miniToggle.addEventListener("click", (event) => {
					event.stopPropagation();
					syncMini(!seat.classList.contains("dsh-tc-seatMini"));
				});
				seat.appendChild(miniToggle);
				miniRef.current = { seat, button: miniToggle };
			}, []);
			...
			const restoreInput = (0, react.useCallback)(() => {
				...
				seat.classList.remove("dsh-tc-seatMini");
				const mini = miniRef.current;
				miniRef.current = null;
				if (mini !== null && mini.button !== null && mini.button.parentElement !== null) mini.button.remove();
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				...
				(open || closing) && portalTarget !== null ? (0, react_dom.createPortal)((0, react_jsx_runtime.jsx)("div", {
					...
					children: (0, react_jsx_runtime.jsx)(TaskBackPanel, { useSessions, useSessionStatus, onClose: requestClose, plugins: taskPluginsPort, modules: taskModulesPort })
				}), portalTarget) : null
			] });
		}
		//#endregion
		//#region plugin entry
		/**
		 * Required services: slots (shell.overlay), the client module system (the 工作时序
		 * card materializes another plugin through it), and the nested dynamic Cordis
		 * runner service behind the plugin-management card.
		 */
		const inject = ["slots", "modules", "remote", "remote.dynamicCordisRunner"];
		/**
		...
		function apply(ctx) {
			...
			// The 工作时序 card renders another plugin's panel through the module system.
			taskModulesPort = ctx.get("modules") ?? null;
		...
		exports.__dshTestHooks = { TptChronicleCardBody, loadTptBundle, TPT_PLUGIN_ID, chronicleStatus, TASK_CARD_DEFAULTS, instanceCardOf, canvasAdoptItems, TaskConsoleRoot, TaskBackPanel, TaskCard, TaskFlipToggle, TaskUserBody, TaskBlockView, TaskCardChat, TaskCardEditor, TaskCardFullscreen, TaskCanvasView, TaskDiagramDock, TaskEmbedView, taskDiagramHtml, taskDiagramWidthClamp, TASK_DIAGRAM_SHA256, taskViewerHtml, TASK_VIEWER_SHA256, readTaskModelFile, isTaskModelFile, TaskModelView, CANVAS_MODEL_INLINE, taskListCurrentId, taskSessionsListState, taskSessionSubagents, taskTempCreateOptions, createTaskCardAgentBridge, TASK_CANVAS_AGENT_TASK, cleanTaskEmbedUrl, taskEmbedHost, taskEmbedIsSameOrigin, clampTaskEmbedHeight, patchTaskEmbedHeight, taskEditorBlocks, TASK_EMBED_LIMIT, TASK_EMBED_SANDBOX, TASK_EMBED_SANDBOX_OPAQUE, TASK_BLOCK_KINDS, isTaskChatTextKind, assistantTextOfBlocks, assistantTextOfChunk, latestTaskAssistantText, latestTaskEventSeq, taskSeedCarriesPendingWork, taskWindowPromptFacts, taskSessionSnapshot, purgeTaskSessionQueue, waitTaskSessionIdle, taskTempCreateOptions, composeTaskCardChatPrompt, currentTaskSessionId, createTaskTempSession, createTaskBlankSession, openTaskChildSession, createTaskCardAgentBridge, TASK_CARD_AGENT_TASK, TASK_CANVAS_AGENT_TASK, composeCanvasChatPrompt, composePagePromptSection, parseCanvasSpec, applyCanvasSpec, canvasInventory, CanvasChatDock, canvasSnapshot, canvasCreate, canvasPublishCard, canvasOpenInstance, canvasInstanceId, isCanvasInstance, canvasDuplicateCard, canvasAddControl, canvasReplaceBlock, canvasSetBare, canvasUpdateControl, isBareCandidate, TaskImageView, CanvasLinkLayer, canvasLinkAdd, canvasLinkRemove, canvasLinksOf, canvasLinkClusters, sanitizeCanvasLink, TASK_LINK_KINDS, TASK_LINK_LABELS, linkedCardsOf, composeLinkPromptSection, composeCanvasLinkSection, taskBlocksAsContext, taskBlockDefault, TASK_CANVAS_ADD_KINDS, CanvasControlView, TaskBlockQuickEdit, PluginHostView, canvasAddPlugin, rebuildHostedPlugin, sanitizeTaskBlock, sanitizeTaskBlocks, coerceTaskBlockShape, taskBlockKindOf, describeTaskBlockRequest, applyConversationCanvasSpec, publishTaskNotice, subscribeTaskNotice, startTaskCardWatcher, stopTaskCardWatcher, pluginGlobalName, pluginBareName, pluginEntryNames, sanitizePluginRef, listPluginCandidates, listDynamicPlugins, pluginBundleUrl, findPluginEntry, loadPluginBundle, taskPluginState, publishTaskPlugin, subscribeTaskPlugin, composePluginPromptSection, TaskBridgePanel, taskBridgeTarget, taskBridgePageUrl, taskBridgePageState, setTaskBridgePageState, readTaskBridgeSettings, saveTaskBridgeSettings, freshTaskBridgeSettings, sanitizeTaskBridgeSettings, sanitizeTaskBridgeAction, parsePageSpec, describeBridgeAction, summarizeBridgeResults, pickBridgeTarget, runPageBlockFromReply, collectBridgeTargets, runTaskBridgeActions, pingTaskBridge, subscribeTaskBridge, subscribeTaskBridgePage, ensureTaskBridgeMonitor, releaseTaskBridgeMonitor, TASK_BRIDGE_ACTIONS, canvasSendToBoard, canvasExplodeBlocks, canvasRebuildInstance, canvasUnbindCard, canvasWriteBackBlock, canvasRemoveBoundBlock, canvasSyncBlockOrder, canvasSyncInstanceWithCard, syncTaskCardInstance, canvasPushBinding, canvasCardBinding, stampTaskCardBlocks, estimateTaskBlockHeight, taskBlockLabel, snapshotTaskCards, sanitizeCanvasCard, canvasUpsertCard, canvasDeleteCard, canvasSetActive, canvasRename, canvasJoinShare, canvasShareString, canvasApplySpec, canvasCopyCard, canvasCardFromEditor, mergeCanvasCard, mergeCanvasState, sanitizeCanvas, sanitizeCanvasState, encodeCanvasShare, decodeCanvasShare, freshTaskCards, loadTaskCards, saveTaskCards, fmtTaskDuration, taskJobDot, orderedTaskJobs, parseTaskCardSpec, sanitizeTaskButton, sanitizeTaskBlocks, sanitizeTaskStyle, nextUserTaskCardId, builtinTaskCardIds, cascadeTaskCardSpot, applyTaskCardSpec };
		exports.apply = apply;
