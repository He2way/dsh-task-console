...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TaskCard(props) {
			const { id, title, badge, layout, dragging, zIndex, boardRef, actions, onDragStart, onDragMove, onDragEnd, onResizeStart, onResizeMove, onResizeEnd, children } = props;
			...
			/** Bottom-right resize handle: pointer capture, clamped to the board and to the size bounds. */
			const resizing = (0, react.useRef)(false);
			const resizeOrigin = (0, react.useRef)({ px: 0, py: 0, w: 0, h: 0 });
			const onResizePointerDown = (event) => {
				...
				resizeOrigin.current = {
					px: event.clientX,
					py: event.clientY,
					w: card !== null ? card.offsetWidth : (typeof layout.w === "number" ? layout.w : 300),
					h: card !== null ? card.offsetHeight : (typeof layout.h === "number" ? layout.h : 200)
				};
			...
			const onResizePointerMove = (event) => {
				...
				let nextW = resizeOrigin.current.w + (event.clientX - resizeOrigin.current.px);
				let nextH = resizeOrigin.current.h + (event.clientY - resizeOrigin.current.py);
				nextW = Math.min(Math.max(TASK_CARD_MIN_W, nextW), TASK_CARD_MAX_W);
				nextH = Math.min(Math.max(TASK_CARD_MIN_H, nextH), TASK_CARD_MAX_H);
				const board = boardRef.current;
				if (board !== null) {
					const rect = board.getBoundingClientRect();
					nextW = Math.min(nextW, Math.max(TASK_CARD_MIN_W, rect.width - layout.x - 12));
					nextH = Math.min(nextH, Math.max(TASK_CARD_MIN_H, rect.height - layout.y - 12));
				}
				onResizeMove(id, Math.round(nextW), Math.round(nextH));
			};
		...
		function TaskBackPanel(props) {
			...
			const onResizeMove = (0, react.useCallback)((id, w, h) => {
				setState((previous) => ({ cards: { ...previous.cards, [id]: { ...previous.cards[id], w, h } }, order: previous.order }));
			}, []);
			const onResizeEnd = (0, react.useCallback)((id) => {
				setActiveId(null);
				const latest = stateRef.current;
				...
				if (card.style !== null && typeof card.style === "object" && typeof card.style.width === "string") {
					...
					delete style.width;
					next = Object.keys(style).length > 0 ? { ...card, style } : (() => {
						const copy = { ...card };
						delete copy.style;
						return copy;
					})();
				}
				commit({ ...latest.cards, [id]: next }, latest.order);
			}, [commit]);
