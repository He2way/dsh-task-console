	...
	factory: (require) => {
		...
		function composeCanvasChatPrompt(payload, text) {
			...
			return [
				...
				"规则：只输出 [canvas] 块；不要在块外写任何文字；一次可以给出多个 ops；引用已有项时必须使用上面列出的 id 原样。",
		...
		function applyCanvasSpec(canvasId, spec) {
			...
			const counts = { add: 0, addCard: 0, update: 0, move: 0, remove: 0, rename: 0, note: 0 };
			const notes = [];
			let skipped = 0;
			for (const op of spec.ops) {
				...
				if (op.op === "add") {
					if (canvasAddControl(canvasId, op.block.kind, op.x, op.y, op.block) !== null) counts.add += 1;
					else skipped += 1;
					continue;
				}
				if (op.op === "addCard") {
			...
			const changed = counts.add + counts.addCard + counts.update + counts.move + counts.remove + counts.rename;
			const parts = [];
		...
		function CanvasChatDock(props) {
			const { canvas, canvasId, onApply } = props;
			const [text, setText] = (0, react.useState)("");
			const [busy, setBusy] = (0, react.useState)(false);
				...
				children: mini
					...
					: [
						...
						open && (0, react_jsx_runtime.jsxs)("div", {
							...
							ref: bodyRef,
							children: [
								log.length === 0 && (0, react_jsx_runtime.jsx)("div", { className: "dsh-tc-chatHint", children: "说说你想怎么改这块画布，例如「把本地的清单和进度放到左上角，再加一个倒计时到明天 18 点」。" }),
