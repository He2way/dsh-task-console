...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TASK_CSS_EXTRA = [
			...
			".dsh-tc-card.dsh-tc-canvasCard{width:340px}",
		...
		function TaskUserBody(props) {
			const { card, onWidget, onWidgetState, onBlockResize, cardId } = props;
		...
		/**
		 * Maximize one card into its own canvas — a private workspace seeded with a copy of
		 * the card, like entering an instance: pan/zoom freely, derive more cards, invite
		 * others, and come back to the same place next time. The canvas is created on first
		 * use and reused afterwards, so the instance keeps its state.
		 */
		function canvasOpenInstance(card) {
			if (card === null || typeof card !== "object") return null;
			const state = canvasSnapshot();
			const id = canvasInstanceId(card.id);
			...
			const seed = canvasCopyCard(card, 90, 90);
			canvas.cards[seed.id] = seed;
			publishCanvasState({
				...state,
		...
		function CanvasCardView(props) {
			const { card, scale, onMove, onResize, onEdit, onChat, onDelete, onWidgetState, onBlockResize, onFull, onDuplicate, onBoard } = props;
		...
		function TaskBackPanel(props) {
			...
			const cardView = (id) => {
				...
				if (id === "session") {
					...
					body = (0, react_jsx_runtime.jsx)(TaskUserBodyView, { card: layout, cardId: id, onWidgetState: setWidgetState, onBlockResize: resizeEmbedBlock });
				} else {
					title = "工作区";
				...
				const actions = [
					{
						...
						onClick: () => {
							...
							const canvasId = canvasOpenInstance(layout);
							if (canvasId === null) {
								setFullCardId(id);
								return;
							}
