...
- **Draggable glassmorphism cards / 可拖拽毛玻璃卡片** — hold a card header and drag (pointer capture, clamped to the board); cards raise to front while dragging; every card can be collapsed (`▴/▾`); user cards can be pinned on top (`📌`); built-in cards can be hidden (`×`); positions/state persist in `localStorage` (`dsh.taskconsole.v1`); `复位卡片` restores the built-in layout while keeping your user cards.
...
3. Drag cards by their headers; `▴/▾` collapses a card, `×` hides a built-in card. Built-in cards are read-only.
