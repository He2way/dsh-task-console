...
- **Draggable & resizable glassmorphism cards / 可拖拽、可缩放毛玻璃卡片** — hold a card header and drag (pointer capture, clamped to the board); cards raise to front while dragging; **drag the bottom-right corner to resize any card** (220–900px wide, 120–1200px tall, clamped to the board); every card can be collapsed (`▴/▾`); user cards can be pinned on top (`📌`); built-in cards can be hidden (`×`); positions, sizes and state persist in `localStorage` (`dsh.taskconsole.v1`); `复位卡片` restores the built-in layout while keeping your user cards. Gestures are **compositor-driven**: pointer moves apply a transform (or an inline size) inside a `requestAnimationFrame` and the board commits React state / storage once on release. While a gesture runs the board switches to a cheap rendering path — every card drops its `backdrop-filter`, ambient animation pauses, and card bodies are memoized so unrelated re-renders never rebuild them; once the flip settles the rotated front face is culled (`visibility` + `content-visibility: hidden`) so the compositor only handles the board. `⚡ 低特效` in the board header (persisted per browser) turns the glass blur and ambient animation off entirely for low-power machines.
