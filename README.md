...
- **Maximize a card → it becomes a canvas instance / 卡片最大化后变成画布（副本）** — a user card's `⛶` opens **the card's own canvas instance**: a private workspace seeded with an independent copy of that card, like entering an instance in a game. Inside you can pan/zoom, rearrange, resize, drop images, refactor cards by talking, derive variants with `⧉`, and push a card back to the board with `📥 发回任务台`. The instance is created on first use and **reused afterwards** (canvas `cv-inst-<cardId>`, named `副本 · <title>`) so it keeps its state; the bar swaps the canvas switcher for `⟲ 放入源卡副本` / `删除副本` and `返回任务台`, and instance canvases stay out of the normal canvas list. It is a normal canvas underneath, so share strings and relay collaboration work there too. Built-in read-only cards keep the plain fullscreen view.
