...
- **Anything but cards / 画布上可以直接放任何控件** — the canvas is not limited to cards: `+ 控件` drops **any control straight onto the plane with no card around it** (text, heading, note, stats, progress, trend, key-values, links, chips, checklist, counter, button, table, code, toggle, countdown, bars, embedded web app — 18 kinds, each with starter content). A bare control is dragged by its hover toolbar (or by any non-interactive part of itself), resized from its corner, edited with `✎` (a plain field for the simple kinds plus a **raw-JSON editor** with the same validation for everything else), refactored by talking (`💬`), duplicated (`⧉`), sent to the board (`📥`) and deleted (`🗑`). Pasted/dropped images become bare image controls too, and `▢`/`▣` flips any single-control item between the card look and the bare look in both directions. A conversation can create the same controls through `[canvas]`/`[taskcard]` blocks, and the parser accepts the shapes models really write: the canonical `{ "kind": "stats", "items": [ … ] }`, the kind used as a key (`{ "stats": [ … ] }`), Chinese kind names (`表格`, `计数器`, `折线图` …) and blocks with no kind at all when the fields give it away (`{ values: [1,2,3] }` → trend). Whatever still cannot be built is skipped **and named** in the summary (`已应用：新增控件 2 · 跳过 1（看板）`) rather than silently dropped.
- **Host any plugin, not just controls / 画布可以显示任何插件** — `🧩 插件` in the canvas bar lists every client bundle in the boot manifest (`window.__DSH_BOOT__.entries`) plus the dynamic plugins the plugin port reports, and picking one puts **that plugin's own UI** on the plane as an item (drag, resize, `✎`/`💬`/`⛶`/`⧉`/`📥`/`🗑`, `▢` for the bare look). The bundle is loaded **once per session** and every item of that plugin mounts into its own host node — no iframe, no second React copy, so the plugin keeps its own DOM, styles and API access. The item header shows a live badge (`待挂载` / `加载中…` / `已挂载` / `未挂载`) and `⟳` rebuilds it (fresh revision → bundle re-materialized → remounted). A conversation can do the same through two `[canvas]` ops:
  ...
  **Both conversations work / 主对话与专属对话都可以**: the canvas' own chat box applies a `[canvas]` block, and so does the **main conversation** — the chat-row watcher that already authors cards from `[taskcard]` also applies a `[canvas]` block found in any reply, to the canvas the block names (`"canvas": "名字或 id"`, created when it does not exist yet) or to the active canvas, and the board raises a clickable notice (`画布「发布画布」已应用：新增插件 1`) that opens exactly that canvas.
- **Links between cards / 卡片之间可以连线** — `🔗` on an item starts a link; clicking a second item draws the edge and the two (or more) items become a group wrapped in a **dashed frame**. The initial meaning of a link is **workspace context sharing**: a conversation started from any card of the group is handed the other cards' content as shared context (the card and canvas prompts carry a `共享的工作区上下文` section listing the neighbours and what they hold). The chip on the edge (`⚯ 工作区上下文共享 ×`) removes it; the frame's corner label explains the group. A conversation can do the same with `{ "op": "link", "from": …, "to": … }` / `{ "op": "unlink", … }`. Links sync over the relay like everything else (`link.upsert` / `link.delete`, last-writer-wins per edge) and disappear with the item they were attached to.
- **3D models on the canvas / 画布内嵌 3D 查看器（不需要 Blender）** — `+ 控件 → 3D 模型` gives you a viewer item; drag a `.glb`/`.gltf`/`.obj`/`.stl` onto the canvas (or onto the item) and it renders right there — orbit with the left button, pan with the middle button or shift-drag, wheel to zoom, plus `线框`/`网格`/`背景`/`重置视角` and a status line with triangles, meshes and size. The viewer is a dependency-free WebGL page embedded in the bundle (`viewer/viewer.html`, rebuilt with `npm run embed:viewer`), runs inside a sandboxed iframe, and needs no Blender and no network. Models up to ~450 KB are stored in the canvas state (they survive a reload and sync); bigger ones are shown for that session only and labelled `仅本机`. A `src` may also be an `https` URL or a `data:` payload, and a control with an empty `src` is simply a drop target.
- **Images** — drop files onto the canvas or paste from the clipboard; images are downscaled to a bounded JPEG (at most 700 KB of data URL) and stored as an `image` control. Larger images are refused with a hint. A remote `src` is fetched by **your browser**, so it can fail for reasons the plugin never sees (host unreachable from your network, hot-link protection, dead link, or a page URL instead of a picture): the control then says which host failed and why, and offers `用本地图片替换…` (pick a file → it is downscaled and written back as a data URL, no network involved), `重试`, `在新标签打开` and `复制地址` — so a picture never fails silently.
