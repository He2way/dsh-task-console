...
| `embed` | 内嵌其它网页应用（沙箱 iframe） | `url`（仅 http/https）, `title?`, `height?`(120–1200), `fill?` |
...
- 内嵌网页应用（`embed`）把别的网页应用放进卡片：`url` 必须是 `http://` 或 `https://` 开头的完整地址（例：`https://example.com/app`、`http://localhost:5173`），其它协议（`javascript:` / `data:` / `file:` / `blob:`）一律丢弃；`height` 是像素高度（120–1200，默认 300），`fill: true` 让应用铺满卡片剩余高度（建好卡片后把它拖大即可，一整张卡片就是一个应用窗口）。一张卡片最多 4 个 `embed`，并且可以和 `text` / `button` 等控件混排；卡片上的 ↻ 重新加载、↗ 在新标签打开、⧉ 复制地址。若某个站点禁止被嵌入（`X-Frame-Options` / CSP），画面会空白 —— 这是站点的限制，用 ↗ 打开即可。
