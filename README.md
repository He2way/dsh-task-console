...
- **The canvas has its own conversation box / 画布自带对话框** — every canvas carries a docked chat: describe the change in one sentence (“把清单和进度放到左上角，再加一个倒计时到明天 18 点”) and a temporary agent edits the canvas — adding any control or card, rewriting one, moving, removing, renaming — through a single `[canvas]` block of operations. The agent is handed a **full inventory of the plane** (ids, card vs bare control, controls, positions) so it references what is really there; `▴ 记录` shows the transcript, and the result is reported as `已应用：新增控件 1 · 移动 1 · 重命名画布`, with unusable operations skipped and counted instead of failing the batch.
